...
class Kraken:
    ...
    # metodo que obtiene la informacion de todos los assets disponibles en
    # Kraken
    @classmethod
    ...
    # metodo que genera el par tras recibir las dos monedas
    @classmethod
    def set_pair(cls, coin1, coin2):
        pair = coin1 + coin2
        return pair

    # metodo que recibe los parametros de Dia e Hora Inicio-Fin y los
    # convierte a unixtime
    @classmethod
    ...
    # metodo que recibe el par, y los unixtime y obtiene con esa informacion
    # los trades desde el unixtime de inicio hasta el unixtime del final
    @classmethod
    def get_recent_dates(cls, k, pair, from_date, to_date):
        ...
        while from_date < to_date:
            ...
            df_trades = df_trades.append(trades.reset_index(),
                                         ignore_index=True)
            from_date = int(last / 1e9)
            i += 1
        df_trades = df_trades[df_trades['dtime'] <=
                              k.unixtime_to_datetime(to_date)]
        df_trades.sort_values(by=['dtime'], ascending=True, inplace=True)
        return df_trades

    # metodo que hace el calculo del vwap para los precios y volumenes dados
    @classmethod
    ...
    # metodo que recibe los trades para el par indicado y la frecuencia por
    # la que quiere calcular el vwap
    @classmethod
    def get_df_vwap(cls, df_trades, frecuency):
        ...
        df_vwap = (df_trades.groupby(pd.Grouper(key='dtime', freq=frecuency)).
                   apply(
            lambda x: cls.calculate_vwap(x['price'], x['volume'])).
