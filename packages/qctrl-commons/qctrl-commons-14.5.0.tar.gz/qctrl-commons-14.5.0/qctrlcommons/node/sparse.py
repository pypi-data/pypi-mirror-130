...
class SparsePwcOperator(Node):
    ...
    Examples
    --------
    Sum two sparse PWC operators to build the Hamiltonian of a system.

    >>> constant_term
        <SparsePwc: operation_name="constant_sparse_pwc_operator", value_shape=(20, 20)>
    >>> drive_envelope
        <Pwc: name="pwc_signal_#1", operation_name="pwc_signal", value_shape=(), batch_shape=()>
    >>> type(drive_hamiltonian_value)
        <class 'scipy.sparse.coo.coo_matrix'>
    >>> drive_hamiltonian_value.shape
        (20, 20)
    >>> drive_term = graph.sparse_pwc_operator(
    ...     signal=drive_envelope, operator=drive_hamiltonian_value
    ... )
    >>> drive_term
        <SparsePwc: operation_name="sparse_pwc_operator", value_shape=(20, 20)>
    >>> hamiltonian = graph.sparse_pwc_sum([constant_term, drive_term])
    >>> hamiltonian
        <SparsePwc: operation_name="sparse_pwc_sum", value_shape=(20, 20)>

    Refer to the `How to simulate large open system dynamics
...
class ConstantSparsePwcOperator(Node):
    ...
    >>> type(constant_term_value)
        <class 'scipy.sparse.coo.coo_matrix'>
    >>> constant_term_value.shape
        (20, 20)
    >>> constant_term = graph.constant_sparse_pwc_operator(
    ...     duration=1e-7, operator=constant_term_value
    ... )
    >>> constant_term
        <SparsePwc: operation_name="constant_sparse_pwc_operator", value_shape=(20, 20)>
    >>> drive_term
