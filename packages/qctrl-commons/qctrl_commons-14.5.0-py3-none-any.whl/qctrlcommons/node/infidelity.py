...
class InfidelityPwc(Node):
    ...
    Examples
    --------
    Calculate infidelity of an X gate using a (2, 2) PWC Hamiltonian and target operation.

    >>> hamiltonian
        <Pwc: name="add_#0", operation_name="add", value_shape=(2, 2), batch_shape=()>
    >>> target_operation = graph.target(operator=np.array([[0, 1], [1, 0]]))
    >>> infidelity = graph.infidelity_pwc(
    ...     hamiltonian=hamiltonian, target=target_operation, name="infidelity"
    ... )
    >>> infidelity
        <Tensor: name=”infidelity”, operation_name=”infidelity_pwc”, shape=()>
    >>> optimization_result = qctrl.functions.calculate_optimization(
    ...     graph=graph, cost_node_name="infidelity", output_node_names=["infidelity"]
    ... )

    Refer to the `How to optimize controls with non-linear dependencies
    <https://docs.q-ctrl.com/boulder-opal/user-guides/how-to-optimize-controls-with-nonlinear-dependences>`_
...
class InfidelityStf(Node):
    ...
    Examples
    --------
    Calculate the infidelity of a (3, 3) STF Hamiltonian with respect to a target operation.

    >>> hamiltonian
        <Stf: operation_name="add", value_shape=(3, 3), batch_shape=()>
    >>> target_operator
        <Target: operation_name="target", value_shape=(3, 3)>
    >>> infidelity = graph.infidelity_stf(
    ...     sample_times=np.linspace(0, 100e-9, 200),
    ...     hamiltonian=hamiltonian,
    ...     target=target_operator,
    ...     name="infidelity"
    ... )
    >>> infidelity
        <Tensor: name="infidelity", operation_name="infidelity_stf", shape=()>
