...
class PwcOperation(Node):
    ...
    Examples
    --------
    Create a piecewise-constant signal with non-uniform segment durations and use it to construct a Hamiltonian.

    >>> omega_signal = graph.pwc(
            values=np.array([1.1e6, 1.2e6, 1.3e6]),
            durations=np.array([1.1e-6, 1.2e-6, 1.2e-6]),
            name="omega",
        )
    >>> omega_signal
    ...
    >>> system_hamiltonian
        <Pwc: name="pwc_operator_hermitian_part_#0", operation_name="pwc_operator_hermitian_part", value_shape=(2, 2), batch_shape=()>

    Refer to the `How to simulate quantum dynamics subject to noise with graphs
    <https://docs.q-ctrl.com/boulder-opal/user-guides/how-to-simulate-quantum-dynamics-subject-to-noise-with-graphs>`_
...
class PwcSignal(Node):
    ...
    Examples
    --------
    >>> raw_values = np.array([0, 1, 2, 3, 4])
    >>> omega_c_raw = graph.pwc_signal(
            duration=1e-6, values=raw_values, name="omega_c_raw"
        )
    >>> result = qctrl.functions.calculate_graph(
            graph=graph, output_node_names=["omega_c_raw"]
        )
    >>> result.output["omega_c_raw"]
        [{'value': 0.0, 'duration': 2e-07},
         {'value': 1.0, 'duration': 2e-07},
         {'value': 2.0, 'duration': 2e-07},
         {'value': 3.0, 'duration': 2e-07},
         {'value': 4.0, 'duration': 2e-07}]

    Refer to the `How to represent quantum systems using graphs
    <https://docs.q-ctrl.com/boulder-opal/user-guides/how-to-represent-quantum-systems-using-graphs>`_
    user guide to find the example in context.
    """
...
class ComplexPwcSignal(Node):
    ...
    Examples
    --------
    Create a complex piecewise-constant signal using previously defined moduli and phases.

    >>> moduli = graph.optimization_variable(64, 0, 200e3 * np.pi)
    >>> moduli
        <Tensor: name="optimization_variable_#0", operation_name="optimization_variable", shape=(64,)>
    >>> phases = graph.optimization_variable(64, 0, 2 * np.pi)
    >>> phases
        <Tensor: name="optimization_variable_#1", operation_name="optimization_variable", shape=(64,)>
    >>> drive = graph.complex_pwc_signal(
            moduli=moduli, phases=phases, duration=0.0002, name="drive"
        )
    >>> drive
        <Pwc: name="drive", operation_name="complex_pwc_signal", value_shape=(), batch_shape=()>

    Refer to the `How to optimize error-robust Mølmer–Sørensen gates for trapped ions
    <https://docs.q-ctrl.com/boulder-opal/user-guides/how-to-optimize-error-robust-molmer-sorensen-gates-for-trapped-ions>`_
    user guide to find the example in context.
    """
...
class PwcOperator(Node):
    ...
    Examples
    --------
    >>> pwc_operator = graph.pwc_operator(
            signal=graph.pwc_signal(
                durations=np.array([1e-7, 2e-7, 3e-7, 4e-7]),
                values=np.array([1e6, 2e6, 3e6, 4e6]),
            ),
            operator=np.diag([1, np.sqrt(2)], 1),
        )
    >>> pwc_operator
        <Pwc: name="pwc_operator_#1", operation_name="pwc_operator", value_shape=(3, 3), batch_shape=()>
...
class ConstantPwcOperator(Node):
    ...
    Examples
    --------
    Define a constant PWC operator and use it to construct a Hamiltonian.

    >>> operator = 2 * np.pi * 2e9 * np.diag([1.0, -1.0, 1.0, -1.0])
    >>> operator.shape
        (4, 4)
    >>> operator_node = graph.constant_pwc_operator(operator=operator, duration=1e-7)
    >>> operator_node
        <Pwc: name="constant_pwc_operator_#0", operation_name="constant_pwc_operator", value_shape=(4, 4), batch_shape=()>
    >>> hamiltonian = operator_node + other_terms
...
class ConstantPwc(Node):
    ...
    Examples
    --------
    Create a batch of constant PWC operators using an array of previously defined dephasing coefficients.

    >>> dephasing_coefficients.shape
        (101,)
    ...
    >>> dephasing_term
        <Pwc: name="truediv_#1", operation_name="truediv", value_shape=(2, 2), batch_shape=(101,)>

    Refer to the `How to evaluate control susceptibility to quasi-static noise
    <https://docs.q-ctrl.com/boulder-opal/user-guides/how-to-evaluate-control-susceptibility-to-quasi-static-noise>`_
    user guide to find the example in context.
    """
...
class PwcOperatorHermitianPart(Node):
    ...
    --------
    >>> omega
        <Pwc: name="pwc_#0", operation_name="pwc", value_shape=(), batch_shape=()>
    >>> sigma_m = np.array([[0, 1], [0, 0]])
    >>> operator = omega * sigma_m
    >>> operator
        <Pwc: name="multiply_#1", operation_name="multiply", value_shape=(2, 2), batch_shape=()
    >>> rabi_term = graph.pwc_operator_hermitian_part(operator=operator)
    >>> rabi_term
        <Pwc: name="pwc_operator_hermitian_part_#2", operation_name="pwc_operator_hermitian_part", value_shape=(2, 2), batch_shape=()>
    >>> hamiltonian = rabi_term + other_terms
...
class PwcSum(Node):
    ...
    --------
    >>> noise_x_terms
        [<Pwc: name="multiply_#0", operation_name="multiply", value_shape=(2, 2), batch_shape=()>,
         <Pwc: name="multiply_#1", operation_name="multiply", value_shape=(2, 2), batch_shape=()>,
         <Pwc: name="multiply_#2", operation_name="multiply", value_shape=(2, 2), batch_shape=()>]
    >>> total_noise_terms = graph.pwc_sum(noise_x_terms)
    >>> total_noise_terms
        <Pwc: name="total_noise_terms_#3", operation_name="pwc_sum", value_shape=(2, 2), batch_shape=()>
    >>> hamiltonian = total_noise_terms + other_terms
...
class SymmetrizePwc(Node):
    ...
    --------
    >>> half_gamma
        <Pwc: name="complex_pwc_signal_#0", operation_name="complex_pwc_signal", value_shape=(), batch_shape=()>
    >>> half_gamma.durations
        [1e-09 2e-09 3e-09]
    >>> gamma = graph.symmetrize_pwc(half_gamma, name="gamma")
    >>> gamma
        <Pwc: name="gamma", operation_name="symmetrize_pwc", value_shape=(), batch_shape=()>
    >>> gamma.durations
        [1e-09 2e-09 3e-09 3e-09 2e-09 1e-09]

    Refer to the `How to optimize controls with time symmetrization
    <https://docs.q-ctrl.com/boulder-opal/user-guides/how-to-optimize-controls-with-time-symmetrization>`_
    user guide to find the example in context.
    """
