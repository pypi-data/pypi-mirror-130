...
class TargetOperation(Node):
    ...
    Examples
    --------
    Define target operation for an X gate and use a PWC Hamiltonian of shape (2, 2)
    for calculating the infidelity of the system's time evolution.

    >>> target_operation = graph.target(operator=np.array([[0, 1], [1, 0]]))
    >>> target_operation
        <Target: operation_name="target", value_shape=(2, 2)>
    >>> hamiltonian
        <Pwc: name="add_#1", operation_name="add", value_shape=(2, 2), batch_shape=()>
    >>> infidelity = graph.infidelity_pwc(
    ...     hamiltonian=hamiltonian, target=target_operation
    ... )
