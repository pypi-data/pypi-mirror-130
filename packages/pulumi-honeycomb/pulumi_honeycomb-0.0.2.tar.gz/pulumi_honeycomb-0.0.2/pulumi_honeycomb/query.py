...
class QueryArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[str] query_json: A JSON object describing the query according to the [Query Specification](https://docs.honeycomb.io/api/query-specification/#fields-on-a-query-specification). While the JSON can be constructed manually, it is easiest to use the `Query` data source.
        """
    ...
    def query_json(self) -> pulumi.Input[str]:
        """
        A JSON object describing the query according to the [Query Specification](https://docs.honeycomb.io/api/query-specification/#fields-on-a-query-specification). While the JSON can be constructed manually, it is easiest to use the `Query` data source.
        """
...
class Query(pulumi.CustomResource):
    ...
    def __init__(__self__,
        ...
        test_query_get_query = honeycomb.get_query(calculations=[honeycomb.GetQueryCalculationArgs(
                op="AVG",
                column="duration_ms",
            )],
            filters=[honeycomb.GetQueryFilterArgs(
                column="duration_ms",
                op=">",
                value="200",
            )])
        test_query_query = honeycomb.Query("testQueryQuery",
            dataset="%s",
            query_json=test_query_get_query.json)
        ```
