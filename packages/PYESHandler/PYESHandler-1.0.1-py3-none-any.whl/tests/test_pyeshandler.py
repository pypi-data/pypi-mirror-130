...
class PYESHandlerTestCase(unittest.TestCase):
    DEFAULT_ES_SERVER = 'localhost'
    DEFAULT_ES_PORT = 9200

    def getESHost(self):
        return os.getenv('TEST_ES_SERVER',PYESHandlerTestCase.DEFAULT_ES_SERVER)

    def getESPort(self):
        try:
            return int(os.getenv('TEST_ES_PORT',PYESHandlerTestCase.DEFAULT_ES_PORT))
        except ValueError:
            return PYESHandlerTestCase.DEFAULT_ES_PORT
    ...
    def test_ping(self):
        handler = PYESHandler(hosts=[{'host': self.getESHost(), 'port': self.getESPort()}],
    ...
    def test_index_name_frequency_functions(self):
        ...
        handler = PYESHandler(hosts=[{'host': self.getESHost(), 'port': self.getESPort()}],
                               ...
                               use_ssl=False,
                               index_name_frequency=PYESHandler.IndexNameFrequency.DAILY,
                               raise_on_indexing_exceptions=True)
        self.assertEqual(
            handler._index_name_func.__func__(index_name),
            PYESHandler._get_daily_index_name(index_name)
        )
        ...
        handler = PYESHandler(hosts=[{'host': self.getESHost(), 'port': self.getESPort()}],
                               ...
                               es_index_name=index_name,
                               use_ssl=False,
                               index_name_frequency=PYESHandler.IndexNameFrequency.WEEKLY,
                               raise_on_indexing_exceptions=True)
        self.assertEqual(
            handler._index_name_func.__func__(index_name),
            PYESHandler._get_weekly_index_name(index_name)
        )
        ...
        self.assertEqual(
            handler._index_name_func.__func__(index_name),
            PYESHandler._get_monthly_index_name(index_name)
        )
        ...
        handler = PYESHandler(hosts=[{'host': self.getESHost(), 'port': self.getESPort()}],
                               ...
                               es_index_name=index_name,
                               use_ssl=False,
                               index_name_frequency=PYESHandler.IndexNameFrequency.YEARLY,
                               raise_on_indexing_exceptions=True)
        self.assertEqual(
            handler._index_name_func.__func__(index_name),
            PYESHandler._get_yearly_index_name(index_name)
        )
        ...
        handler = PYESHandler(hosts=[{'host': self.getESHost(), 'port': self.getESPort()}],
                               ...
                               es_index_name=index_name,
                               use_ssl=False,
                               index_name_frequency=PYESHandler.IndexNameFrequency.NEVER,
                               raise_on_indexing_exceptions=True)
        self.assertEqual(
            handler._index_name_func.__func__(index_name),
            PYESHandler._get_never_index_name(index_name)
        )
