...
class RelationalDbHelper(BaseSqlHelper, ABC):
    ...
    async def insert_one_data(self, table_name: str, data: dict):
        ...
        rs = await self.execute_sql(insert_sql, tuple(data.values()))
        return rs
    ...
    async def update_data(self, condition_wrapper: ConditionWrapper, param: dict):
        ...
        :return:
        """
        update_sql, values = self.prepare_update_sql(condition_wrapper, param)
        rs = await self.execute_sql(update_sql, list(param.values()) + values)
        return rs
