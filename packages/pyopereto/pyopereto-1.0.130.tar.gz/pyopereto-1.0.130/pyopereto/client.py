    ...
    def f_call(*args, **kwargs):
        for i in range(3):
            try:
                ...
                logger.debug('API request failed: {}'.format(str(e)))
                if i<2:
                    time.sleep(1)
                else:
                    raise e
    ...
    def create_process(self, service, agent=None, title=None, mode=None, service_version=None, **kwargs):
        ...
        * *service_version* (`string`) -- Version of the service to execute
        ...
        for i in range(3):
            ...
            try:
                if i > 0:
                    self.logger.info('Retry attempt #{i}..')
                    res = self._call_rest_api('post', '/search/processes',
                                    data={'filter': {'api_request_id': api_request_id}},
                                    error='Failed to search in processes')
                    if res:
                        return res[0]['id']
