...
class Chrome(selenium.webdriver.chrome.webdriver.WebDriver):
    ...
    def __init__(
        ...
        if platform.endswith("win32"):
            file = "~/appdata/roaming/undetected_chromedriver"
        elif platform.startswith("linux"):
            file = "{}/.local/share/undetected_chromedriver".format(Path.home())
        elif platform.endswith("darwin"):
            file = "~/Library/Application Support/undetected_chromedriver"
        else:
            file = "~/.undetected_chromedriver"

        patcher = file + '/chromedriver'

        if os.path.isfile(patcher) is not True:
            patcher = Patcher(
