...
def get_YTDPDF(username, password, groupnum,
    ...
    if response.status_code != 200:
        ...
        if download == True:
            ...
            if downloadDir:
                filepath = Path(downloadDir) / 'Downloads' / disposition[disposition.find('"'):].replace('"', '')
            else:
...
def get_csv(username, password,
    ...
    password (str):
        Your Windows password as a string. \n
    model (str):
        Name of a model specified in dbUI's testdb app.
        Default is 'Claims' \n
    ...
    if response.status_code != 200:
        ...
        if download == True:
            if params:
                filename = model.capitalize()
                for k, v in params.items():
                    filename = filename + '_' + k + '=' + v
                filename = filename + '.csv'
            else:
                filename = model.capitalize() + '.csv'
            
            if downloadDir:
                filepath = Path(downloadDir) / 'Downloads' / filename
            else:
