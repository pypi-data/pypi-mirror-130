...
def create_vpc(cluster):
    ...
    _apply_bright_tags(cluster.vpc, cluster.vpc)
...
def _create_internet_gateway(vpc, ec2):
    ...
    _apply_bright_tags(vpc, internet_gateway)
...
def _create_public_subnet(vpc, ec2):
    ...
    tags = _construct_bright_tags(vpc, public_subnet_name, BCM_TYPE_HEAD_NODE)
...
def _create_private_subnet(vpc, ec2):
    ...
    tags = _construct_bright_tags(vpc, private_subnet_name, BCM_TYPE_HEAD_NODE)
    kwargs["TagSpecifications"] = [{"ResourceType": "subnet", "Tags": tags}]
...
def _set_default_gateway_subnet(vpc, subnet, internet_gateway):
    ...
    _apply_bright_tags(vpc, route_table)
...
def _create_head_node(vpc, hostname, public_subnet, head_node_image, head_node_sg_id, node_sg_id):
    ...
    tags = _construct_bright_tags(vpc, head_node_instance_name, BCM_TYPE_HEAD_NODE)
...
def _construct_bright_tags(vpc, obj_name, bcm_type=None):
    tags = {
...
def _apply_bright_tags(vpc, obj, bcm_type=None):
    tags = _construct_bright_tags(vpc, obj.name, bcm_type)
    _retry_not_found(obj.create_tags, Tags=tags)
