...
__all__ = ['ConditionalEvent', 'get_event', 'routes2str']


def get_event(obj) -> Union[multiprocessing.Event, threading.Event]:
...
class ConditionalEvent:
    ...
    :param runtime_backend: The runtime type to decide which type of Event to instantiate
    :param events_list: The list of events that compose this composable event
    """

    def __init__(self, events_list, runtime_backend: str = 'thread'):
        super().__init__()
        self.event = None
        if runtime_backend == 'thread':
...
def routes2str(msg: 'bert2tf_pb2.Message', flag_current: bool = False) -> str:
