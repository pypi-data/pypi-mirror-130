...
from ..model.csv_rows.debug_exception_log_csv_row import DebugExceptionLogCsvRow
...
class Shield34Listener:
    ...
    def on_test_start(self, test_name, test_class_name, block_params=''):
        try:
            ...
            RunReportContainer.get_current_block_run_holder().blockRunContract.runContract = \
            ...
            RunReportContainer.get_current_block_run_holder().set_block_run_contract(
                ListenerUtils.save_or_update_block_run(block_run_contract))
            block_contract = RunReportContainer.get_current_block_run_holder().blockRunContract.blockContract
            RunReportContainer.add_report_csv_row(GeneralCsvRow(RunReportContainer.runContract.id,
                                                                block_contract.id,
                                                                self.parentSuiteName,
                                                                block_run_contract.startTimestamp,
                                                                block_start_timestamp_as_long,
                                                                self.suiteName,
                                                                test_class_name,
                                                                test_name, ""))
            ...
            RunReportContainer.add_report_csv_row(DebugExceptionLogCsvRow("Failed to on_test_start", e))
