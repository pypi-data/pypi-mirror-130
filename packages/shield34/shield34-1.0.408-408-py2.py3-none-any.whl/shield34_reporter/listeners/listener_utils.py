...
class ListenerUtils(object):

    @staticmethod
    def save_or_update_run(run_contract):
        if SdkAuthentication.isAuthorized:
            payload = json.dumps(run_contract.__dict__, default=encode_block_run_contract)
            command_line = str(sys.argv)
            if Shield34Properties.pabotlib_enabled and (
                    command_line.find("PABOT") > -1 or command_line.find("pabot") > -1):
                ...
                try:
                    ...
                    Shield34Logger.logger.console(
                        "Please make sure to use the flag --pabotlib to run the tests using Shield34 listener, "
                        "for example: pabot --pabotlib --listener "
                        "shield34_reporter.listeners.robot_listener.RobotListener")
    ...
    def save_sdk_details(sdk_contract):
        ...
        if request.status_code != 200:
            RunReportContainer.add_report_csv_row(
                DebugLogCsvRow("save_sdk_details failed {}".format(request.status_code)))
    ...
    def save_or_update_block_run(block_run_contract):
        if SdkAuthentication.isAuthorized:
            payload = json.dumps(block_run_contract.__dict__, default=encode_block_run_contract)
            headers = {'content-type': 'application/json',
                       'Authorization': 'Shield34-Project ' + SdkAuthentication.get_user_token()}
            ...
            if request.status_code == 200:
                ...
                block_contract_id = content_as_json['data']['id']
                return BlockRunContract(status, start_timestamp, end_timestamp, report_file_path, browser_details,
                                        window_resolution, driver_details, os_type, block_contract, run_contract,
                                        block_contract_id)

        raise Exception("Failed to save block run . ")

    @staticmethod
    def save_report(report_path, report_output_path, block_run_contract):
        from shield34_reporter.container.run_report_container import RunReportContainer
        CsvFileHandler.write_csv_file(os.path.join(report_path, 'report.csv'),
                                      RunReportContainer.get_current_block_run_holder().blockReport,
                                      ["timestamp", "row_sub_type", "row_type", "row_value"])
        general_report = [RunReportContainer.get_current_block_run_holder().generalReport]
        CsvFileHandler.write_csv_file(os.path.join(report_path, 'general.csv'), general_report,
                                      ["timestamp", "run_id", "block_id", "parent_suite_name", "run_start_timestamp",
                                       "test_start_timestamp", "suite_name", "test_class_name",
                                       "test_name" "test_params"])
        if block_run_contract.status == Status.PASSED:
            RunReportContainer.get_current_block_run_holder().get_dynamic_element_locator().save_dynamic_elements(
                report_path, 'webelement_locators_descriptor.json')
            ...
            TarFileHandler.make_tarfile(report_output_path, "reports.tar.gz", [os.path.join(report_path, 'report.csv'),
                                                                               os.path.join(report_path,
                                                                                            'general.csv')])
        s3_file_details = S3FileDetails(block_run_contract.runContract.id, block_run_contract.id, "reports.tar.gz",
                                        FileType.TAG_GZ_FILE.value)
        tar_gz_s3_file_details = TarGzS3FileDetails(block_run_contract.runContract.id, block_run_contract.id,
                                                    "reports.tar.gz")
        ...
        file_uploaded = ListenerUtils.upload_report_to_s3(pre_signed_url_contract,
                                                          os.path.join(report_path, "reports.tar.gz"))
        if file_uploaded:
    ...
    def save_report_file(s3_file_details):
        if SdkAuthentication.isAuthorized:
            ...
            if request.status_code != 200:
                RunReportContainer.add_report_csv_row(
                    DebugLogCsvRow("save_report_file failed {}".format(request.status_code)))

    @staticmethod
    def get_test_run_url(current_block_run):
        if SdkAuthentication.isAuthorized:
            payload = json.dumps(current_block_run.__dict__, default=encode_block_run_contract)
            headers = {'content-type': 'application/json',
                       'Authorization': 'Shield34-Project ' + SdkAuthentication.get_user_token()}
    ...
    def fetch_browser_network():
        try:
            ...
            if len(browser_network_data) > 0:
                path_to_save_files = os.path.join(
                    RunReportContainer.get_current_block_run_holder().get_current_test_folder(), 'har_files')
                files_to_compress = ListenerUtils.write_browser_network_data_to_files(browser_network_data,
                                                                                      path_to_save_files)

                Shield34Logger.logger.info(
                    "After filtering static resources compressing " + str(len(files_to_compress)) + " network files ")
                if len(files_to_compress) != 0:
                    current_time = str(int(round(time.time() * 1000.)))
                    from random import seed, random
                    file_name_to_save = "browser_network{}-{}.tar.gz".format(current_time, random())
    ...
    def write_browser_network_data_to_files(browser_network_data, path_to_save_files):
        ...
        for idx, browser_network in enumerate(converted_browser_network_data):
            saved_file = ListenerUtils.write_browser_network_entry_to_files(browser_network, file_index,
                                                                            path_to_save_files)
        ...
        for i in range(0, len(RunReportContainer.get_current_block_run_holder().proxyServers)):
            ...
            for entry in proxy_server.har['log']['entries']:
                try:
                    ...
                    if converted_browser_data is not None:
                        saved_file = ListenerUtils.write_browser_network_entry_to_files(converted_browser_data,
                                                                                        file_index, path_to_save_files)
                        if saved_file is not None:
                            file_index = file_index + 1
                            files_to_compress.append(saved_file)
                except Exception as e:
...
def encode_block_run_contract(obj):
    if isinstance(obj, RunContract):
        return obj.__dict__
    if isinstance(obj, BlockContract):
        return obj.__dict__
