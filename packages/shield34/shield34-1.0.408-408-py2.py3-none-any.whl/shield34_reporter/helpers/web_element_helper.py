...
def wait_for_textarea_to_be_writable(element, web_driver_wait):
    ...
    return web_driver_wait.until(lambda d: wait_for_writeable.is_text_area_writable())
    ...
    try:
        if is_element_totally_in_view_port(driver, element):
            ...
            RunReportContainer.add_report_csv_row(
                ElementPositionAtViewPortCsvRow(ElementPosition.PARTIALLY_AT_VIEW_PORT))
            driver.execute_script("arguments[0].scrollIntoView(true);", element)
            RunReportContainer.add_report_csv_row(PutElementInViewPortCsvRow(True))
    except WebDriverException as wde:
...
def is_element_totally_out_of_view_port_old(driver, element):
    try:
        ...
        return element_right_x < 0 or element_right_y < 0 or element_left_x > window_width or element_left_y > window_h
...
class WaitForElementVisibility(object):
    ...
    def is_web_element_visible(self, driver):
        ...
        opacity = driver.execute_script(
            "return window.getComputedStyle(arguments[0], null).getPropertyValue('opacity')",
            self.web_element)
        if opacity == "0":
...
class WaitForTextAreaWriteable(object):
    ...
    def is_text_area_writable(self):
...
def reallocate_web_element(driver, web_element):
    if web_element.id in RunReportContainer.get_current_block_run_holder().web_element_descriptors_map:
        locator = RunReportContainer.get_current_block_run_holder().web_element_descriptors_map[web_element.id]
        try:
            relocated_element = driver.find_element(locator.by, locator.expression)
            if relocated_element is not None:
                RunReportContainer.add_report_csv_row(ReallocateElementCsvRow(locator.by + ":" + locator.expression))
