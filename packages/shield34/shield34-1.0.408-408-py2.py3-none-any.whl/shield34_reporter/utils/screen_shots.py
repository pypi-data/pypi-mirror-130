...
class ScreenShot():
    ...
    @staticmethod
    def get_screenshot_file(driver, on_failure):
        try:
            return driver.get_screenshot_as_png()
        except Exception as e:
            if on_failure and RunReportContainer.get_current_block_run_holder().screen_shot_file_before_quit is not None:
                return RunReportContainer.get_current_block_run_holder().screen_shot_file_before_quit
            else:
                return None
    ...
    def capture_screen_shoot(file_name, placement, on_failure = False):
        try:
            ...
            if driver_not_closed:
                screen_shot_file = ScreenShot.get_screenshot_file(DriverUtils.get_current_driver(), on_failure)
