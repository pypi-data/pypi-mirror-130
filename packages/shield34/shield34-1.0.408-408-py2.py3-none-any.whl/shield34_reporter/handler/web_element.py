...
from shield34_reporter.model.csv_rows.internalactions import PutElementInViewPortCsvRow, \
    WaitForElementVisibilityCsvRow, WaitForElementClickabilityCsvRow, WaitForElementToBeWritableCsvRow, \
    WaitForNumberOfElementToBeMoreThanCsvRow, WaitForElementPresenceCsvRow
...
class ElementFindElements(SeleniumMethodHandler):
    ...
    def on_successful_action(self):
        ...
        self._add_web_element_descriptor(self.located_web_elements[0], self.orig_by, self.orig_locator)
    ...
    def handle_unsuccessful_action(self):
        web_elements, current_by, current_value = locate_elements_by_element_descriptor(self.driver, self.orig_by, self.orig_locator)
...
class ElementFindElement(SeleniumMethodHandler):
    def __init__(self, driver, element, find_element_method, by, locator):
        super(ElementFindElement, self).__init__(ActionName.WEB_ELEMENT_FIND_ELEMENT, WebElementFindElementCsvRow(
            element, driver, by + ": " + locator))
        self.by = by
        self.locator = locator
        self.orig_by = by
    ...
    def on_successful_action(self):
        ...
        RunReportContainer.get_current_block_run_holder().add_web_element(
            self.located_web_element.id, self.by + ": " + self.locator)
...
class ElementClickHandler(SeleniumMethodHandler):
    ...
    def handle_exception(self, exception):
        try:
            if isinstance(exception, ElementNotVisibleException) or \
                    isinstance(exception, ElementNotInteractableException) or \
                    isinstance(exception, ElementClickInterceptedException) or \
                    isinstance(exception, WebDriverException) and \
                    "is not clickable at point" in exception.msg:
    ...
    def wait_for_visibility(self):
        ...
        try:
            ...
            after = time.time()
            RunReportContainer.add_report_csv_row(
                WaitForElementVisibilityCsvRow(self.timeout, element_is_visible, after - before))
    ...
    def wait_for_enabled(self):
        ...
        try:
            ...
            after = time.time()
            RunReportContainer.add_report_csv_row(
                WaitForElementClickabilityCsvRow(self.timeout, element_is_clickable, after - before))
...
class ElementSendKeysHandler(SeleniumMethodHandler):
    ...
    def wait_to_be_writeable(self):
        ...
        try:
            ...
            after = time.time()
            RunReportContainer.add_report_csv_row(
                WaitForElementToBeWritableCsvRow(self.timeout, element_is_visible, after - before))
    ...
    def handle_exception(self, exception):
        try:
            if isinstance(exception, ElementNotVisibleException) or \
                    isinstance(exception, ElementNotInteractableException) and \
                    exception.msg.contains("is not clickable at point"):
                RunReportContainer.add_report_csv_row(PutElementInViewPortCsvRow(False))
                self.driver.execute_script("arguments[0].scrollIntoView(false);", self.element)
                return True
            elif isinstance(exception, TimeoutException):
            ...
            RunReportContainer.add_report_csv_row(
                DebugExceptionLogCsvRow("click element reallocate_web_element failed.", e))
        return False
