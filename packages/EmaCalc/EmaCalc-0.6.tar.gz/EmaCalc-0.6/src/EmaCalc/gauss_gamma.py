...
class GaussianRV:
    ...
    def initialize(cls,
        ...
        :param prec_a: scalar or 1D precision gamma shape parameter
        :param prec_b: 1D precision gamma inverse-scale parameter
            prec_a broadcast-compatible with len(prec_b) == len(loc)
        ...
        """
        # prec_old = PrecisionRV(b=np.asarray(scale)**2 / 2,
        #                        a=learned_weight / 2)
        # version 0.5.1
...
class PrecisionRV:  # ********** might be subclass of scipy.stats.gamma ??? **********
    ...
    def mean_inv(self):
        ...
        if np.all(self.a > 1):
            ...
            m_inv = np.full_like(self.b, np.nan)
            ok = self.a > 1.
            m_inv[ok] = self.b[ok] / (self.a[ok] - 1)
            return m_inv  # OR raise error?
...
class StudentRV:  # ***** might be subclass of scipy.stats.t ???
    ...
    def rvs(self, size=1):
        ...
        :param size: scalar number of sample vectors
