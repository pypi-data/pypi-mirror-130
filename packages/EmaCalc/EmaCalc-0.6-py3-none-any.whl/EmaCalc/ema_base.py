...
# = default GaussianRV precision for most EmaModel parameters
...
class PopulationMixtureBase:
    ...
    def initialize(cls, n_comp, emf, effects, rv_class,
        ...
        # logger.debug('E{1 / comp_prior.prec}= '
        #              + np.array_str(comp_prior.prec.mean_inv,
        #                             precision=5))
...
def _make_prior(n_parameters, n_beta,
    ...
    :param prior_attribute_scale: (optional) scale of interindividual variance
        of theta_{ni0} = attribute location at FIRST scenario category,
    :param prior_threshold_scale: (optional) scale of interindividual variance
        of eta = mapped response-interval widths
    :return: single GaussianRV instance
    """
    prec_a = PRIOR_PREC_A * np.ones(n_parameters)
    prec_b = PRIOR_PREC_B * np.ones(n_parameters)
    # if prior_attribute_scale is not None:
    #     for a_slice in attribute_slices:
    #         # prec_a[a_slice.start] = PRIOR_REF_PREC_SHAPE
    #         # prec_b[a_slice.start] = (PRIOR_REF_PREC_SHAPE - 1.) * restrict_attribute ** 2
    #         prec_a[a_slice][:n_beta] = PRIOR_REF_PREC_SHAPE
    #         # prec_b[a_slice][:n_beta] = (PRIOR_REF_PREC_SHAPE - 1.) * restrict_attribute ** 2
    #         # E{ 1 / beta.prec } = restrict_attribute ** 2
    #         prec_b[a_slice][:n_beta] = prior_attribute_scale ** 2
    #         # = expected inter-individual variance
    # if prior_threshold_scale is not None:
    #     for a_slice in attribute_slices:
    #         prec_a[a_slice][n_beta:] = PRIOR_REF_PREC_SHAPE
    #         # prec_b[a_slice][n_beta:] = (PRIOR_REF_PREC_SHAPE - 1.) * restrict_threshold ** 2
    #         # E{ 1 / eta.prec } = restrict_threshold ** 2
    #         prec_b[a_slice][n_beta:] = prior_threshold_scale ** 2
    for a_slice in attribute_slices:
