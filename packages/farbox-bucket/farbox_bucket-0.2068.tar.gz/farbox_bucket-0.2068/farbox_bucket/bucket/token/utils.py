...
from farbox_bucket.bucket.utils import get_bucket_site_configs, get_bucket_by_private_key, has_bucket, get_admin_bucket
...
def get_logined_bucket_by_token_readonly_allowed():
    bucket = get_logined_bucket_by_token()
    if bucket:
        return bucket
    raw_token = request.values.get("api_token") or request.values.get("token") or ""
    if raw_token and "-" in raw_token:
        raw_bucket, token = raw_token.split("-", 1)
        site_settings = get_bucket_site_configs(raw_bucket) or {}
        if token and site_settings.get("readonly_token") == token:
            return raw_bucket
    return None
