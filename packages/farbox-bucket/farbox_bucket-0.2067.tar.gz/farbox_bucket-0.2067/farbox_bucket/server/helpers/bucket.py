...
from farbox_bucket.bucket.utils import set_bucket_in_request_context, get_bucket_deleted_paths
...
def show_bucket_deleted_paths_for_web_request(bucket=None, cursor=None):
    bucket = bucket or get_logined_bucket_by_token_readonly_allowed()  # by api token
    if not bucket:
        return jsonify([])
    cursor = cursor or request.values.get('cursor')
    paths = get_bucket_deleted_paths(bucket, cursor=cursor)
