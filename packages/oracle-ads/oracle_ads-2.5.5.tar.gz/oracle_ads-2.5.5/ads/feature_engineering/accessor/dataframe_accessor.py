...
class ADSDataFrameAccessor(
    ...
    def model_schema(self, max_col_num: int = 1000):
        """
        Generates schema from the dataframe.

        Parameters
        ----------
        max_col_num : int -> max_col_num: (int, optional). Defaults to 1000.
        ...
        ads.feature_engineering.schema.Schema
            data schema.
        """
        if len(self._obj.columns) > max_col_num:
            raise ValueError(
                "The data size is too big, input schema is not auto generated."
            )
