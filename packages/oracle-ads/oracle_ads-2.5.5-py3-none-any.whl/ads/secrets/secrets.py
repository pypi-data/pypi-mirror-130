...
class SecretKeeper(Vault, ContextDecorator):
    ...
    OCI Vault provides a mechanism for safe storage and acess. SecretKeeper uses
    OCI Vault as a backend to store and retreive the credentials.
    ...
    def load_secret(
        ...
        **kwargs,
        ...
        export_env : str, Default False
            ...
            When `load_secret` is invoked using `with` statement, information exported as environment variable is unset before leaving the `with` scope
        export_prefix: str, Default ""
            Prefix to the environment variable that is exported.
        auth: dict, optional.
            By default authentication will follow what is configured using ads.set_auth API. Accepts dict returned from `ads.common.auth.api_keys()` or `ads.common.auth.resource_principal()`.
        storage_options: dict, optional.
            storage_options dict as required by `fsspec` library
        **kwargs:
            key word arguments accepted by the constructor of the class from which this method is invoked.
        ...
        >>> with APIKeySecretKeeper.load_secret(source="ocid1.vaultsecret.oc1..<unique_ID>",
        ...                         export_prefix="mykafka",
        ...                         export_env=True
        ...                        ) as apisecret:
        ...     import os
        ...     print("Credentials inside environment variable:", os.environ.get('mykafka.api_key'))
        ...
        ...     import pandas as pd
        ...     df2 = pd.DataFrame.ads.read_sql("select * from ATTRITION_DATA", connection_parameters=adw_creds2)
        ...     print(df2.head(2))
    ...
    def to_dict(self) -> dict:
        ...
        -------
        dict
            dict of credentials retrieved from the vault or set through constrctor.
        """
