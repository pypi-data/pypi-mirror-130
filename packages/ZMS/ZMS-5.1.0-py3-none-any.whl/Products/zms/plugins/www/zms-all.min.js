...
zmiParams['base_url']=base_url;if(self.location.href.indexOf('/manage')>0||self.location.href.indexOf('preview=preview')>0){var hilight_css=`
<style type="text/css">
body:not(.zmi).contentEditable.zmi-highlight:hover {cursor:pointer;background:unset !important;box-shadow:0 0 0 1px black}
body:not(.zmi).contentEditable.zmi-highlight:hover:before {content:" ";width:20px;height:20px;position:absolute;background-color:black;z-index:10;background-image:url("data:image/svg+xml;utf8,<svg width='20' height='20' viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'><circle fill='black' cx='256' cy='256' r='247.66' /><path d='m400 284c0 6.6-5.4 12-12 12h-92v92c0 6.6-5.4 12-12 12h-56c-6.6 0-12-5.4-12-12v-92h-92c-6.6 0-12-5.4-12-12v-56c0-6.6 5.4-12 12-12h92v-92c0-6.6 5.4-12 12-12h56c6.6 0 12 5.4 12 12v92h92c6.6 0 12 5.4 12 12z' fill='white'/></svg>")}
</style>`;$('head').append(hilight_css);$('.contentEditable').mouseover(function(evt){evt.stopPropagation();$(this).addClass('zmi-highlight')}).mouseout(function(evt){$(this).removeClass('zmi-highlight')}).click(function(evt){evt.stopPropagation();if(evt.target !="undefined"&&$.inArray(evt.target.nodeName.toLowerCase(),['a','button','input','select','textarea'])>-1){return}
...
});var index=bookmarks.indexOf(data_path);if(index >=0){$('.fa-bookmark-empty',this).removeClass("fa-bookmark-empty text-muted").addClass("fa-bookmark text-primary")} else {$('.fa-bookmark',this).removeClass("fa-bookmark text-primary").addClass("fa-bookmark-empty text-muted")}
})}
...
});var $change_dt=$(".zmi-change-dt,.zmi-created-dt");if($change_dt.length > 0){var fn=function(){$change_dt.each(function(){var $el=$(this);var mydate=$el.attr("title");if(typeof mydate!="undefined"){var myformat=getZMILangStr('DATETIME_FMT');var dtsplit=mydate.split(/[\/ .:]/);var dfsplit=myformat.split(/[\/ .:]/);df=new Array();for(dc=0;dc<dtsplit.length;dc++){df[dfsplit[dc]]=dtsplit[dc];df[dfsplit[dc].substr(1)]=parseInt(dtsplit[dc])}
...
})});$(".form-group label.col-lg-2 control-label.mandatory",this).each(function(){var $label=$(this);$label.prepend('<i class="fas fa-exclamation"></i>')});$("input,select,textarea",this).each(function(){var $this=$(this);$this.attr("data-initial-value",$this.val());$this.change(function(){if($this.val()==$this.attr("data-initial-value")){$this.removeClass("form-modified")} else {$this.addClass("form-modified")}
})});$('input.zmi-input-icon-clazz',this).each(function(){var $input=$(this);$input.wrap('<div class="input-group"></div>');$input.before('<span class="input-group-text"><i class="fas fa-invisible"></i></span>');$input.parent().children('span.input-group-text').wrap('<div class="input-group-prepend"></div>');var fn=function(){var $container=$input.parents(".input-group");var $i=$("i",$container);$i.attr("class",$input.val())};$input.change(fn).keyup(fn).change()});var fn_url_input_each=function(){var $input=$(this);var fmName=$input.parents("form").attr("name");var elName=$input.attr("name");$input.wrap('<div class="input-group"></div>');var $inputgroup=$input.parent();if($input.prop("disabled")){$inputgroup.append(''+'<div class="input-group-append">'+'<span class="input-group-text"><i class="fas fa-ban-circle"></i></span>'+'</div>')} else {$inputgroup.append(''+'<div class="input-group-append">'+'<a class="btn btn-secondary" href="javascript:;" onclick="return zmiBrowseObjs(\''+fmName+'\',\''+elName+'\',getZMILang())">'+'<i class="fas fa-link"></i>'+'</a>'+'</div>')}
...
var is_firefox=/firefox/i.test(navigator.userAgent);var icon_datepicker='far fa-calendar';var icon_datetimepicker='far fa-calendar';var icon_timepicker='far fa-clock';$("input.datepicker,input.datetimepicker,input.timepicker",this).each(function(){$(this).closest("div").addClass("input-group").removeClass("col-sm-9").removeClass("col-sm-10").removeClass("col-md-10").wrap('<div class="col-sm-5 col-md-4 col-lg-3"></div>');var icon_picker=eval('icon_'+$(this).prop('class').split(' ').filter(function(v){return v.endsWith('picker')}));$(this).before('<div class="input-group-prepend"><span class="input-group-text"><i class="'+icon_picker+'"></i></span></div>');if(is_firefox){if($(this).attr('type')=='datetime-local'){$(this).attr('type','date')}
...
if(typeof opt['iframe'] !='undefined'){var width='100%';var height=typeof opt['height']=='undefined' ? '100%':opt['height'];opt['body']='<iframe src="'+url+'" width="'+width+'" height="'+height+'" frameBorder="0"></iframe>';zmiModal(null,opt);$ZMI.setCursorAuto("zmiIframe")} else {$.get(href,data,function(result){var $result=$(result);if($("div#system_msg",$result).length>0){var manage_tabs_message=$("div#system_msg",$result).text();manage_tabs_message=manage_tabs_message.substr(0,manage_tabs_message.lastIndexOf("("));var href=self.location.href;href=href.substr(0,href.indexOf("?"))+"?lang="+getZMILang()+"&manage_tabs_message="+manage_tabs_message;self.location.href=href} else {opt['body']=result;if(typeof opt['title']=="undefined"){var title=$("div.zmi",result).attr("title");if(typeof title !="undefined"&&title){opt['title']=title}}
...
html+='<a href="'+page_absolute_url+'"'+' data-link-url="'+link_url+'"'+' data-uid="'+page_uid+'"'+' data-page-physical-path="'+page_physical_path+'"'+' data-anchor="'+anchor+'"'+' data-page-is-page="'+page_is_page+'"'+' data-page-titlealt="'+page_titlealt.replace(/"/g,'&quot;').replace(/'/g,'&apos;')+'"'+' onclick="return zmiSelectObject(this);return false;">';if(!page_is_pageelement){html+='<i class="'+page_icon+'"></i> '}
...
var base=$ZMI.getPhysicalPath();base=base.substr(0,base.indexOf('/'+homeId));$container.append('<div id="loading" class="zmi-page"><i class="fas fa-spinner fa-spin"></i>&nbsp;&nbsp;'+getZMILangStr('MSG_LOADING')+'</div>');var params={lang:getZMILang(),preview:'preview',physical_path:$('meta[name=physical_path]').attr('content'),'get_attrs:int':0};if(typeof that.p["params"]=="object"){for(var i in that.p["params"]){params[i]=that.p["params"][i]}}
...
ZMIActionList.prototype.exec=function(sender,label,target){var id_prefix=this.getContextId(sender);if(typeof id_prefix !='undefined'&&id_prefix !=''){id_prefix=id_prefix.replace(/(\d*?)$/gi,'')} else {id_prefix='e'}
...
function zmiToggleSelectionButtonClick(sender,v){var $fm=$(sender).parents('form,.zmi-form-container');var $inputs=$('input:checkbox:not([id~="active"]):not([id~="attr_dc_coverage"])',$fm);if(typeof v=="undefined"){v=!$inputs.prop('checked')}
