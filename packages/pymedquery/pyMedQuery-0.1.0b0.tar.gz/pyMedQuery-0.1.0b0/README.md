...
The average user is expected to instantiate the `MedQuery()` class and run the methods `extract` or `batch_extract` to extract the necessary data for their given project. If the user wants to use the default SQL script then set the argument `get_all` to true in the `(batch_)extract` method and specify which projectID the data should relate to. The method will then extract all the data beloning to that projectID. Other users might find it usefull to write custom SQL scripts where the argument `format_params` can be passed into the extraction for flexible formating of SQL queries. This is especially useful for applications and pipelines.
...
#### 1. installing and completing the settings (this a one time procedure)

1. Use poetry or any other kind of dependency manager to install the package.

```$ poetry add pymedquery```

2. If you are using poetry then run the `init` script to setup the database authentication. (A vaid username, password and cert files are currently required in forehand) The setup script will guide you through the authentication.

```$ poetry exec mq-init```

<details>
<summary>If you don't have poetry installed or you are one Windows:</summary>
<br>
...
```python
from pymedquery import MedQuery

# instantiate the class
mq = MedQuery()
small_data_sample = mq.extract(get_all=True, project_id='fancyID')
        # do more stuff here
...
small_data_sample = mq.extract(get_all=False, format_params=FORMAT_PARAMS, sql_file_path=config.SQL_FILE_PATH)
        # do more stuff
...
large_data = mq.batch_extract(get_all=True, project_id='fancyID', batch_size=20)
...
##### The project Skeng is primarily developed by the research unit CRAI at the university hospital of Oslo
