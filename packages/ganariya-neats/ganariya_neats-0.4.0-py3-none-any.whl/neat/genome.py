...
from typing import Dict, List, Optional, Type, Union, Tuple, Set
...
class DefaultGenomeConfig(object):
    ...
    def __init__(
        self,
        params: Dict[
            str, Union[str, Type[DefaultNodeGene], Type[DefaultConnectionGene]]
        ],
    ) -> None:
        # parameters
...
class DefaultGenome(object):
    ...
    def mutate(self, config: DefaultGenomeConfig) -> None:
        """Mutates this genome."""
    ...
    def mutate_add_node(self, config: DefaultGenomeConfig) -> None:
    ...
    def mutate_delete_node(self, config: DefaultGenomeConfig) -> int:
    ...
    def size(self) -> Tuple[int, int]:
        ...
        """
        num_enabled_connections: int = sum(
    ...
    def create_connection(
        config: DefaultGenomeConfig, input_id: int, output_id: int
    ) -> DefaultConnectionGene:
        connection: DefaultConnectionGene = config.connection_gene_type(
            (input_id, output_id)
        )
        connection.init_attributes(config)
        return connection
    ...
    def compute_full_connections(
        self, config: DefaultGenomeConfig, direct: bool
    ) -> List[Tuple[int, int]]:
        """
    ...
    def connect_full_direct(self, config: DefaultGenomeConfig):
        ...
        for input_id, output_id in self.compute_full_connections(config, True):
            connection: DefaultConnectionGene = self.create_connection(
                config, input_id, output_id
            )
            self.connections[connection.key] = connection
