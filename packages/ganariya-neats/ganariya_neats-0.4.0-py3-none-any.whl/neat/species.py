...
class GenomeDistanceCache(object):
    ...
    def __call__(self, genome0: DefaultGenome, genome1: DefaultGenome) -> float:
        g0: int = genome0.key
        g1: int = genome1.key
...
class DefaultSpeciesSet(DefaultClassConfig):
    ...
    def speciate(
        self, config: Config, population: Dict[int, DefaultGenome], generation: int
    ):
        """
        ...
        for sid, rid in new_representatives.items():
            ...
            member_dict: Dict[int, DefaultGenome] = dict(
                (gid, population[gid]) for gid in members
            )
    ...
    def get_species(self, individual_id: int) -> Species:
        """
        個体iが所属する種を返す
        """
