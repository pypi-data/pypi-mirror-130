...
class ConfigParameter(object):
    ...
    **実際の値はもたない。**
    ...
    def __repr__(self):
        ...
        return "ConfigParameter({!r}, {!r}, {!r})".format(
            self.name, self.value_type, self.default
        )
    ...
    def interpret(self, config_dict: Dict[str, str]) -> ConfigInstanceType:
        ...
        if value is None:
            if self.default is None:
                ...
                warnings.warn(
                    "Using default {!r} for '{!s}'".format(self.default, self.name),
                    DeprecationWarning,
                )
                if (str != self.value_type) and isinstance(
...
class DefaultClassConfig(object):
    ...
    def __init__(
        self, param_dict: Dict[str, str], param_list: List[ConfigParameter]
    ) -> None:
...
class Config(object):
    ...
    def __init__(
        self,
        genome_type: Type[DefaultGenome],
        reproduction_type: Type[DefaultReproduction],
        species_set_type: Type[DefaultSpeciesSet],
        stagnation_type: Type[DefaultStagnation],
        filename: str,
    ) -> None:
        ...
        for p in self.__params:
            if p.default is None:
                ...
                try:
                    ...
                    warnings.warn(
                        "Using default {!r} for '{!s}'".format(p.default, p.name),
                        DeprecationWarning,
                    )
            param_list_names.append(p.name)
        ...
        species_set_dict: Dict[str, str] = dict(
            parameters.items(species_set_type.__name__)
        )
        self.species_set_config: DefaultClassConfig = species_set_type.parse_config(
            species_set_dict
        )

        stagnation_dict: Dict[str, str] = dict(
            parameters.items(stagnation_type.__name__)
        )
        self.stagnation_config: DefaultClassConfig = stagnation_type.parse_config(
            stagnation_dict
        )

        reproduction_dict: Dict[str, str] = dict(
            parameters.items(reproduction_type.__name__)
        )
        self.reproduction_config: DefaultClassConfig = reproduction_type.parse_config(
            reproduction_dict
        )
