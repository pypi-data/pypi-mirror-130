...
class FlaskReactize:
    ...
    def __start_react_server(self, source_react_folder: str, port: int) -> None:
        ...
        # In case there is already a server running (refresh for instance),
        # start a new one only if no server is found
        if node_wrapper.is_running():
            rprint("There is a already a running React application.")
        else:
            # Create the underlying nodejs server
            rprint("Starting React application (nodejs) ...")
