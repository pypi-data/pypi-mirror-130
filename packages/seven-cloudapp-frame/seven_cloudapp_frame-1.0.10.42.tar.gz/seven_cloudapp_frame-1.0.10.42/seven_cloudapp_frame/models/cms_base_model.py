...
@LastEditTime: 2021-12-03 10:58:29
...
class CmsBaseModel():
    ...
    def get_cms_info_list(self, place_id, page_size, page_index, order_by="id desc", app_id="", act_id=0, is_cache=True):
        ...
        cms_info_model = CmsInfoModel(context=self.context)
        if is_cache == True:
            page_list, total = cms_info_model.get_cache_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params,dependency_key=f"cms_info_list:placeid_{place_id}",cache_expire=600)
        else:
            page_list, total = cms_info_model.get_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
