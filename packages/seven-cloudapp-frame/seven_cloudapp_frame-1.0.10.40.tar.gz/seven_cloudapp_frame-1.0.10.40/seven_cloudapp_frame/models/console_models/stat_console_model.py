...
@LastEditTime: 2021-12-06 13:38:44
        ...
        while True:
            try:
                ...
                if not stat_queue_json:
                    time.sleep(1)
                    continue
                stat_queue = json.loads(stat_queue_json)
                try:
                    sub_table = SevenHelper.get_sub_table(stat_queue.act_id, config.get_value("stat_sub_table_count", 0))
                    stat_log_model = StatLogModel(sub_table=sub_table, db_transaction=db_transaction)
                    stat_report_model = StatReportModel(sub_table=sub_table,db_transaction=db_transaction)
                    ...
                    if stat_queue.process_count <= 10:
                        redis_init.rpush(f"stat_queue_list:{mod_value}", SevenHelper.json_dumps(stat_queue))
                    else:
