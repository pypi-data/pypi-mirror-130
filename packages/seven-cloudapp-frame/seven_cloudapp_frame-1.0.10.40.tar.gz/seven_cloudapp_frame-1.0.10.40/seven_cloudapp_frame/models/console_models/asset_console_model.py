...
@LastEditTime: 2021-12-06 15:10:15
        ...
        while True:
            try:
                ...
                if not asset_queue_json:
                    time.sleep(1)
                    continue
                asset_queue = json.loads(asset_queue_json)
                try:
                    old_asset_inventory_id = 0
                    app_id = asset_queue["app_id"]
                    act_id = asset_queue["act_id"]
                    open_id = asset_queue["open_id"]
                    user_nick = asset_queue["user_nick"]
                    user_id = asset_queue["user_id"]
                    asset_type = asset_queue["asset_type"]
                    asset_object_id = asset_queue["asset_object_id"]
                    now_day_int = asset_queue["now_day_int"]
                    create_date = asset_queue["create_date"]
                    now_value = asset_queue["now_value"]
                    operate_value = asset_queue["operate_value"]
                    history_asset_value = asset_queue["history_asset_value"]
                    ...
                    asset_queue.process_count += 1
                    if asset_queue.process_count <= 10:
                        redis_init.rpush(f"asset_queue_list:{mod_value}", SevenHelper.json_dumps(asset_queue))
                    else:
                        logger_error.error(f"资产队列{mod_value}异常,json串:{SevenHelper.json_dumps(asset_queue)},ex:{traceback.format_exc()}")
                    continue
            except Exception as ex:
