...
class ReadEmail(object):
    ...
    def get_latest_n_email(self, n=1, subject="", from_addr="", to_addr="",before="",after=""):
        _, mails, _ = self.__pop.list()
        index = len(mails)
        results = []
        count = 0
        for i in range(index, 0, -1):
            ...
            if "+0000" in email_date:
                offset=8
            elif "+0800" in email_date:
                offset=0
            elif "-0000" in email_date:
                offset=8
            elif "-0800" in email_date:
                offset=16
            else:
                offset=0
            if "+0000" in email_date or "+0800" in email_date:
                email_date=email_date.split("+")[0].strip()
            if "-0000" in email_date or "-0800" in email_date:
                email_date = email_date.split("-")[0].strip()
            if before:
                before_time=time.mktime(time.strptime(before, "%Y-%m-%d %H:%M:%S"))
            else:
                before_time=time.time()
            if after:
                after_time=time.mktime(time.strptime(after, "%Y-%m-%d %H:%M:%S"))
            else:
                after_time=0

            email_time = time.mktime(time.strptime(email_date,"%a, %d %b %Y %H:%M:%S"))
            local_time=datetime.datetime.fromtimestamp(email_time)+datetime.timedelta(hours=offset)
            email_time_float=time.mktime(local_time.timetuple())
            if email_time_float>before_time:
                continue
            if email_time_float<after_time:
                break

            obj.date = local_time.strftime("%Y-%m-%d %H:%M:%S")
...
class Email(object):
    ...
    def get_latest_n_email(self, n=1, subject="", from_addr="", to_addr="",before="",after=""):
        if not self.__read_email:
            self.__init_read_email()
        self.__read_email.login(self.__username, self.__auth_code)
        return self.__read_email.get_latest_n_email(n=n, subject=subject, from_addr=from_addr, to_addr=to_addr,before=before,after=after)

    def get_latest_email(self, subject="", from_addr="", to_addr="",before="",after=""):
        """
        功能：返回最新的邮件
        @param subject: 邮件主题过滤条件，可选，不填时默认不进行过滤，支持子串或者正则匹配
        @param from_addr: 发件人邮箱
        @param to_addr: 收件人邮箱
        @param before: 指明具体时间之前，不填时默认为当前时间之前，格式为：yyyy-mm-dd HH:MM:SS
        @param after: 指明具体时间之后，不填时默认为查找所有邮箱，格式为：yyyy-mm-dd HH:MM:SS
        @return:
        """
        if not self.__read_email:
            self.__init_read_email()
        self.__read_email.login(self.__username, self.__auth_code)
        objs = self.__read_email.get_latest_n_email(n=1, subject=subject, from_addr=from_addr, to_addr=to_addr,before=before,after=after)
        if objs:
            return objs[0]
