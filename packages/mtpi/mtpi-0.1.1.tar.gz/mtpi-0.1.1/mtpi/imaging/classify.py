...
def find_category(path, category):
    ...
    for folderName, subfolders, filenames in os.walk(path):
        for filename in filenames:
            ...
            if reImage.search(filePath) is None:
                continue
            predicted = predict_category(filePath)[0]
...
def find_best_fit(hits, category):
    """Given a list of files predicted to be in a given category, find the file with the highest probability
...
def find_best_fit_by_folder(path, category):
    hits = find_category(path, category)
