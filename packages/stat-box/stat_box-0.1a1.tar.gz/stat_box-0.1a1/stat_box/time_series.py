    ...
    def set_diff(
        ...
        if method == "sequential":
            for i in range(1, len(tts)):
                result[tts.index[i]] = (tts.iloc[i] - tts.iloc[i - 1]).value
                if percent:
                    result[tts.index[i]] = (
                            result[tts.index[i]] / tts.iloc[i - 1] * 100
                    ).value
        elif method == "end":
            for i in range(len(tts) - 1):
                result[tts.index[i]] = (tts.iloc[-1] - tts.iloc[i]).value
                if percent:
                    result[tts.index[i]] = (
                            result[tts.index[i]] / tts.iloc[-1] * 100
                    ).value
        else:
            raise ValueError(f"Not valid method ({method})")
        self.diff = pd.Series(data=result.values(), index=result.keys())
        return self.diff

    def set_linear_trend(self, target="index"):
        tts = self._get_tts(target)
        clear_tts = tts.dropna()
        y = clear_tts.values.reshape(1, -1)[0]
        x_is_time = isinstance(clear_tts.index, DatetimeIndex)
        try:
            x = (
                [t.timestamp() for t in clear_tts.index]
                if x_is_time
                else clear_tts.index
            )
            self.lr_function = np.poly1d(np.polyfit(x, y, 1))
        except:
            x = range(len(clear_tts))
            self.lr_function = np.poly1d(np.polyfit(x, y, 1))

        linear_trend = self.lr_function(x)
        self.linear_trend = pd.Series(
            {
                datetime.fromtimestamp(x[i]) if x_is_time else x[i]: linear_trend[i]
                for i in range(len(linear_trend))
            }
        )
