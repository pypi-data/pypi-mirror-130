...
def run_main_function(worker_modules = []):
    ...
    log.info("Modules: "+','.join(str(e) for e in worker_modules))
    log.info("Registry: "+os.getenv("REGISTRY"))
    log.info("Name: "+os.getenv("WORKERNAME"))
...
if __name__ == '__main__':
    ...
    if os.getenv("MODULES") is not None:
        worker_modules = os.getenv("MODULES").split(",")
