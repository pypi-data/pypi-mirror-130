...
import sys
import time
from typing import cast, List, Callable, TypeVar, Type, Dict, Any, Set, Tuple, Iterator, Optional
...
class DependencyInjector:
    ...
    def bind(self, provider: AbstractInstanceProvider):
        ...
        for provided_interface in provider.get_provided_interfaces():
            dependencies = self._providers.get(provided_interface)
            if dependencies is None:
                dependencies = []
                self._providers[provided_interface] = dependencies
    ...
    def discover(
        self,
        module,
        blacklisted_interfaces: Set[Type] = None,
        blacklisted_modules: Set[Any] = None,
        _depth: int = 0,
    ):
        """
        ...
        if blacklisted_interfaces is None:
            blacklisted_interfaces = set()
        if blacklisted_modules is None:
            blacklisted_modules = set()
