...
class WindowMean(BaseWindowFunction):

    def __init__(self, window, expression):
        super().__init__(
            'window mean',
            window,
            lambda x, y: f"AVG({y.get_sql()}) {x.get_sql()}",
            lambda x, y: x == SqlValType.Window,
...
class WindowCount(BaseWindowFunction):

    def __init__(self, window, expression):
        super().__init__(
            'window count',
            window,
            lambda x, y: f"COUNT({y.get_sql()}) {x.get_sql()}",
            lambda x, y: x == SqlValType.Window,
...
class WindowMax(BaseWindowFunction):

    def __init__(self, window, expression):
        super().__init__(
            'window max',
            window,
            lambda x, y: f"MAX({y.get_sql()}) {x.get_sql()}",
            lambda x, y: x == SqlValType.Window,
            lambda x, y: y,
            expression
        )
...
class WindowMin(BaseWindowFunction):

    def __init__(self, window, expression):
        super().__init__(
            'window min',
            window,
            lambda x, y: f"MIN({y.get_sql()}) {x.get_sql()}",
            lambda x, y: x == SqlValType.Window,
            lambda x, y: y,
            expression
        )
...
class WindowSum(BaseWindowFunction):

    def __init__(self, window, expression):
        super().__init__(
            'window sum',
            window,
            lambda x, y: f"TOTAL({y.get_sql()}) {x.get_sql()}",
            lambda x, y: x == SqlValType.Window,
            lambda x, y: y,
            expression
        )
