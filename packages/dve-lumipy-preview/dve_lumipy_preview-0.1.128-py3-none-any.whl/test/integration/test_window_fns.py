...
class TestWindowFunction(unittest.TestCase):
    ...
    def test_cumulative_fns_match_pandas(self):
        ...
        qry = self.ar.select(
            ...
            CumeSum=self.ar.duration.cume_sum(),
            CumeMean=self.ar.duration.cume_mean(),
            CumeMin=self.ar.duration.cume_min(),
            CumeMax=self.ar.duration.cume_max(),
        ).where(
        ...
        df['PdCumeMean'] = df.Duration.expanding().mean()
        ...
        self.assertSequenceEqual(df.PdCumeMean.round(9).tolist(), df.CumeMean.round(9).tolist())
    ...
    def test_drawdown_matches_pandas(self):
        ...
        qry = self.ar.select(
            self.ar.timestamp, self.ar.duration,
            Drawdown=win.drawdown(self.ar.duration)
        ).where(
            (self.ar.application == 'lusid')
            & (self.ar.method == 'UpsertInstruments')
            & (self.ar.event_type == 'Completed')
        ...
        df['PdDrawdown'] = (df.Duration - df.Duration.cummax()) / df.Duration.cummax()
        df = df.iloc[1:]
        self.assertSequenceEqual(
    ...
    def test_cume_dist_matches_pandas(self):
        ...
        qry = self.ar.select(
            self.ar.timestamp, self.ar.duration,
            CumeDist=self.ar.duration.cume_dist()
        ).where(
            (self.ar.application == 'lusid')
            & (self.ar.method == 'UpsertInstruments')
            & (self.ar.event_type == 'Completed')
