...
async def create_network(
    ...
    purge: bool = Query(
        False,
        description="If delete_existing, permanently delete elements (history is lost)",
    ),
):
...
@networks.post(
    "/{network_uid}/links/search", response_model=List[Union[(*all_links, TopoLink)]]
)
async def get_links(
    network_uid: str,
    ...
    include_subgroups: bool = Query(
        False, description="Recursively include groups that are subgroups of group_uids"
    ),
    datetime: Optional[datetime] = Query(
        None, description="Matches elements created and not deleted by this datetime"
    ),
    skip: int = Query(None),
...
@networks.post(
    "/{network_uid}/nodes/search", response_model=List[Union[(*all_nodes, TopoNode)]]
)
async def get_nodes(
    network_uid: str,
    uids: Optional[List[str]] = Body(None),
...
@networks.post(
    "/{network_uid}/groups/search", response_model=List[Union[(*all_groups, TopoGroup)]]
)
...
@schemas.get("/{domain}/{collection}/{class_name}", response_model=Dict)
