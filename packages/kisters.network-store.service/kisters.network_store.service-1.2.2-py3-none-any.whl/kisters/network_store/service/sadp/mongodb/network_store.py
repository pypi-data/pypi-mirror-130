...
logger = logging.getLogger("network-store")
...
class NetworkStore:
    _network_database_prefix = "network_store_"

    def __init__(self, appname: str = "network-store", event_loop=None):
        kwargs = {
            "appname": appname,
            "tz_aware": True,
            "replicaset": settings.mongodb_replica_set_name,
        }
        if event_loop:
            kwargs["io_loop"] = event_loop
        self._client = AsyncIOMotorClient(settings.mongodb_host, **kwargs)
