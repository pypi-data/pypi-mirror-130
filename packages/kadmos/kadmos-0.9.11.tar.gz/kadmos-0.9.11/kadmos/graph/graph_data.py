...
class DataGraph(KadmosGraph):
    ...
    def _load_cmdows_problem_def(self, cmdows):
        ...
        if cmdows_problem_formulation is not None:
            ...
            if cmdows_problem_formulation.find('executableBlocksGrouping') is not None:
                ...
                cmdows_executable_blocks_grouping = \
                    cmdows_problem_formulation.find('executableBlocksGrouping').findall('group')
    ...
    def connect_variable(self, vnode, input_of=(), output_of=()):
        ...
        for inp in input_of:
            self.add_edge(vnode, inp)
        for outp in output_of:
            self.add_edge(outp, vnode)

    def connect_function(self, fnode, input_vars=(), output_vars=()):
        """Connect a function's inputs and/or outputs"""
        for inp in input_vars:
            self.add_edge(inp, fnode)
        for outp in output_vars:
            self.add_edge(fnode, outp)
    ...
    def _get_sequencing_method(n_nodes):
        ...
        :param n_nodes: number of nodes that need to be sequenced
        :type n_nodes: int
        """
    ...
    def create_mathematical_problem(self, n_disciplines, coupling_density=None, n_clusters=1, cluster_strength=1,
        ...
        if 'B' not in kwargs:
            while True:
                ...
                cluster_division = n_disciplines / float(n_clusters)
                clusters = [disciplines[int(round(cluster_division * i)):int(round(cluster_division * (i + 1)))] for i
                            in range(n_clusters)]
        ...
        I_matrix = np.array([[float(random.randint(-5, 5)) for _ in range(sum(n_local_var))] for _ in
                             range(n_global_constraints)]) if 'I' not in kwargs else kwargs['I']
        mathematical_problem['I-matrix'] = I_matrix
        ...
        for constraint in range(n_global_constraints):
            ...
            for discipline in range(n_disciplines):
                for local_var in range(n_local_var[discipline]):
                    ...
                    if I_matrix[constraint][sum(n_local_var[:discipline]) + local_var] != 0:
                        constraint_eq += '+{0}*x{1}_{2}'.format(I_matrix[constraint][sum(n_local_var[:discipline]) +
                                                                                     local_var],
                                                                discipline + 1,
                                                                local_var + 1)
            for discipline in range(n_disciplines):
...
class FundamentalProblemGraph(DataGraph):
                    ...
                    if category_check:
                        ...
                        if doe_method in get_list_entries(self.OPTIONS_DOE_METHODS, 5):  # Box-Behnken
                            ...
                            if category_check:
                                ...
                                category_check, i = check(test,
                                                          'Invalid DOE center_runs (%s) specified in the '
                                                          'doe_settings.' % self.graph['problem_formulation'][
                                                              'doe_settings']['center_runs'],
                                                          status=category_check,
                                                          category='B',
                                                          i=i)
    ...
    def get_partition_info(self, partitions, coupling_dict=None, use_runtime_info=False, local_convergers=False):
        ...
        for partition, nodes in enumerate(partitions):
            ...
            partition_connections.append(int(partition_feedback))
        total_connections = int(system_connections + sum(partition_connections))
    ...
    def evaluate_partition_range(self, partition_range='pareto', node_selection=None, use_runtime_info=False,
        ...
        for idx, n_partitions in enumerate(partition_range):
            ...
            partition_info.append([idx, int(n_parts), partition_variables, int(system_variables), int(total_var),
                                   max(runtime)])
    ...
    def determine_scope_design_variables(self, des_vars=None, coupled_functions_groups=None,
                                         pre_coupling_functions=None, post_coupling_functions=None):
        """
        ...
        :param post_coupling_functions: list with list of post-coupled function groups
    ...
    def determine_scope_constraint_functions(self, cnstrnt_vars=None, coupled_functions_groups=None,
        ...
        :type post_coupling_functions: list
        :param pre_coupling_functions: (optional) list with pre-coupling functions
        :type pre_coupling_functions: list
        ...
        for cnstrnt_func in cnstrnt_funcs:
            ...
            if len(linked_groups) == 0:
                ...
                for pre_coupling_function in pre_coupling_functions:
                    if subgraph.has_node(cnstrnt_func) and nx.has_path(subgraph, pre_coupling_function, cnstrnt_func) \
                            and pre_coupling_function != cnstrnt_func:
    ...
    def check_and_get_pre_coupling_functions(self, pre_coupling_functions=None):
        ...
        for pre_coupling_function in pre_coupling_functions:
            ...
            assert self.nodes[pre_coupling_function]['problem_role'] in \
                   [self.FUNCTION_ROLES[3], self.FUNCTION_ROLES[4]], 'Pre-coupled function {} lacks the problem ' \
    ...
    def _isolate_surrogate_models(self, sa):
        ...
        for g in range(n_groups):
            ...
            for g_des_var in group_des_vars:
                ...
                des_var_targets = [t for t in des_var_targets if self.nodes[t]['problem_role'] not in
                                   [self.FUNCTION_ROLES[1], self.FUNCTION_ROLES[2]]]
                des_vars_funs.update(des_var_targets)
            subgraph = self.get_subgraph_by_function_nodes(sa['functions_dicts'][0][self.FUNCTION_ROLES[4]] +
                                                           sa['functions_lists'][1][g])
            ...
            group_postdesvar_functions = [f for f in sa['functions_dicts'][0][self.FUNCTION_ROLES[4]] if f in
                                          group_uncoupled_functions]
            sa['functions_dicts'][1][g][self.FUNCTION_ROLES[4]] = group_postdesvar_functions + \
                sa['functions_dicts'][1][g][self.FUNCTION_ROLES[4]]
        ...
        if sa['functions_dicts'][0][self.FUNCTION_ROLES[4]]:
            ...
            subgraph2 = subgraph1.merge_functions(sa['functions_dicts'][0][self.FUNCTION_ROLES[4]],
                                                  new_label='__merged_node__')
            subgraph3 = subgraph2.merge_functions(sa['functions_dicts'][0][self.FUNCTION_ROLES[2]],
                                                  new_label='__merged_node2__')
            additional_couplings = \
                set([n[2] for n in subgraph3.get_direct_coupling_nodes(['__merged_node__', '__merged_node2__'])])
        else:
        ...
        for add_coup in additional_couplings:
            ...
            if not common_groups:
                # todo: Implement by simply keeping at system level?
                ...
                group_des_vars = [des_var for des_var, groups in sa['des_vars']['groups'].items() if
                                  add_coup_group in groups]
                des_vars_funs = set()
                for g_des_var in group_des_vars:
                    des_var_targets = self.get_targets(g_des_var)
                ...
                if len(found_groups) > 1:
                    raise NotImplementedError('For multiple common groups the function has to be used in one of them '
                                              'at the moment.')
        ...
        if mdao_arch == graph.OPTIONS_ARCHITECTURES[0]:  # unconverged-MDA
            ...
            for idx, subsys_functions_dict in enumerate(fun_dict_iter):
                ...
                group_wcf_node, group_wcf_obj_node, weight_nodes = \
                    mdg.connect_weighted_couplings_objective_function(idx, [f for f in local_group_couplings_group if f
                                                                            in sa['couplings']['basic']])
                ...
                if cycles:
                    ...
                    mdg.connect_converger(sub_convs[idx], 'Gauss-Seidel', best_order, False,
                                          label=sub_convs_labels[idx])
                else:
            ...
            ccf_node, cc_nodes = \
                mdg.connect_consistency_constraint_function(ccf_mapping,
...
class MdaoDataGraph(DataGraph, MdaoMixin):
            ...
            for cmdows_single_architecture_exe_block in list(cmdows_architecture_exe_block):
                ...
                if cmdows_uid is not None:
                        ...
                        for setting_option in setting_options:
                            if setting_option == 'apply_scaling':
                                ...
                                if role == 'optimizer':
                                    ...
                                    des_var_facs = \
                                        cmdows_single_architecture_exe_block.find('settings/designVariableFactors')
                    ...
                    if role == 'optimizer':
                        ...
                        self.nodes[cmdows_uid]['training'] = OrderedDict((('inputs',
                                                                           [tr_inp.text for tr_inp in training_inputs]),
                                                                          ('outputs',
                                                                           [tr_out.text
                                                                            for tr_out in training_outputs]),
                                                                          ('settings',
                                                                           OrderedDict((('fitting_method',
                                                                                         training_fitting_method),)))))
                        prediction_inputs = \
                            cmdows_single_architecture_exe_block.findall('prediction/inputs/input/parameterUID')
                        prediction_outputs = \
                            cmdows_single_architecture_exe_block.findall('prediction/outputs/output/parameterUID')
    ...
    def copy_node_as(self, node, architecture_role, add_instance_if_exists=False, ignore_duplicates=True):
        ...
        if not self.has_node(new_node):
            node_data_dict['label'] = label_prefix + get_correctly_extended_latex_label(node_data_dict['label'],
                                                                                        label_suffix)
    ...
    def connect_weighted_couplings_objective_function(self, group_idx, couplings):
        ...
        for idx, var1 in enumerate(couplings):
            ...
            root = xpath_var1[1]
            var2 = '/{}/distributedArchitectures/group{}/couplingWeights/w{}_{}'.format(root, group_idx, idx,
                                                                                        var_ref_label)
            self.add_node(var2,
                          category='variable',
    ...
    def connect_consistency_constraint_function(self, ccv_mappings, architecture_role=None):
        ...
        self.add_fnode(new_function_node,
                       ...
                       problem_role=self.FUNCTION_ROLES[2],  # post-coupling
                       architecture_role=self.ARCHITECTURE_ROLES_FUNS[8] if architecture_role is None else
                       architecture_role,
                       function_type='consistency')
        # Connect the variable inputs for the function
        new_con_nodes = []
        ...
        for external_coupling in external_couplings:
            ...
            if local_targets:
                ...
                external_coupling_copy = self.copy_node_as(related_node,
                                                           # copy design variable or coupling copy variable
                                                           self.ARCHITECTURE_ROLES_VARS[9 if make_copies_des_vars
                                                                                        else 2],
        ...
        for local_coupling in local_couplings:
            ...
            if external_targets:
                local_coupling_copy = self.copy_node_as(local_coupling,
                                                        # copy design variable or coupling copy variable
                                                        self.ARCHITECTURE_ROLES_VARS[9 if make_copies_des_vars else 2],
                                                        add_instance_if_exists=False)
                ...
                local_coupling_copy = '/{}/architectureNodes/{}s/{}Copy/{}'\
                    .format(root, make_camel_case(self.ARCHITECTURE_ROLES_VARS[9 if make_copies_des_vars else 2]),
                            root, '/'.join(xpath_nodes[2:]))
    ...
    def connect_distributed_system_converger(self, converger, inputs, outputs, label='SYS-CONV'):
        ...
        for inp in inputs:
            self.add_edge(inp, converger)

        for outp in outputs:
            self.add_edge(converger, outp)
        ...
        for node in consconcs_nodes:
            ...
            if rel_node in self.nodes:
                if 'upper_bound' in self.nodes[rel_node]:
                    self.nodes[optimizer]['design_variables'][rel_node]['upper_bound'] = \
                        self.nodes[rel_node]['upper_bound']
                else:
                    self.nodes[optimizer]['design_variables'][rel_node]['upper_bound'] = None
                if 'lower_bound' in self.nodes[rel_node]:
                    self.nodes[optimizer]['design_variables'][rel_node]['lower_bound'] = \
                        self.nodes[rel_node]['lower_bound']
                else:
    ...
    def get_initial_guess_node(self, coupling):
        ...
        if not ini_guess_node:
            # If there is one source, the initial guess node needs to be created will be an input node. If there are
            # more sources, the node with the lowest instance will be the initial guess
            ini_guess_node = self.copy_node_as(coupling[2], self.ARCHITECTURE_ROLES_VARS[0]) if len(all_sources) == 1 \
                else sorted(all_sources, key=lambda x: self.nodes[x]['instance'])[0]
    ...
    def get_mpg(self, name='MPG'):
        ...
        if mdao_arch == mdg.OPTIONS_ARCHITECTURES[0]:  # unconverged-MDA
            ...
            if sys_conv:
                if partitions:
                    mpg.add_process_partitions([sys_conv], partitions, [], mpg.nodes[sequence1[-1]]['process_step'],
            ...
            for idx, subgroup in enumerate(distr_function_ordering[1]):
                if not mpg.has_node(sub_convs[idx]):
                    sequence2 = sequence1 + [sub_does[idx]] + subgroup[self.FUNCTION_ROLES[3]] + \
                                [sub_opts[idx]] + subgroup[self.FUNCTION_ROLES[4]] + \
                                subgroup[self.FUNCTION_ROLES[1]] + subgroup[self.FUNCTION_ROLES[2]]
