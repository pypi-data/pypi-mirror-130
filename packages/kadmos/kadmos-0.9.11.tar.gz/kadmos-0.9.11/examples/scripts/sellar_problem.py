...
mdao_definitions = ['unconverged-MDA',      # 0
                    'converged-MDA-GS',     # 1
                    'converged-MDA-J',      # 2
                    'unconverged-OPT',      # 3
                    'MDF-GS',               # 4
...
def run_kadmos(analyze_mdao_definitions, repository_type, folder_str='sellar_problem'):
    ...
    vistoms_dir = os.path.join('{}_{}'.format(folder_str, repository_type), 'VISTOMS')
    ...
    for mdao_definition in mdao_defs_loop:
        ...
        # Get function order
        function_order = fpg.get_possible_function_order()
