...
class Pruner:
    ...
    def build_pruned_network(self, network, site_limit, size_dict = None, kinase_size = 2500):
        """
        Builds a heuristic pruned network where each kinase has a specified number of connected sites and
        each site has an upper limit to the number of kinases it can connect to.  Allows for each kinase to
        have a different kinase_size via size_dict parameter
        ...
        if size_dict is None:
            ...
            kinase_list = self.kinases.copy()
            
            #create size dictionary that includes all kinases, get maximum kinase size
            for kinase in self.kinases:
                if kinase not in size_dict.keys():
                    size_dict[kinase] = kinase_size
            max_kinase_size = max(size_dict.values())       
            
            for i in range(max_kinase_size):
                random.shuffle(kinase_list)
                for kinase in kinase_list:
                    ...
                    pruned_network.at[sample.name, kinase] = 1
                    site_sizes[sample.name] = site_sizes[sample.name] + 1
                    # check to see if site limit has been reached
                    if site_sizes[sample.name] >= site_limit:
                        network.at[sample.name, :] = np.nan
                    # check to see if kinase size limit has been reached
                    if pruned_network[kinase].sum() >= size_dict[kinase]:
                        kinase_list.remove(kinase)
    ...
    def compendia_pruned_network(self, compendia_sizes, site_limit, odir):
        ...
        for comp_size, kinase_size in compendia_sizes.items():
            ...
            if isinstance(kinase_size, int):
                pruned_network.append(self.build_pruned_network(network, site_limit, kinase_size = kinase_size))
            else:
                pruned_network.append(self.build_pruned_network(network, site_limit, size_dict = kinase_size))
    ...
    def calculate_compendia_sizes(self, kinase_size, size_dict = None):
        """
        ...
        Parameters
        ----------
        kinase_size: int or dict
            number of sites each kinase should connect to for all kinases, if single number, else a dictionary that indicates the
            number of sites each kinase should connect to (individually)
        ...
        if size_dict is None:
            ...
            for kinase in self.kinases:
                if kinase not in size_dict.keys():
                    size_dict[kinase] = kinase_size
            sizes = {}
            for kin in size_dict.keys():
                sizes[kin] = [-np.inf]
                multiplier = size_dict[kin]
                while sum(sizes[kin]) < size_dict[kin]:
                    sizes[kin] = self.compendia.groupby('KSTAR_NUM_COMPENDIA').size() / len(self.compendia) * multiplier
                    sizes[kin] = sizes[kin].astype(int)
                    multiplier = multiplier + 1
            sizes[kin] = sizes[kin].to_dict()
            
            tmp = {}
            for comp_size in np.sort(self.compendia['KSTAR_NUM_COMPENDIA'].unique()):
                tmp[comp_size] = {}
                for kinase in sizes.keys():
                    tmp[comp_size][kinase] = sizes[kinase][comp_size]
                    
            sizes = tmp
        return sizes

    def build_multiple_compendia_networks(self, kinase_size, site_limit, num_networks, network_id, odir, size_dict = None, PROCESSES = 1):
        """
        ...
        if size_dict is None: 
            ...
            compendia_sizes = self.calculate_compendia_sizes(kinase_size, size_dict)
...
def parse_args():
    ...
    parser.add_argument('--size_dict', action='store', dest='size_dict',type = dict, help='indicates network sizes for specific kinases, will override kinase_size', default=None)
    parser.add_argument('--PROCESSES',action='store', dest='PROCESSES', type = int, help='number of processes to run in parallel', default=1)
...
def run_pruning(network, log, use_compendia, phospho_type, kinase_size, site_limit, num_networks, network_id, odir,site_dict = None, PROCESSES = 1):
    """
    ...
    if use_compendia:
        ...
        pruner.build_multiple_compendia_networks(kinase_size, site_limit, num_networks, network_id, odir, site_dict, PROCESSES = PROCESSES)
    # else:
...
def main():
    results = parse_args()
    network, log, use_compendia, size_dict = process_args(results)
    ...
    odir = results.odir
    site_dict = results.site_dict
    PROCESSES = results.PROCESSES
    log.info("Beginning to build pruning networks")
    pruner = run_pruning(network, log, use_compendia, phospho_type, kinase_size, site_limit, num_networks, network_id, odir, site_dict, PROCESSES)
