...
from nexinfosys.bin.cli_script import set_log_level_from_cli_param, prepare_base_state
...
from enbios.common.helper import print_issues, list_to_dataframe, generate_workbook
...
def list_lcia_indicators(nis_file, output_file):
        ...
        if lcia_methods:
            ...
            for t in different:
                lst.append(["LCIA",
                            ...
                            "",  # UnitLabel
                            "",  # Benchmarks
                            f"Method {t[0]}, indicator {t[1]}, horizon {t[2]}, compartment, {t[3]}, subcompartment {t[4]}, category {t[5]}",
                            "Generated by enbios 'lcia_list_indicators', using a base NIS file where the LCIA methods are defined."
                            ])
    ...
    print_issues(issues, f"Please check the issues")
    if state:
        if state:
...
class Enbios:
    ...
    def list_lcia_indicators(nis_file: str, output_file: str, log: str = None):
        ...
        :param nis_file: Full path of the NIS file
