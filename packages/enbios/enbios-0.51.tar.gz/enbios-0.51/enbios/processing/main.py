...
from nexinfosys.bin.cli_script import get_valid_name, get_file_url, prepare_base_state
...
class Enviro:
    ...
    def compute_indicators_from_base_and_simulation(self,
        ...
        if not state:
            print("Base not ready due to errors, exiting")
            sys.exit(1)
        ...
        if n_fragments > 0:
            ...
            logging.debug(f"Only {n_fragments} the first fragment{'s' if n_fragments > 1 else ''} will be processed")
