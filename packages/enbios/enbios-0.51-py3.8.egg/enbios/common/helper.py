...
def print_issues(issue_lst: List[Issue], msg_in_case_of_error: str = None):
    w = len(str(len(issue_lst)))
    for i, issue in enumerate(issue_lst):
        print(f"{i:{w}}: {issue.itype.name:7}: {issue.description}")
    ...
    if (warnings or errors) and msg_in_case_of_error:
