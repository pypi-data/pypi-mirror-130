...
def _valid_attachment_parameter(name):
    """Check attachment attribut validity."""
    if name in ('type', 'label'):
        return True
    if name.startswith('_'):
        return True
    return re.match(r'^[a-z][a-z0-9.]+[a-z0-9]$', name) and '.' in name


def process_output(stdout, reader, writer):
    """Process output, filling structures.

    # Required parameters

    - stdout: a list of strings
    - reader: a function copying a remote file to a local path
    - writer: a function copying a local file to a remote path
    ...
    for line in stdout:
        ...
        if wcmd := re.match(ATTACH_COMMAND, line):
            ...
            try:
                ...
                reader(remote, local_path)
                attachments.append(local_path)
                attachments_metadata[local_path] = {'uuid': local_uuid}
                if params:
                    for param in params.split():
                        k, _, v = param.partition('=')
                        if not _valid_attachment_parameter(k):
                            ...
                        attachments_metadata[local_path][k] = v
            except Exception as err:
                core.error(f'Could not read {remote}: {err}.')
                resp = 2
