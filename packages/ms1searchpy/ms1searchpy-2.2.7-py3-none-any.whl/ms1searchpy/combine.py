...
def run():
    ...
    parser.add_argument('-prots_order', help='prots_order', default='')
    ...
    for idx, filen in enumerate(args['file']):
        ...
        if df1 is False:
            df1 = df3
            if args['prots_order']:
                df2 = pd.read_csv(args['prots_order'], sep='\t')
                prots_spc_basic2 = df2.set_index('dbname')['score'].to_dict()
            else:
    ...
    final_iteration(resdict, mass_diff, rt_diff, pept_prot, protsN, base_out_name, prefix, isdecoy, isdecoy_key, escore, fdr, args['nproc'], fname=False, prots_spc_basic2=prots_spc_basic2)
