...
def run():
    ...
    print(df_final[df_final['protein'] == 'sp|P45578|LUXS_ECOLI'].values)
    ...
    # for custom_min_samples in range(1, len(all_lbls)+1, 1):
    # for custom_min_samples in [int(args['min_samples']), ]:

    df_final = df_final_copy.copy()

    max_missing = len(all_lbls) - custom_min_samples

    print('max_missing', max_missing)
    ...
    print(df_final[df_final['proteins'] == 'sp|P45578|LUXS_ECOLI'].values)
    ...
    # df_final.loc[(pd.isna(df_final['S2_mean'])) & (~pd.isna(df_final['S1_mean'])), 'p-value'] = 0
    # df_final.loc[(~pd.isna(df_final['S2_mean'])) & (pd.isna(df_final['S1_mean'])), 'p-value'] = 0
    ...
    for p_val_threshold, FC_threshold in itertools.product(t_p, t_fc):
        ...
        # df_final['up'] = df_final.apply(lambda x: np.abs(x['up'] - x['down']), axis=1)
        ...
        # FC_down_dict = df_final[df_final['down']>0].groupby('proteins')['FC'].median().to_dict()
    ...
    df_out['BH_threshold'] = -np.log10(df_out['score'].rank(ascending=False) * 0.05 / len(df_out))
