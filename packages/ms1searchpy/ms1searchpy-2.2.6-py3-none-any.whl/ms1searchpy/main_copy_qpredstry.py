...
def process_peptides(args):
    ...
    df1['qpreds'] = pd.qcut(df1['preds'], 100, labels=range(100))
    ...
    # df1un = df1u
    df1un['qpreds'] = pd.qcut(df1un['preds'], 100, labels=range(100))

    qval_dict = {}
    for qval_cur in range(100):
        df1unt = df1un[df1un['qpreds'] == qval_cur]
        decoy_ratio = df1unt['decoy'].sum() / (len(df1unt) - df1unt['decoy'].sum())
        print('!@#', qval_cur, decoy_ratio)
        qval_dict[qval_cur] = decoy_ratio

    qdict = df1un.set_index('seqs').to_dict()['qpreds']

    df1['qpreds'] = df1['seqs'].apply(lambda x: qdict.get(x, 101))
    df1['qz'] = df1['seqs'].apply(lambda x: qval_dict.get(qdict.get(x, 101), 1.0))
    ...
    resdict['qz'] = df1['qz'].values


    e_ind = resdict['qpreds'] <= 100
    resdict = filter_results(resdict, e_ind)
...
def worker(qin, qout, mass_diff, rt_diff, resdict, protsN, pept_prot, isdecoy_key, isdecoy, fdr, prots_spc_basic2, win_sys=False):
    ...
    for item in (iter(qin.get, None) if not win_sys else qin):
        ...
        qval_dict = {}
        for pepseq, qzed in zip(resdict2['seqs'], resdict2['qz']):
            qval_dict[pepseq] = min(qval_dict.get(pepseq, 1.0), qzed)
            ...
            while 1:
                if not prots_spc2:
                    ...
                    for k, v in prots_spc2.items():
                        if len(v):
                            prots_spc3[k] = np.mean([qval_dict[vv] for vv in v])
                            if isdecoy_key(k):
                                for vv in v:
                                    top100decoy_pep.append(qval_dict[vv])
                        else:
                            prots_spc3[k] = 1.0
                    q0 = np.mean(top100decoy_pep)
                    print('pep_mean=%f' % (q0, ))


                    # p_array = np.array([p * prots_spc3[k] / pep_mean for k in names_arr])
                    # print(p_array)

                    # all_pvals = calc_sf_all(v_arr, n_arr, p_array)
                    # print('MRK', len(aux.filter(prots_spc.items(), fdr=0.01, key=escore, is_decoy=isdecoy, remove_decoy=True, formula=1, full_output=True, correction=0)))
                ...
                for k in unstable_prots:
                    n_arr_small.append(protsN[k])
                    names_arr_small.append(k)
                    v_arr_small.append(prots_spc[k])
                    p_arr_small.append(p0 * prots_spc3.get(k, 1.0) / q0)


                p_arr_small = np.clip(np.array(p_arr_small), 1e-3, 0.999)
                ...
                # prots_spc_basic = dict()
                # all_pvals = calc_sf_all(np.array(v_arr_small), n_arr_small, p0, prev_best_score)
                all_pvals = calc_sf_all(np.array(v_arr_small), n_arr_small, p_arr_small, prev_best_score)
                for idx, k in enumerate(names_arr_small):
                    prots_spc_basic[k] = all_pvals[idx]
                ...
                if best_prot not in prots_spc_final:
                    ...
                    v_arr = np.array([prots_spc[k] for k in names_arr])
                    p_arr = np.clip(np.array([p0 * prots_spc3.get(k, 1.0) / q0 for k in names_arr]), 1e-3, 0.999)
                    all_pvals = calc_sf_all(v_arr, n_arr, p_arr)
                    # all_pvals = calc_sf_all(v_arr, n_arr, p0)
                    for idx, k in enumerate(names_arr):
                        prots_spc_basic[k] = all_pvals[idx]
