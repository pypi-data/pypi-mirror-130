...
from argo_workflow_tools.dsl.dag_task import DAGReference, NodeReference, TaskReference, WorkflowTemplateReference
...
def _build_dag_task(dag_task: NodeReference) -> argo.DagTask:
    ...
    if isinstance(dag_task, DAGReference):
        ...
        task = argo.DagTask(
            name=dag_task.id,
            templateRef=argo.TemplateRef(name=dag_task.workflow_template_name, template=dag_task.name),
            dependencies=list(dependencies),
            arguments=get_arguments(arguments),
            withParam=with_param,
            when=build_condition(dag_task.conditions),
