...
from argo_workflow_tools.dsl.dag_task import DAGReference, TaskReference, WorkflowTemplateReference
...
class Node(object):
    ...
    def _reduce_fan_in_arguments(name: str, arg: Any) -> Any:
        ...
        if not isinstance(arg, str) and isinstance(arg, Sequence) and any([item.is_node_output for item in arg]):
            raise ValueError(
    ...
    def _get_arg(argument_name: str, argument_value: Any) -> InputDefinition:
        ...
        return InputDefinition(source_type=SourceType.CONST, value=argument_value ,name=argument_name)
...
class WorkflowTemplateNode(DAGNode):
    ...
    def __call__(self, *args, **kwargs) -> Any:
        ...
        arguments = self._bind_arguments(*args, **kwargs)
        partitioned_arguments = list(
            filter(lambda argument: isinstance(argument, InputDefinition) and argument.is_partition, arguments.values())
        )
        if len(partitioned_arguments) > 1:
            raise ValueError(
                "Nested loops are not allowed in the same DAG, split your loops into nested DAG's instead"
