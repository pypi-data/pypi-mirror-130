...
AGGREGATIONS = {
    __("checkbox.mean"): Aggregation("mean", {ContinuousVariable, TimeVariable}),
    __("checkbox.median"): Aggregation("median", {ContinuousVariable, TimeVariable}),
    __("checkbox.mode"): Aggregation(
        lambda x: pd.Series.mode(x)[0], {ContinuousVariable, TimeVariable}
    ),
    __("checkbox.standard_deviation"): Aggregation("std", {ContinuousVariable, TimeVariable}),
    __("checkbox.variance"): Aggregation("var", {ContinuousVariable, TimeVariable}),
    __("checkbox.sum"): Aggregation("sum", {ContinuousVariable, TimeVariable}),
    __("checkbox.concatenate"): Aggregation(
        concatenate,
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
    ),
    __("checkbox.min_value"): Aggregation("min", {ContinuousVariable, TimeVariable}),
    __("checkbox.max_value"): Aggregation("max", {ContinuousVariable, TimeVariable}),
    __("checkbox.span"): Aggregation(
        lambda x: pd.Series.max(x) - pd.Series.min(x),
        {ContinuousVariable, TimeVariable},
    ),
    __("checkbox.first_value"): Aggregation(
        "first", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("checkbox.last_value"): Aggregation(
        "last", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("checkbox.random_value"): Aggregation(
        lambda x: x.sample(1, random_state=0),
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
    ),
    __("checkbox.count_defined"): Aggregation(
        "count", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("checkbox.count"): Aggregation(
        "size", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("checkbox.proportion_defined"): Aggregation(
...
TABLE_COLUMN_NAMES = [__("box.attr"), __("box.agg")]
...
class OWGroupBy(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        data = Input("Data", Table, doc=__("input_desc"), label=i18n.t("widget.data.data.common.data"))

    class Outputs:
        data = Output("Data", Table, doc=__("output_desc"), label=i18n.t("widget.data.data.common.data"))
    ...
    def __init_control_area(self) -> None:
        ...
        box = gui.vBox(self.controlArea, __("name"))
    ...
    def __init_main_area(self) -> None:
        ...
        gui.widgetBox(self.mainArea, orientation=grid_layout, box=__("box.agg"))
