...
class OWDistanceFile(widget.OWWidget, RecentPathsWComboMixin):
    ...
    def open_file(self):
        ...
        if distances is not None:
            self.infoa.setText(
                __("info.point").format(len(distances)) + ([__("info.unlabelled"), __("info.labelled")][distances.row_items is not None]))
            self.warnings.setText("")
    ...
    def send_report(self):
        if not self.loaded_file:
            ...
            self.report_items([(__("report.file_name"), self.loaded_file)])
