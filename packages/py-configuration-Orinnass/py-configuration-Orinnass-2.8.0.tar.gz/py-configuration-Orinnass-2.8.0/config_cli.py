...
def __parse_template__(element):
    ...
    if element.get("$ref"):
        path = element["$ref"].split("/")
        link_element = element
        for i in path:
            match i:
                case "#": link_element = global_template
                case _: link_element = link_element[i]
    ...
    for i in walk(params.dir_find):
        for file in i[2]:
            if file == "merge_template.json":
                ...
                if __wait_confirmation__(f"Найден файл для слияние: {merge_file}. Выполняем слияние?"):
