...
class PeerSettings(TLObject):  # type: ignore
    ...
    def read(data: BytesIO, *args: Any) -> "PeerSettings":
        ...
        request_chat_title = String.read(data) if flags & (1 << 9) else None
        request_chat_date = Int.read(data) if flags & (1 << 9) else None
    ...
    def write(self) -> bytes:
        ...
        if self.request_chat_title is not None:
            data.write(String(self.request_chat_title))
        
        if self.request_chat_date is not None:
            data.write(Int(self.request_chat_date))
