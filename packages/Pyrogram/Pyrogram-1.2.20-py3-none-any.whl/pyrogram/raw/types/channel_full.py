...
class ChannelFull(TLObject):  # type: ignore
    ...
    def read(data: BytesIO, *args: Any) -> "ChannelFull":
        ...
        requests_pending = Int.read(data) if flags & (1 << 28) else None
        recent_requesters = TLObject.read(data, Long) if flags & (1 << 28) else []
        
        default_send_as = TLObject.read(data) if flags & (1 << 29) else None
    ...
    def write(self) -> bytes:
        ...
        if self.default_send_as is not None:
            data.write(self.default_send_as.write())
