...
from sfproxy_test import get_snowflake_proxy_test_context, get_snowflake_proxy
    ...
    def test_list_roles(self):
        ...
        with common.get_planner(ctx) as conn:
            ...
            for i in range(total_roles):
                ...
                conn.execute_ddl("CREATE ROLE IF NOT EXISTS %s WITH GROUPS %s" % (role_name, user_name))
                conn.execute_ddl("""
                GRANT IF NOT EXISTS SELECT ON DATABASE %s
                HAVING ATTRIBUTE IN (%s)
                TO ROLE %s
                POLICYPROPERTIES('enabled'='true', 'start_datetime'='543453453', 'end_datetime'='543453459')
                """ % (DB, ATTR, role_name))
                conn.execute_ddl("""
            ...
            PERF.measure(list(t_params, total_roles), "Listing Roles w/role name substring filter",
                         "admin", "ListRoles")

    @unittest.skipIf(not PERF.run_perf_snowflake(), "Skipping snowflake perf tests")
    def test_measure_snowflake_speed_of_light(self):
        sf_ctx = get_snowflake_proxy_test_context()
        ...
        with common.get_planner(planner_ctx) as conn:
            for ddl_tmpl in setup_ddls:
                ddl = ddl_tmpl.format(**setup_ddls_params)
                conn.execute_ddl(ddl)
        ...
        with get_snowflake_proxy(sf_ctx) as conn:
        ...
        with common.get_planner(ctx) as conn:
            def list_db(db=None, pattern=None, tags=None, match_level=None, privilege=None):
                def get():
                    ...
                    if tags is not None:
                        result = conn.list_databases_v2(tags=tags, tag_match_level=match_level)
                        assert len(result.databases) >= 1
                return get
            ...
            PERF.measure(list_db(privilege=priv), "Privilege filter",
                         "admin", "list_databases_v2")
            PERF.measure(list_db(tags=[attr], match_level=TAttributeMatchLevel.DATABASE_PLUS),
