...
class FsTest(unittest.TestCase):
    ...
    # s3://cerebrodata-test/fs_test/file1 --> has attribute abac_uri.attr1
    # s3://cerebrodata-test/fs_test/file2 --> does not have attribute abac_uri.attr1
    def test_abac_uri_grant_respects_enabled_flag(self):
        URI_BASE = 's3://cerebrodata-test/fs_test/'
        USER = 'fstestuser'
        ROLE = 'abac_uri_test_role'
        ATTR = 'abac_uri.attr1'
        ...
        with common.get_planner(ctx) as conn:
            ...
            ddls = [
                ...
                TO ROLE %s POLICYPROPERTIES('enabled'='false')
                """ % (URI_BASE, ATTR, ROLE),
            ]
            ...
            # User should not have access because the grant is disabled
            with self.assertRaisesRegex(_thrift_api.TRecordServiceException,
                                        'does not have access'):
                authorize_uri(conn, USER, TListFilesOp.READ, '%sfile1' % URI_BASE)
            # User should not have access because the file doesn't have the tag
            with self.assertRaisesRegex(_thrift_api.TRecordServiceException,
                                        'does not have access'):
                authorize_uri(conn, USER, TListFilesOp.READ, '%sfile2' % URI_BASE)
            ...
            conn.execute_ddl("""
                ...
                """ % (URI_BASE, ATTR, ROLE))
            # Grant is now enabled so should have access
            authorize_uri(conn, USER, TListFilesOp.READ, '%sfile1' % URI_BASE)

            # File is missing tag so still don't have access
            with self.assertRaisesRegex(_thrift_api.TRecordServiceException,
                                        'does not have access'):
                authorize_uri(conn, USER, TListFilesOp.READ, '%sfile2' % URI_BASE)

    def test_rbac_uri_grant_respects_enabled_flag(self):
        URI_BASE = 's3://cerebrodata-test/nytaxi-data'
        USER = 'fstestuser'
        ROLE = 'rbac_uri_test_role'
        ...
        with common.get_planner(ctx) as conn:
            ...
            with self.assertRaisesRegex(_thrift_api.TRecordServiceException,
                                        'does not have access'):
                authorize_uri(conn, USER, TListFilesOp.READ,
                      '%s/csv/trips_xcp.csv.gz' % URI_BASE)
