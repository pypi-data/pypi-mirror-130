...
class data_treatment:
    ...
    def add_inds(self, data):
        ...
        first_cond = df['close']>df['close'].shift(1)
        second_cond = df['close']<df['close'].shift(1)

        df['obv'] = np.where(first_cond, df['volume'], np.where(second_cond, -df['volume'], 0)).cumsum()
    ...
    def resampled_data_ind(self, tf_list, data):
        
        df = data.copy()
        d_tf = {}
        for x in tf_list:
            d_tf[x] = self.resample_and_ind(x, df)
