...
typedef bool(*in_shared_object_t)(CPUState *cpu, OsiProc *p);
extern in_shared_object_t __in_shared_object;
#ifdef PLUGIN_MAIN
in_shared_object_t __in_shared_object = NULL;
#endif
static inline bool in_shared_object(CPUState *cpu, OsiProc *p);
static inline bool in_shared_object(CPUState *cpu, OsiProc *p) {
    assert(__in_shared_object);
    return __in_shared_object(cpu,p);
}
...
typedef void(*notify_task_change_t)(CPUState *cpu);
extern notify_task_change_t __notify_task_change;
#ifdef PLUGIN_MAIN
notify_task_change_t __notify_task_change = NULL;
#endif
static inline void notify_task_change(CPUState *cpu);
static inline void notify_task_change(CPUState *cpu) {
    assert(__notify_task_change);
    return __notify_task_change(cpu);
}
...
bool init_osi_api(void) {
    ...
    IMPORT_PPP(module, in_shared_object)
    ...
    IMPORT_PPP(module, notify_task_change)
    return true;
}
