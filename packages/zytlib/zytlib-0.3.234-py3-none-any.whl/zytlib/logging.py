...
class Logger:
    ...
    def parser_from_logging_file(f_name):
        ...
        with open(f_name, "r") as finput:
            content = "start"
            for line in finput.readlines():
                if content == "start" and line.startswith("Argument Parser:"):
                    content = "start@1"
                elif content == "start@1":
                    content = "start@2"
                elif content == "start@2":
                    assert line.startswith("> Specified Vars:")
                    content = "specified"
                elif content == "specified":
                    if line.startswith("> Default Vars:"):
                        content == "default"
                    else:
                        assert line.startswith(" " * 4)
                        item, _, value = content.strip().partition("=")
