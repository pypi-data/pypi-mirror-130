...
class Trainer(object):
    def __init__(self, tr_dataloader, model,  optimizer, losses, regulizers = [], is_deep_supervision = False, gradient_checkpoint = True,
            ...
            while not self.tr_dl.is_end():
                ...
                if isinstance(self.optimizer, SAMSGD):
                    ...
                    if self.gradient_checkpoint == True:
                        output = checkpoint_sequential(self.models, 2, images)
                    else:
                        output = self.model(images)
                    self.optimizer.zero_grad()
                    loss = self.calculate_loss(output, seg, writer, current_iter)
