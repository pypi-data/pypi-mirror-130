...
def change_value_from_coords(image, patch_coords, index, patch):
    ...
    image[:, torch.meshgrid(*patch_coords[index])] += torch.softmax(patch, dim = 0)
    return image
    ...
    def get_result(self, image, data_index = None, current_name = None):
        ...
        for mini_batch_patch, mini_batch_index in zip(patch_bundle, index_bundle):
            ...
            for batch_index in range(mini_batch_index.shape[0]):
                print('result shape', result.shape, 'coords x max', coords[mini_batch_index[batch_index]][0].max(), coords[mini_batch_index[batch_index]][1].max(), coords[mini_batch_index[batch_index]][2].max(), 'batch index', batch_index, 'mini batch index', mini_batch_index[batch_index])
