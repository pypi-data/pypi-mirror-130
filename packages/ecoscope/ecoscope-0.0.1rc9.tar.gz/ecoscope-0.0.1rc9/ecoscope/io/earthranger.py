...
class SourceIO(EarthRanger):
    ...
    def download_sources(self, sources=None):
        if sources is not None:
            ...
            sources = self.er_client._get('sources')
...
class ObservationsIO(EarthRanger):
    ...
    def upload_multirelocations(self,
        ...
        df2 = df[~df.source.isnull()]  # select dataframes with no-missing source_id.

        df2['recorded_at'] = df2['recorded_at'].apply(lambda x: x.isoformat())
        df2['location'] = df2.geometry.apply(lambda geom: dict(latitude=float(geom.y), longitude=float(geom.x)))
        post_data = df2.loc[:, [manufacturer_id, 'recorded_at', 'location', 'source']].to_dict('records')
