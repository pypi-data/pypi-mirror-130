...
def subsample_xy(z, g, level, dtype='uint16'):
    ...
    zn = g.empty(str(level), shape=dn.shape, chunks=(1, 1, 1024, 1024), dtype=dtype, overwrite=True)
    dn.to_zarr(zn)
    return zn, dn.shape
...
def main():
    ...
    parser.add_argument("--nplanes", type=int, help="Number of planes to be processed at one time")
