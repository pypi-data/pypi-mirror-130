...
def _handle_deposit_error(message=None, response=None):
    if message is None:
        try:
...
def _handle_revoked_task(deposit, client, token):
...
def kadi_publish_record(record, provider, client, token, task):
    ...
    try:
        ...
        if response.status_code != 201:
            return _handle_deposit_error(response=response)
        ...
        for index, file in enumerate(files_query):
            if task is not None and task.is_revoked:
                return _handle_revoked_task(deposit["links"]["self"], client, token)
        ...
        return _handle_deposit_error(message=repr(e))
