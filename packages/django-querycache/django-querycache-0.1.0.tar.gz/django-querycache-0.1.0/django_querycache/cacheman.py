...
def inputmodel_parse(inputmodel: InputModel) -> Tuple[QuerySet, Model]:
    """
    Parameters
    ----------
    inputmodel
    ...
    if hasattr(inputmodel, "objects"):
        return inputmodel.objects.all(), inputmodel  # type: ignore
    elif hasattr(inputmodel, "model"):
        return inputmodel, inputmodel.model  # type: ignore
    elif isinstance(inputmodel, tuple):
        _m = apps.get_model(inputmodel[0], inputmodel[1])  # type: ignore
        query = _m.objects.all()  # type: QuerySet
        return query, _m  # type: ignore
    raise TypeError(f"Could not determine the model or queryset from {inputmodel}")
