    ...
    def detections_to_ai4adas_format(self, detections, objects_pred, image_shape=(1, 1, 0)):
        ...
        for obj in objects_pred:
            ...
            self.__ai4adas_objects_dict['objects'].append({"obj_id": str(object_id), "class_name": class_name,
                                                           "score": str(class_scores[object_id]*100), "xmin": str(obj.box2d[0]/w), "ymin": str(obj.box2d[1]/h), "xmax": str(obj.box2d[2]/w), "ymax": str(obj.box2d[3]/h)})
            object_id += 1
    ...
    def to_ai4adas_format(self, bboxes, labels, class_scores, image_shape=(1, 1, 0)):
        ...
        for label in labels:
            ...
            self.__ai4adas_objects_dict['objects'].append({"obj_id": str(id), "class_name": class_name,
                                                           "score": str(class_scores[id]*100), "xmin": str(bboxes[id][0]/w), "ymin": str(bboxes[id][1]/h), "xmax": str(bboxes[id][2]/w), "ymax": str(bboxes[id][3]/h)})
            id += 1
        ...
        for label_id in label_ids:
            if label_id < len(dataset_classnames['PASCAL VOC']):
                self.__ai4adas_objects_dict['objects'].append({"obj_id": str(id), "class_name": ALFA_Utils.__convert_label_id_to_ai4adas_label(label_id),
                                                               "score": str(class_scores[id][label_id+1]*100), "xmin": str(bboxes[id, 0]/w), "ymin": str(bboxes[id, 1]/h), "xmax": str(bboxes[id, 2]/w), "ymax": str(bboxes[id, 3]/h)})
            id += 1
        ...
        for i in range(classes.shape[0]):
            ...
            if cls_id >= 0:
                ...
                ai4adas_objects.append({"obj_id": str(len(ai4adas_objects)), "class_name": str(dataset_classnames['PASCAL VOC'][classes[i]]),
                                        "score": str(score*100), "xmin": str(bboxes[i, 0]/w), "ymin": str(bboxes[i, 1]/h), "xmax": str(bboxes[i, 2]/w), "ymax": str(bboxes[i, 3]/h)})
