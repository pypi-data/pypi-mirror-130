...
def prepare_for_alfa(detections, objects_pred, tail_img_name, for_alfa_detections, forJson=True, objects_labels=None, image_shape=(1, 1, 0)):
    ...
    h, w = check_image_shape(image_shape)
...
def get_image_shape(detections, image_shapes):
    ...
    if img_key in image_shapes:
        h, w = check_image_shape(image_shapes[img_key])
        return h, w
    else:
        return 1, 1
...
def transfer_ground_truth_to_alfa_apth(results_dir, image_file_name, alfa_path, ai4adas_object):
    """Save and transfer single ALFA specific formatted object as as ground truth json."""
    ...
    print(
        f'\nWriting {image_file_json} for ALFA within {alfa_path} folder...')
...
def transfer_detections_to_alfa_path(results_dir, alfa_json_name, alfa_path, ai4adas_alfa_detections):
    ...
    print(
        f'\nWriting {alfa_json_name}.json for ALFA within {results_dir} folder...')
