...
class ALFA_Utils:
    ...
    def __init__(self):
        """
         ALFA Utilities class (default constructor)

        """
        self.__alfa_results_dir = None
        self.__alfa_path_detections = None
        self.__alfa_path_ground_truth = None

        self.save_alfa = False
        self.save_ground_truth = True
    ...
    def to_ai4adas_format(self, bboxes, labels, class_scores, image_shape=(1, 1, 0)):
        ...
        for label in labels:
            ...
            self.__ai4adas_objects_dict['objects'].append({"obj_id": str(id), "class_name": class_name,
                                                           "score": ALFA_Utils.__to_ai4adas_class_score(class_scores[id]), "xmin": str(bboxes[id][0]/w), "ymin": str(bboxes[id][1]/h), "xmax": str(bboxes[id][2]/w), "ymax": str(bboxes[id][3]/h)})
            id += 1
    ...
    def convert_to_ai4adas_format(self, bboxes, label_ids, class_scores, image_shape=(1, 1, 0)):
        ...
        for label_id in label_ids:
            if label_id < len(dataset_classnames['PASCAL VOC']):
                self.__ai4adas_objects_dict['objects'].append({"obj_id": str(id), "class_name": ALFA_Utils.__convert_label_id_to_ai4adas_label(label_id),
                                                               "score": ALFA_Utils.__to_ai4adas_class_score(class_scores[id][label_id+1]), "xmin": str(bboxes[id, 0]/w), "ymin": str(bboxes[id, 1]/h), "xmax": str(bboxes[id, 2]/w), "ymax": str(bboxes[id, 3]/h)})
            id += 1
        ...
        for i in range(classes.shape[0]):
            ...
            if cls_id >= 0:
                ...
                ai4adas_objects.append({"obj_id": str(len(ai4adas_objects)), "class_name": str(dataset_classnames['PASCAL VOC'][classes[i]]),
                                        "score": ALFA_Utils.__to_ai4adas_class_score(score), "xmin": str(bboxes[i, 0]/w), "ymin": str(bboxes[i, 1]/h), "xmax": str(bboxes[i, 2]/w), "ymax": str(bboxes[i, 3]/h)})
