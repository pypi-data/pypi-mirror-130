        ...
        if not message.author.bot:
          ...
          for commandinfo in commandloader.commandsToLoad:
            ...
            if str(message.content).startswith(self.__prefix + commandname):
              ...
              while "$addVar[" in commandcode: #----------ПРОВЕРЕНО-----------
                va = commandcode.split("$addVar[")[1].split("]")[0].split(";")
                with open("variables.json", "r+", encoding = "utf-8") as f:
                  ...
                  if va[0] in data:
                    commandcode = commandcode.replace(f"$addVar[{va[0]};{va[1]}]", f"```addVar: Variable '{va[0]}' already exist```")
                  else:
                    data[va[0]] = va[1]

                    with open("variables.json", "w+", encoding="utf-8") as f:
                      json.dump(data, f, sort_keys=True, ensure_ascii=False, indent = 2)
                      commandcode = commandcode.replace(f"$addVar[{va[0]};{va[1]}]", str(""))
                      break

              while "$getVar[" in commandcode:
                try:
                  varname = commandcode.split("$getVar[")[1].split("]")[0]
                  with open("variables.json", "r+", encoding = "utf-8") as f:
                    varsname = json.loads(f.read())
                    commandcode = commandcode.replace(f"$getVar[{varname}]", str(varsname[varname]))
                except:
                  commandcode = commandcode.replace(f"$getVar[{varname}]", f"```updateVar: Variable '{varname}' is not exist```")
                  break

              while "$updateVar[" in commandcode: #----------ПРОВЕРЕНО-----------
                try:
                  varname = commandcode.split("$updateVar[")[1].split("]")[0].split(";")
                  ...
                  with open("variables.json", "r+", encoding = "utf-8") as f:
                    ...
                    if varname[0] in f_json:
                      ...
                      with open("variables.json", "r+", encoding = "utf-8") as f:
                        ...
                        commandcode = commandcode.split(f"$updateVar[{varname[0]};{varname[1]}]", str(""))
                    else:
                      commandcode = commandcode.split(f"$updateVar[{varname[0]};{varname[1]}]", f"```updateVar: Variable '{varname[0]}' is not exist```")
                except TypeError:
                  pass

              while "$openAllVars" in commandcode:
                try:
                  with open("variables.json", "r+", encoding = "utf-8") as f:
                    commandcode = commandcode.replace("$openAllVars", f"```py\n{f.read()}```")
                except:
                  commandcode = commandcode.replace("$openAllVars", "openAllVars: error")
