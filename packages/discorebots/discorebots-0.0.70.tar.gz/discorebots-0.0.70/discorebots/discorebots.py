...
from .__database__ import Database
...
class setupBot(commandloader):
  ...
  def onWebsocketResponse(self, to_console: str=None, status: str=None, status_type: str=None, text="discorebots.py lib", stream_url: str="https://discord.com"):
        ...
        try:
          if self.__status == "game":
            if self.__statust == "dnd":
              await self.__clientUse.change_presence(status = discord.Status.dnd, activity=discord.Game(name=f"{text}"))
            elif self.__statust == "idle":
              await self.__clientUse.change_presence(status = discord.Status.idle, activity=discord.Game(name=f"{text}"))
            elif self.__statust == "online":
              await self.__clientUse.change_presence(status = discord.Status.online, activity=discord.Game(name=f"{text}"))
            else:
              print("Status: Error status")
          elif self.__status == "stream":
            if self.__statust == "dnd":
              await self.__clientUse.change_presence(status = discord.Status.dnd, activity=discord.Streaming(name=f"{text}", url=f"{stream_url}"))
            elif self.__statust == "idle":
              await self.__clientUse.change_presence(status = discord.Status.idle, activity=discord.Streaming(name=f"{text}", url=f"{stream_url}"))
            elif self.__statust == "online":
              await self.__clientUse.change_presence(status = discord.Status.online, activity=discord.Streaming(name=f"{text}", url=f"{stream_url}"))
            else:
              print("Status: Error status")
          elif self.__status == "listen":
            if self.__statust == "dnd":
              await self.__clientUse.change_presence(status = discord.Status.dnd, activity=discord.Activity(type=discord.ActivityType.listening, name=f"{text}"))
            elif self.__statust == "idle":
              await self.__clientUse.change_presence(status = discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.listening, name=f"{text}"))
            elif self.__statust == "online":
              await self.__clientUse.change_presence(status = discord.Status.online, activity=discord.Activity(type=discord.ActivityType.listening, name=f"{text}"))
            else:
              print("Status: Error status")
          elif self.__status == "watch":
            if self.__statust == "dnd":
              await self.__clientUse.change_presence(status = discord.Status.dnd, activity=discord.Activity(type=discord.ActivityType.watching, name=f"{text}"))
            elif self.__statust == "idle":
              await self.__clientUse.change_presence(status = discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.watching, name=f"{text}"))
            elif self.__statust == "online":
              await self.__clientUse.change_presence(status = discord.Status.online, activity=discord.Activity(type=discord.ActivityType.watching, name=f"{text}"))
            else:
              print("Status: Error status")
          else:
            ...
            if str(trigger).lower() == str(prefix + commandname):
              ...
              while "$createUserVar[" in commandcode:
                try:
                  ...
                  try:
                    self.__database.createVar(f"user_{keys[0]}_{keys[2]}", str(f"{keys[1]}"))
              ...
              while "$updateUserVar[" in commandcode:
                try:
                  ...
                  try:
                    self.__database.updateVar(f"user_{keys[0]}_{keys[2]}", str(f"{keys[1]}"))
              ...
              while "$getUserVar[" in commandcode:
                try:
                  ...
                  try:
                    getvar = self.__database.getVar(f"user_{keys[0]}_{keys[1]}")
                    commandcode = commandcode.replace(f"$getUserVar[{keys[0]};{keys[1]}]", str(f"{getvar}"))
                    break
                  except:
                    commandcode = commandcode.replace(f"$getUserVar[{keys[0]};{keys[1]}]", str("getUserVar: Incorrect UserID"))
                    break
                except IndexError:
                  print("getUserVar: error values")
                  commandcode = commandcode.replace(f"$getUserVar[{keys[0]};{keys[1]}]", f"getUserVar: error values")
                  break
              ...
              while "$ban[" in commandcode:
                userandreason = commandcode.split("$ban[")[1].split("]")[0].split(";")
                userb = discord.utils.get(message.guild.members, id=int(userandreason[0]))
                reasonb = userandreason[1]
                try:
                  await userb.ban(reason=reasonb)
                  commandcode = commandcode.replace(f"$ban[{userb};{reasonb}]", f"")
                except Exception as e:
                  commandcode = commandcode.replace(f"$ban[{userb};{reasonb}]", f"ban: ```Failed to ban user```")
                  break
