...
task = {
    "task_type": 1,
    "prediction": [
        {"value": "Male", "is_default": 1, "description": "1"},
        {"value": "Female", "is_default": 0, "description": "0"},
    ],
    "label": [
        {"value": "Male", "is_default": 1, "description": "1"},
        {"value": "Female", "is_default": 0, "description": "0"},
...
task = Task(task_type=TaskTypes.BINARY_CLASSIFICATION, title="inline title")
...
def test_schema_has_not_reserved_names(basic_baseline_and_schema):
    ...
    with pytest.raises(EntitiesValidationError) as validation_error:
        EntitiesValidator(task, schema, baseline).prepare()
...
def test_has_all_required_roles(basic_baseline_and_schema):
    ...
    with pytest.raises(EntitiesValidationError) as validation_error:
        EntitiesValidator(task, a_schema, a_baseline).prepare()
...
def test_regression(basic_baseline_and_schema):
    task.task_type = TaskTypes.REGRESSION.value
