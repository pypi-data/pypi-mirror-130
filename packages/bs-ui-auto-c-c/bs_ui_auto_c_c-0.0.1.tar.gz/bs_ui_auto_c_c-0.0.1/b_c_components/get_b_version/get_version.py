...
from b_c_components.get_config.get_project_root_path import project_root_path
...
def auto_get_browser_driver(config_path):
    ...
    if sys.platform == 'linux':
        linux_data = subprocess.run("google-chrome --version", shell=True, stdout=subprocess.PIPE,
                                    stderr=subprocess.PIPE, encoding="utf-8", timeout=1)
        b_version = re.findall('Google Chrome (.+?) \\n', linux_data.stdout)
        b_version = b_version[0].split('.')[0]
        if b_version not in os.listdir(
                project_root_path() +
                'b_c_components/driver/linux/chrome'):
            os.mkdir(project_root_path() +
                     'b_c_components/driver/linux/chrome/' + b_version)
        if 'chromedriver' not in os.listdir(project_root_path() +
                 'b_c_components/driver/linux/chrome/' + b_version + '/'):
            b_version_complete = requests.get(
                'http://npm.taobao.org/mirrors/chromedriver/LATEST_RELEASE_' +
                b_version).text
            url = 'http://npm.taobao.org/mirrors/chromedriver/' + \
                  b_version_complete + '/chromedriver_linux64.zip'
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko)\
            Chrome/78.0.3904.87 Safari/537.36'}
            filename = project_root_path() + 'b_c_components/driver/linux/chrome/' + b_version
            r = requests.get(url, headers, timeout=30)
            with open(filename + "/chromedriver.zip", "wb") as code:
                code.write(r.content)
            unzip_file(filename + "/chromedriver.zip", filename)
        return os.path.dirname(os.path.dirname(__file__)) + '/driver/linux/chrome/' + b_version + "/chromedriver"
