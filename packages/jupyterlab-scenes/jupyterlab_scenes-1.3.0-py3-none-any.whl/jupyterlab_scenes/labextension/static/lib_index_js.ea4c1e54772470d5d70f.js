...
class NotebookHandler {
    ...
    importLegacyInitializationCells(notebook) {
        ...
        console.log('init_scenes_consistent', init_scenes_consistent);
        console.log('legacy_init_cells_exist', legacy_init_cells_exist);
        if (!init_scenes_consistent && legacy_init_cells_exist) {
            console.log('import');
            const scene_name = 'Legacy Init';
            notebook.widgets.map((cell) => {
                let is_legacy_init_cell = !!cell.model.metadata.get('init_cell');
                console.log(cell, is_legacy_init_cell);
                if (is_legacy_init_cell) {
                    console.log('bla');
                    cell.model.metadata.set(this._getSceneTag(scene_name), true);
                }
            });
            this.createNewEmptyScene(scene_name);
            //this._scenesChanged();
        }
    }
    /* ****************************************************************************************************************************************
...
//# sourceMappingURL=lib_index_js.ea4c1e54772470d5d70f.js.map