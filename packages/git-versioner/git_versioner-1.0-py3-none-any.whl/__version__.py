...
def git_version():
    parts, tag_name, g_hash, on_tag = get_version_info_from_git()
    if not parts:
    ...
    return vers_short, vers_long, g_hash
...
try:
    from _version import version, version_short, git_hash
except:
    pass

error = None
try:
    version_short, version, git_hash = git_version()
...
def main():
    global version, version_short, git_hash, VERBOSE
