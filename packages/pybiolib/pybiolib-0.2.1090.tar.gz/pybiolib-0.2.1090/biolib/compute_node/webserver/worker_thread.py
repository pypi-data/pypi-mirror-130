...
class WorkerThread(threading.Thread):
    ...
    @staticmethod
    def _finalize_and_clean_up_compute_job_with_logging(job_id, log_message):
    ...
    def terminate(self):
        ...
        if self.compute_state['result'] or self.compute_state['status']['error_code']:
            seconds_to_sleep = 60
            logger.debug(f'Worker thread sleeping for {seconds_to_sleep} seconds before cleaning up job {job_id}')
            # sleep to see if the user has begun downloading the result or fetched the error
            time.sleep(seconds_to_sleep)
            if self.compute_state['result']:
                self._finalize_and_clean_up_compute_job_with_logging(
                    job_id=job_id,
                    log_message=f"Job {job_id}'s result was not fetched within {seconds_to_sleep}s, cleaning up..."
                )

            elif self.compute_state['status']['error_code']:
                self._finalize_and_clean_up_compute_job_with_logging(
                    job_id=job_id,
                    log_message=f"Job {job_id}'s error code was not fetched within {seconds_to_sleep}s, cleaning up..."
                )

        # Just clean up job if we don't have to wait for client
        else:
            webserver_utils.finalize_and_clean_up_compute_job(job_id)
