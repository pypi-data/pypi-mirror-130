...
def get_model_flops(model, input_res, print_per_layer_stat=True, input_constructor=None):
    ...
    if input_constructor:
        ...
        if type(model.forward) is functools.partial:
            _ = flops_model([batch])
        else:
            _ = flops_model(batch)
...
def get_model_complexity_info(model0, input_res, print_per_layer_stat=True, as_strings=False, input_constructor=None):
