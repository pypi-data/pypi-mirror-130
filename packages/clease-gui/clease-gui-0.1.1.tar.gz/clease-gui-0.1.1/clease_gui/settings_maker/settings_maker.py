...
def _make_subtab(*sub_dashboards: _SubDashboard) -> widgets.Tab:
    ...
    children = [widgets.Output() for _ in sub_dashboards]
    tab = widgets.Tab(children=children)
    for ii, subdash in enumerate(sub_dashboards):
        tab.set_title(ii, subdash.tab_name)
        output_widget = children[ii]  # The Output object we draw the dashboard on
        with output_widget:
            clear_output()
            subdash.dashboard.display()
    return tab
