...
def get_open_auction_info(order_book_ids, start_date=None, end_date=None, market="cn"):
    ...
    history_permission_denied = realtime_permission_denied = False
    try:
        data = get_client().execute("get_open_auction_info_daybar", order_book_ids,
    ...
    if data:
        ...
        df = convert_bar_to_multi_df(data, 'datetime', OA_FIELDS + extra_fields, int17_to_datetime_v, default=0.0)
        if df is None:
