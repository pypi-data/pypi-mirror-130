...
class Port(Markovitz):
    ...
    rfr=0.023467/365\n
    market_index='沪深300指数'\n
    path --> 默认缓存路径为：".\\Fund cache\\"，可传入False不缓存
    '''
    ...
    def max_loss(self, level=0.05):
        ...
        '''
        df_pctChg = self.data[['name', 'date', 'pctChg']]
        return dict(zip(self.names, [abs(np.percentile(df_pctChg[df_pctChg.name == name]['pctChg'], level*100)) for name in self.names]))
