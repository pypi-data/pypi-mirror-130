...
class GeneralDevice:
                    ...
                    if channel in self._channels:
                        ...
                        if channelvalue != setupvalue:
                            ...
                            if self._setup[name].forced:
                                ...
                                logging.info(str(self._name) + " forcing " + str(name) + " to "+str(setupvalue)+".")
                            else:
        ...
        if self._datasetdefinitions != None:
            for dataset in self._datasetdefinitions:
                try:
                    ...
                    logging.warning(str(self._name) + ", dataset=" + str(dataset) + ", channel not found")
                    value = None
                if value != None:
                    element = {
                        "time": self._registerTimeStamp,
    ...
    def getintlogvalue(self, datasetname):      #todo change everywhere to getlogvalue(datasetname, "int") and cleanup
        # logdata = self.getlogvalue(datasetname)
        # if logdata != None:
        #     return int(logdata)
        # else:
        #     return None
        return self.getlogvalue(datasetname, "int")
    ...
    def showinfo(self):
        if self._datasetdefinitions != None:
            for dataset in self._datasetdefinitions:
                try:
                    logging.info(str(self._name) + " " + dataset + ": Live: " + str(self.getchannelvalue(dataset)) + ", Log: " + str(self.getlogvalue(dataset)) + ", HealthState: " + str(self.gethealthstate()))
                except Exception as e:
                    logging.debug(str(self._name) + " Error printing deviceInfo. " + str(e))
        else:
            logging.debug(str(self._name) + " no datasets defined")
    ...
    def getchannelvalue(self, channel, safemode=False):
        try:
            ...
            for register in config.registers:
                try:
                    ...
                    logging.warning("Register " + str(e) + " for channel " + str(channel) + " not found. Device: " + str(self._name))
