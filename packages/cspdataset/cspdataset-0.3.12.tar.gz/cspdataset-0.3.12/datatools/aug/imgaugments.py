...
def label_create_split(args):
    """
    valid_data would be splited from original, not include the augment data.
    """

    def create_label(lines, label_type='train'):
        with open(args["basic"]["top_dir"] + os.sep + 'ImageSets/Main/%s.txt' % label_type, 'w', encoding='utf8') as f:
            f.writelines(lines)

    os.makedirs(args["basic"]["top_dir"] + os.sep + 'ImageSets/Main', exist_ok=True)
    lines = []
    for labelfile in os.listdir(args["basic"]["top_dir"] + os.sep + 'Annotations'):
...
def data_aug(aug_file):
    config_dict, methods = get_methods_from_yml(aug_file)
    top_dir = config_dict["basic"]["top_dir"]

    t1 = time.time()
    if not os.path.exists(top_dir):
        raise FileNotFoundError('未能找到相应文件夹，请核查传入的路径')
    IMG_DIR = top_dir + os.sep + "JPEGImages"
    XML_DIR = top_dir + os.sep + "Annotations"
    reset_dir(IMG_DIR)
    reset_dir(XML_DIR)
    ...
    if config_dict["basic"]["split"]:
        label_create_split(config_dict)

    # 生成新的train.txt
    os.makedirs(os.path.join(top_dir, "ImageSets", "Main"))
    txt_path = os.path.join(top_dir, "ImageSets", "Main", "trainval.txt")
    xml_names = []
