""" 
 wrapper for the tokenize module
 see pegen/tokenizer.py
"""

from typing import Dict, Iterator, List
from . import hdl_tokenizer as tokenize
...
def shorttok(tok: tokenize.TokenInfo) -> str:
    return "%-25.25s" % f"{tok.start[0]}.{tok.start[1]}: {tokenize.tok_name[tok.type]}:{tok.string!r}"
...
class Tokenizer:
    ...
    def __init__(self, tokengen: Iterator[tokenize.TokenInfo], *, path: str = "", verbose: bool = False):
        self._tokengen = tokengen  # token generator
        self._tokens = []          # cached tokens
        self._index = 0
    ...
    def peek(self) -> tokenize.TokenInfo:
        ...
        while self._index == len(self._tokens):   # break when _index = len(_tokens)-1
            ...
            # skip second newline
            if (
                tok.type == tokenize.NEWLINE
                and self._tokens
                and self._tokens[-1].type == tokenize.NEWLINE
            ):
                continue
