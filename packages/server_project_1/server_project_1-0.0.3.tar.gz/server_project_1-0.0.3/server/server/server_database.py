...
class ServerStorage:
    ...
    def check_user(self, name):
        ...
        if self.session.query(self.AllUsers.name).filter_by(name=name).count():
            return True
        else:
            return False
    ...
    def get_contacts(self, username):
        ...
        print('RESULT CONTACT', self.UsersContacts, self.AllUsers.name)
...
if __name__ == '__main__':
    test_db = ServerStorage(str(BASE_DIR) + '\server_base.db3')
    # test_db.user_login('1111', '192.168.1.113', 8080, 'key')
