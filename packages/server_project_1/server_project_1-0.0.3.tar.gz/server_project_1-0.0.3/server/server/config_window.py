...
BASE_DIR = os.getcwd()
configsfile = os.path.join(BASE_DIR, 'config', 'configs.json')
...
class ConfigWindow(QDialog):
    ...
    def initUI(self):
        ...
        print(self.config)
        self.db_path.insert(self.config['database_path'])
        self.db_file.insert(self.config['database_file'])
        self.port.insert(self.config['port'])
        self.ip.insert(self.config['address'])
    ...
    def save_server_config(self):
        ...
        file_name = self.db_file.text()
        print('SERVER ', path, file_name, self.port.text(), self.ip.text())
        try:
            ...
            if 1023 < port < 65536:
                port = str(port)
                print('!!!!!', configsfile)
                with open(configsfile, encoding="utf-8") as configs:
                ...
                with open(configsfile, 'w', encoding='utf-8') as f:
