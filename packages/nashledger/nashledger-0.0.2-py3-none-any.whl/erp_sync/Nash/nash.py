...
class Nash(Resource):
    ...
    def login(self, payload = None, method='POST',endpoint="/auth/login"):
        ...
        self._response = Users(self).login(payload, method, endpoint).response()
    ...
    def sign_up(self, payload = None, method='POST',endpoint="/users", log_in_user = True):
        ...
        if log_in_user:
            if 'created_at' in self._response:
                ...
                self.login(payload = { "username" : payload.get("username"), "password" : payload.get("password") }).response()
