...
cc.fill_dict((1,0),('platform2.png','empty'))
    ...
    def blit_tiles(self,window,*object_camera):
        ...
        """object.blit_tiles(window,*object_camera) - отображение object в окне window. Если за каким-либо объектом должна двигаться встроенная в модуль камера, в object_camera  всех объектов TileTool 
передать объект класса camera.

object.blit_tiles(window,*object_camera) - display object in window. If the  built-in-module camera needs to follow any object, pass the camera class object in object_camera parameter of all
TileTool objects. 
