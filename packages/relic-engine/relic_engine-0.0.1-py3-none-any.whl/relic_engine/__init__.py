...
def get_wfcd_api(rebuild=False):
    if os.path.isfile('wfcd_api.json') and not rebuild:
        with open('wfcd_api.json') as f:
            return json.load(f)
    else:
        ...
        with open('wfcd_api.json', 'w') as fp:
            json.dump(wfcd_api_parsed, fp, indent=4)
...
def get_relic_list(rebuild=False):
    if os.path.isfile('relic_list.json') and not rebuild:
        with open('relic_list.json') as f:
            return json.load(f)
    else:
        ...
        with open('relic_list.json', 'w') as fp:
            json.dump(relic_list, fp, indent=4)
...
def get_set_data(rebuild=False):
    if os.path.isfile('set_data.json') and not rebuild:
        with open('set_data.json') as f:
            return json.load(f)
    else:
        # List of prime parts
        part_list = []
        ...
        with open('set_data.json', 'w') as fp:
            json.dump(set_data, fp, indent=4)
...
def get_relic_data(rebuild=False):
    # Opens the relic list json and stores it as relic_list
    if os.path.isfile('relic_data.json') and not rebuild:
        with open('relic_data.json') as f:
            relic_data = json.load(f)
    else:
        relic_data = get_relic_list()
        ...
        with open('relic_data.json', 'w') as fp:
            json.dump(relic_data, fp, indent=4)

    return relic_data


def update_prices_and_averages():
    set_data = get_set_data()
