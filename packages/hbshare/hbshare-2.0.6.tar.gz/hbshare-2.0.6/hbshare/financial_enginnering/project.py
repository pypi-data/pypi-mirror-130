...
class Config:
    ...
    def __init__(self):
        ...
        with open("hbshare/financial_enginnering/config/Database_Connnection.json", "r") as load_f:
            self.DBconfig= json.load(load_f)
        with open("hbshare/financial_enginnering/config/Columns_Translation.json", "r") as load_f:
            self.Columns_name_trans =  json.load(load_f)
            # json.dump(new_dict, f)
        with open("hbshare/financial_enginnering/config/JY_Sql.json", "r") as load_f:
            self.Jy_sql =  json.load(load_f)
        with open("hbshare/financial_enginnering/config/Mkr_code.json", "r") as load_f:
            self.Mkr_code =  json.load(load_f)
        with open("hbshare/financial_enginnering/config/IndexType.json", "r") as load_f:
...
class Projects:
    ...
    def prv_hld_analysis (self,prd_name,tablename):
        ...
        df3 = Jydb.extract_fin_info(sec_code)

        # extra benchmark data from the JY database
        df4 = Jydb.extract_benchmark(config.Index_type, sec_code=sec_code)
