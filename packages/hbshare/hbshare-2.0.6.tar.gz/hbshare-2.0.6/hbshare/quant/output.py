...
from pyecharts.charts import Line, Tab, Grid, Bar, Boxplot
...
def market_win(start_date, end_date, freq='D', table='index_win', sql_path=sql_write_path_hb['daily']):
    ...
    c.add_yaxis(
        ...
        y_axis=index_win_data[['Q1', 'Q25', 'MEDIAN', 'Q75', 'Q99']].round(2).values.tolist(),
        yaxis_index=1,
        tooltip_opts=opts.TooltipOpts(
            formatter=JsCode(
    ...
    web.extend_axis(
        yaxis=opts.AxisOpts(
            ...
            position="right",
            # interval=5,
            min_=index_win_data[['Q1', 'Q25', 'MEDIAN', 'Q75', 'Q99']].round(2).min().min(),
            max_=index_win_data[['Q1', 'Q25', 'MEDIAN', 'Q75', 'Q99']].round(2).max().max(),
        )
    ).extend_axis(
        yaxis=opts.AxisOpts(
            name='指数涨跌幅',
