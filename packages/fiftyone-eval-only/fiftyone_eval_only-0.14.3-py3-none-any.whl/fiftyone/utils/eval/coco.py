...
class COCODetectionResults(DetectionResults):
    ...
    def plot_pr_curves(self, classes=None, backend="plotly", iou_thresh=None, **kwargs):
        ...
        if iou_thresh is None:
            _classwise_AP = self._classwise_AP
        else:
            if iou_thresh in self.iou_threshs:
                index_iou = np.where(self.iou_threshs == iou_thresh)[0]
                _classwise_AP = np.mean(self.precision[index_iou], axis=(0, 2))
            else:
                logger.info(f"IOU threshold {iou_thresh} not in {self.iou_threshs}")
                return

        if not classes:
            inds = np.argsort(_classwise_AP)[::-1][:3]
            classes = self.classes[inds]

        precisions = []
        for c in classes:
            class_ind = self._get_class_index(c)
            if index_iou is None:
                precisions.append(np.mean(self.precision[:, class_ind], axis=0))
            else:
                precisions.append(np.mean(self.precision[index_iou, class_ind], axis=0))

        return fop.plot_pr_curves(
            precisions, self.recall, classes, backend=backend, **kwargs
        )
    
    def mAP(self, classes=None, iou_thresh=None, weights=None):
        ...
        if classes is not None:
            ...
            classwise_AP = _classwise_AP[class_inds]
        else:
            classwise_AP = _classwise_AP

        classwise_AP = classwise_AP[classwise_AP > -1]
        if classwise_AP.size == 0:
            return -1

        return np.average(classwise_AP, weights=weights)
