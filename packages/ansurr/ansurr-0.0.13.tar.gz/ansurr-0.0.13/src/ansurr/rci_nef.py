            ...
            if panav:
                if is_tool('java'):
                    ...
                    for c in s.chains:
                        ...
                        panav_set_chain[str(os.path.basename(os.path.splitext(path_to_nef_file)[0]))+"_"+c.i+"_"+str(s.i)+'.panav'] = {'set':s.i,'chain':c.i}
                        old_sequence_ID = ''
                        seq_single_letter = ''
                        lines_shifts = []
                        num = 1
                        for r in c.residues:   
                            for a in r.shifts:
                                lines_shifts.append(str(num)+' '+str(r.i)+' '+r.name+' '+a.atom_type+' '+str(a.shift) + ' 0 1')
                                num+=1
                ...
                for p in glob.glob(output_dir+'PANAV/*_forpanav.out_calibrated'):
                    ...
                    for line in open(output_dir+'PANAV/'+panav_output,'r'):
                        ...
                        try:
                            ...
                            if line[1] != '      ' or line[2] != '      ' or line[3] != '      ' or line[4] != '      ' or line[5] != '      ' or line[6] != '      ': 
                                for chemical_shift_set in enumerate(cs_result_sets):  
                                    if panav_set_chain[panav_output]["set"] == chemical_shift_set[0]+1:
                                        for c in s.chains:                                               # iterate over different chains
                                            if panav_set_chain[panav_output]["chain"] == c.i:
                                                for r in c.residues:  
                                                    if resi == r.i:
                                                        append_reref_shift(r,'C',line[1])
                                                        append_reref_shift(r,'CA',line[2])
