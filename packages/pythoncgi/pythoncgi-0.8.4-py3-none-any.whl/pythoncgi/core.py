...
def parse_multi_body(body, ib: bytes, buf: int = get_settings("buf_size")):
    def strip(b):
        b = memoryview(b)
        nl = b[-2:]
        if nl == b"\r\n":
    ...
    def read_file():
        ...
        # content = b""
        while True:
            buffer = body.read(buf+len(sig)+2)
            try:
...
def read_request_body(buf: int = get_settings("buf_size"), parse: bool = False):
    ...
    if ctype == "application/x-www-form-urlencoded":
        ...
        if parse:
            return parse_multi_body(body, ib, buf)
        else:
