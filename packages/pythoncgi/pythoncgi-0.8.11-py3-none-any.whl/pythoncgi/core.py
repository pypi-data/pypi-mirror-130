...
def request_body_wrapper(body, length: int, buf: int = get_settings("buf_size")):
    read = 0
    while True:
        if read >= length:
            break
        size = min(buf, length-read)
        c = yield body.read(size)
        read += size
        if not c:
            break


def parse_multi_body(body, length: int, ib: bytes, buf: int = get_settings("buf_size")):
    def strip(b):
        b = memoryview(b)
        nl = b[-2:]
        if nl == b"\r\n":
    ...
    def read_file():
        ...
        def _read(n=1):
            ...
            if overread:
                if len(overread) >= n:
                    overread = memoryview(overread)
                    try:
                        return overread[:n].tobytes()
                    finally:
                        overread = overread[n:].tobytes()
                elif read+n-len(overread) <= length:
                    read += n-len(overread)
                    return overread+body.read(n-len(overread))
                else:
                    read += length-read
                    return overread+body.read(length-read)
            else:
                read += n
                return body.read(n)
        sig = b"--"+ib
    ...
    while True:
        ...
        if nl_pos == -1:
            ...
            overread = memoryview(overread)
            what = overread[:nl_pos+1].tobytes()
            overread = overread[nl_pos+1:].tobytes()
        line = overread+what.strip()
        overread = b""
        if not line:
            break
        disposition = dict([_.split("=", 1)[0], _.split("=", 1)[-1][1:-1]] for _ in line.decode().split("; ")[1:])
        if "filename" in disposition:
            read += len(body.readline())
        read += len(body.readline())
        if "filename" in disposition:
            disposition["file"] = read_file()
        else:
            data = b""
            while True:
                if read >= length:
