...
def set_log_level(logger_name: str, level: int) -> None:
    _ipyeos.set_log_level(logger_name, level)

def set_block_interval_ms(ms: int) -> None:
    _ipyeos.set_block_interval_ms(ms)
...
def pack_native_object(_type: int, obj: Union[dict, str]) -> bytes:
    ...
    return _ipyeos.pack_native_object(_type, obj)

def unpack_native_object(_type: int, packed_obj: bytes) -> dict:
    return _ipyeos.unpack_native_object(_type, packed_obj)
...
def pack_block(obj: Union[dict, str]) -> bytes:
    if isinstance(obj, dict):
        ...
        assert isinstance(obj, (str, bytes))
    return _ipyeos.pack_native_object(NativeType.signed_block, obj)

def unpack_block(packed_obj: bytes) -> dict:
    return _ipyeos.unpack_native_object(NativeType.signed_block, packed_obj)

def unpack_transaction(packed_obj: bytes) -> dict:
    return _ipyeos.unpack_native_object(NativeType.packed_transaction_v0, packed_obj)
...
def s2n(s: str) -> int:
    ...
    '''
    return _ipyeos.s2n(s)

def n2s(n: int) -> str:
    '''
    Convert int to a EOSIO name
    '''
    return _ipyeos.n2s(n)
...
def set_native_contract(contract: Name, native_contract_lib) -> bool:
    return _ipyeos.set_native_contract(contract, native_contract_lib)

def get_native_contract(contract) -> str:
    return _ipyeos.get_native_contract(contract)

def enable_native_contracts(debug) -> None:
    _ipyeos.enable_native_contracts(debug)

def is_native_contracts_enabled() -> bool:
    return _ipyeos.is_native_contracts_enabled()

def init(argv=None) -> bool:
    if argv:
        return _ipyeos.init(argv)
    return _ipyeos.init(sys.argv)
