...
function setup_named_new_dataset_in_named_new_package_in_named_new_org(){
  LOAD_TYPE=$1
  setup_new_dataset_in_new_package_in_new_org $1 $2 $3 $4 $5 $6 $7
...
function setup_new_dataset_in_new_package_in_new_org(){
  ...
  setup_user_package_creation "$ORG_ID"
...
#    output=$(curl -s -X GET -H 'Content-Type: application/json' -H "Authorization: Bearer $JWT" \
...
#        }
#    }'
#    output=$(curl -s -X PATCH -H 'Content-Type: application/json' -H "Authorization: Bearer $JWT" \
...
#  output=$(curl -s -X DELETE -H 'Content-Type: application/json' -H "Authorization: Bearer $JWT" \
