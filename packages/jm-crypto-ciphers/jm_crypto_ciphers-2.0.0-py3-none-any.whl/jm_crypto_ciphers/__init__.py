...
class MyCiphers:
  ...
  def initialPerm(self, x):
    permutation = [58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8, 57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7]
  ...
  def rotate_left(self, i28, k):
    print("I added rotate_left func from PyPi DES")
  ...
  def genRoundKeys(self, numRounds, key):
    if len(key) == 64:
      ...
      print("My PC-1 input: ", int(key, 2))
      print("My PC-1 output: ", int(out, 2))
    elif len(key) == 56:
      out = key
    else:
    ...
    rndKeys = [0] * numRounds
    print("Key generation fixed!!")
    for i in range(numRounds):
      if i == 0 or i == 1 or i == 8 or i == 15: # Rotate left 1 bit
  ...
  def des_encdec(self, msg, keys, nRounds, purpose):    
    ...
    print("My initial perm out: ", int(msg, 2))
    
    # Split plaintext into two halves
    block = [msg[:32], msg[32:]]

    for key in roundKeys:
      block = [block[1], utils.xor(block[0], self.fFunc(block[1], key))]
