...
ATTENTION: each task is run in a Process. If the task is easy, e.g. calculate the square root of a number, then use
Palpable to calculate the square root of 1000 numbers will be way slower than directly calculate it, due to the overhead
of Process creation, monitoring, communication, and decommission.
...
There are three classes that do the heavy lifting: Server, Manager, Worker. Each of them has a main thread loop that do
some job:
