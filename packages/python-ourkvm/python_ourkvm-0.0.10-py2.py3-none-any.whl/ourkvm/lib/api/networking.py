...
@app.patch("/networking/interface/{name}/connect/{master}")
def set_master_on_interface(name :str, master :str, current_user: User = Depends(get_current_user)) -> bool:
	"""
...
@app.patch("/networking/interface/{name}/move/{namespace}")
def move_interface_to_namespace(name :str, namespace :str, current_user: User = Depends(get_current_user)) -> bool:
	"""
