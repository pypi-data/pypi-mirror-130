...
def discovery_types(request: Request) -> List[str]:
    discovery_types = (
        _type for _type in sorted(request.app.state.XDS_TEMPLATES["__any__"].keys())
    )
    ...
    return [t.value for t in DiscoveryTypes]
...
async def ui_main(request: Request) -> Response:
    try:
        return html_templates.TemplateResponse(
            ...
            context={
                "request": request,
                "all_types": discovery_types(request),
                "last_update": str(request.app.state.POLLER.last_updated),
            },
        )
    except IndexError:
        return html_templates.TemplateResponse(
...
async def resources(
    ...
    return html_templates.TemplateResponse(
        ...
        context={
            ...
            "all_types": discovery_types(request),
            "version": envoy_version,
            "available_versions": list(request.app.state.XDS_TEMPLATES.keys()),
            "service_cluster": service_cluster,
            "available_service_clusters": request.app.state.POLLER.match_keys,
            "last_update": str(request.app.state.POLLER.last_updated),
        },
    )
