...
def select_template(
    ...
    discovery_type: str,
    templates: Dict[str, Dict[str, XdsTemplate]],
) -> XdsTemplate:
...
async def response(
    r: Request, request: DiscoveryRequest, xds_type: str
) -> ProcessedTemplate:
    """
    ...
    """
    template: XdsTemplate = select_template(
        request, xds_type, r.app.state.XDS_TEMPLATES
    )
    context: Dict[str, Any] = await r.app.state.TEMPLATE_CONTEXT.get_context(
        request, template
    )

    config_version: Optional[str] = None
    cache_strat = r.app.state.CONFIG.source_config.cache_strategy

    if cache_strat.context:
        config_version = compute_hash(
            r.app.state.TEMPLATE_CONTEXT.checksum,
            await template.checksum,
            request.node.common,
    ...
    if not template.is_python_source:
        ...
        content = deserialize_config(content, r)

    if cache_strat.content:
...
def deserialize_config(content: str, r: Request) -> Dict[str, Any]:
    try:
        ...
        if SENTRY_INSTALLED and r.app.state.SENTRY_DSN:
