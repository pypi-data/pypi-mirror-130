...
from typing import Any, Dict, Callable, Union, Coroutine
...
class Loadable(BaseModel):
    ...
    async def load(self, default: Any = None) -> Any:
        try:
            return await loaders[self.protocol](self.path, self.serialization)
        except Exception:
...
async def load_file(path: str, loader: Serialization) -> Any:
    with open(path) as f:
        contents = f.read()
        try:
...
async def load_package_data(path: str, loader: Serialization) -> Any:
...
async def load_http(path: str, loader: Serialization) -> Any:
...
async def load_env(variable: str, loader: Serialization = Serialization.raw) -> Any:
...
async def load_module(name: str, _: Serialization = Serialization.raw) -> Any:
    if ":" in name:
...
async def load_s3(path: str, loader: Serialization = Serialization.raw) -> Any:
...
async def load_python(path: str, _: Serialization = Serialization.raw) -> ModuleType:
    p = str(Path(path).absolute())
    loader = SourceFileLoader(p, path=p)
    return loader.load_module(p)


async def load_inline(path: str, loader: Serialization = Serialization.raw) -> Any:
    return serializers[loader](str(path))


LoaderFunction = Coroutine[Union[str, Any], Any, Any]
loaders: Dict[Protocol, Callable[[str, Serialization], LoaderFunction]] = {
