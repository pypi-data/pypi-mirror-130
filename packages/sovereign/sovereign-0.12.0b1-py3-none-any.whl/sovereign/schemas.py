...
class StatsdConfig(BaseModel):
    ...
    async def loaded_tags(self, v: Dict[str, Union[Loadable, str]]) -> Dict[str, Any]:
        ret = dict()
        for key, value in v.items():
            if isinstance(value, dict):
                ret[key] = await Loadable(**value).load()
            elif isinstance(value, str):
                ret[key] = await Loadable.from_legacy_fmt(value).load()
            else:
...
class XdsTemplate:
    ...
    @property
    async def checksum(self) -> int:
        if self.source is None:
            self.source = await self.load_source()
        return zlib.adler32(self.source.encode())

    @property
    async def jinja_variables(self) -> List[str]:
        if self.source is None:
            self.source = await self.load_source()
    ...
    async def __call__(self, *args: Any, **kwargs: Any) -> Union[Dict[str, Any], str]:
        if not hasattr(self, "code"):
            self.code: Union[Template, ModuleType] = await self.loadable.load()
    ...
    async def load_source(self) -> str:
