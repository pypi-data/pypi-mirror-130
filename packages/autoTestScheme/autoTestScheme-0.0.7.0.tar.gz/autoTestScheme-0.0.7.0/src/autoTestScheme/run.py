...
from .common import constant, config, common, start_data, robot
...
class Run(object):
    ...
    def set_logger(self, level="DEBUG", log_file_level="DEBUG", is_allure=True, logger_folder_name='logs',
        ...
        :return:
        """
        start_data.Logger_START_STR = logger_start_str
        start_data.LOGGER_FOLDER = logger_folder_name
        logger.remove()
        logger.add(sys.stderr, level=level)
        logger.add(self.get_log_path(), level=log_file_level, rotation="200 MB")
        if is_allure is True:
            logger.add(self.PropogateHandler(), level=level, format="{time:YYYY-MM-DD at HH:mm:ss} | {message}")
