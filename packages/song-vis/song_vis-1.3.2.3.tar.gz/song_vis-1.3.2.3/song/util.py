...
def union_of_neighors(bmus, neighbors):
    bool_out = np.zeros(len(bmus), dtype=np.int8)
...
def get_fast_knn(X_pc, W_pc, x_k, G):
    ...
    for i in range(X_pc.shape[0]):
        ...
        candidates = combine_neighbors(reverse_index, np.where(G_neis == 1)[0])#np.where(union_of_neighors(closest_cvs, np.where(G_neis==1)[0]))[0]
        # contains[closest_cvs[i]] = 1
        # candidates = shp[contains>0]
        sum_candidates += len(candidates)
        knn_entry, dists = get_fast_pairwise(X_pc[i].reshape(1, X_pc.shape[1]), X_pc[candidates], x_k)
        # indices, distances = NNDescent(X_pc[candidates]).query(X_pc[i].reshape(1, X_pc.shape[1]), k = x_k)
    ...
    print(sum_candidates/X_pc.shape[0])
