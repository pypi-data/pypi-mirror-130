...
from ._pushSignals import PushSignals,PushSignalsOsci,PushSignalsCap,PushSignalsCapOsci
...
def seeq_add_on(sdl_notebook_url):
    ...
    def on_click_push(widget, event, data):
        ...
        if len(overall_results)>0 and checkbox_selected_stiction.v_model==True:
            ...
            if select_capsule.v_model=="":
                ...
                status=PushSignalsCap(df,overall_results,worksheet_url,name,df_capsule[select_capsule.v_model])
        ...
        if len(overall_results_oscillation)>0 and checkbox_selected_oscillation.v_model==True:
            ...
            if select_capsule.v_model=="":
                ...
                status=PushSignalsCapOsci(df,overall_results_oscillation,worksheet_url,name,df_capsule[select_capsule.v_model])
    ...
    global df_capsule
    #-#-#-#-#-#
    spy.login(url='https://explore.seeq.com/login', access_key='ntrBZhJVSu2a5Y0rPioSow', password='xBIOgM6Jlyion5qIVixhGNH8cBiosU', quiet=True,status=spy.Status(quiet=True))
    #-#-#-#-#-#
    ...
    }], children=['Optional: Select a conditin to limit the analysis.'])
