...
def main():
    parser = ArgumentParser(
        ...
        description="Run a Python script and generate a flamegraph from its"
            " execution",
        allow_abbrev=False,
    )
    parser.add_argument(
        ...
        metavar="path",
        help="the path to the flamegraph.pl script (if not specified, search"
            " using the PATH environment variable)",
    )
    parser.add_argument(
        "-a", "--flamegraph-extra-args",
        metavar="arg_string",
        help="a string specifying extra command line arguments to pass when"
            " invoking flamegraph.pl",
    )
    parser.add_argument(
        "-s", "--sample-interval",
        metavar="interval",
        type=float,
        default=0.001,
        help="the amount of time the sampler thread will wait, in seconds,"
            " between capturing stack traces (default: 0.001)",
    )
    parser.add_argument(
        "-o", "--output-file",
        metavar="path",
        help="the location to save the flamegraph (if not specified, save in"
            " the current directory using the name of the Python script with"
            " an \".svg\" extension appended)",
    )
    parser.add_argument(
        "script_path",
