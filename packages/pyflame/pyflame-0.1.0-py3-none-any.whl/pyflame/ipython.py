...
class PyFlameMagic(Magics):
    flamegraph_script_path = Unicode(
        None,
        allow_none=True,
        help="The path to the flamegraph.pl script.  The default value of"
            " None tells pyflame to search for flamegraph.pl using the PATH"
    ...
    default_sample_interval = Float(
        0.001,
        help="The amount time the sampler thread will wait, in seconds,"
            " between capturing stack traces."
    ).tag(config=True)
    ...
    def pyflame(self, line, cell):
        code = self.shell.compile(
            self.shell.transform_ast(
                self.shell.compile.ast_parse(
                    self.shell.transform_cell(cell)
                )
            ),
            COMPILE_FILENAME,
            'exec',
        ...
        try:
            ...
            return HTML(
                '''
                <p>Could not locate the <a
                href="https://github.com/brendangregg/FlameGraph/blob/master/flamegraph.pl">flamegraph.pl</a>
                script. Either copy flamegraph.pl to a location on your <code style="display: inline;">PATH</code>, or
                configure the path to the script by adding a snippet like the following to your Django config:</p>
                '''
            )
        else:
            return HTML(
                ...
                <div style="resize: vertical; overflow: hidden;">
                  <iframe srcdoc="{}" style="height: 100%; width: 100%;">
                </div>
                '''.format(svg.replace('&', '&amp;').replace('"', '&quot;'))
            )
