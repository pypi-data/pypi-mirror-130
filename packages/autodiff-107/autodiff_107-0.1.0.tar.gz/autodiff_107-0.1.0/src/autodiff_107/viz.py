...
def draw_graph_without_edge_labels(nodes, draw_=True):
...
def draw_graph(nodes, draw_=True):
...
def draw_graph_expensive(nodes, draw_=True):
    if type(nodes) == list:
        nodes = [node.item() if type(node) == np.ndarray else node for node in nodes]
    array_nodes=np.array(nodes)
...
def draw_reverse_graph(nodes, draw_=True):
    ...
    while len(nodequeue) > 0:
        ...
        for parent, derivative in node._d.items():
            ...
            if parent not in G:
                G.add_node(parent)
            # Retrieving the operation, which we use as label for the edge
            if labeldict[node]!='Final node':
                edge_labels[(node, parent)]= '+'+str(derivative * node._df)+'x'
            else:
