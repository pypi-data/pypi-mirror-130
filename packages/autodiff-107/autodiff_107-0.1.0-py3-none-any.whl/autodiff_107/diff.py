...
class Node:
    ...
    def __mul__(self, other):
        ...
        try:
            ...
            if self==other:
                # If this is a squaring operation, _fmd will be double what it should be
                result._fmd /= 2
    ...
    def __pow__(self, other):
        ...
        try:
            ...
            if self==other:
                # Same problem as multiplication
                result._fmd /= 2
    ...
    def _derivative(self, X):
       ...
       clear_X_dfs(X)
       self._backward()
       nd_to_df = np.vectorize(lambda x: x._df, otypes=[np.float64])
       return nd_to_df(X)
...
def variable(x):
    ...
    :param x: input
    :type x: float or array(float)
    :return: variable(input)
    :rtype: Node or array(Node)
    """
...
def value(x):
    ...
    :param x: input
    :type x: Node or array(Node)
    :return: value of Node
    :rtype: float or array(float)
    """
    node_to_val = np.vectorize(lambda x: x._value, otypes=[np.float64])
...
def set_fm_seed(X, seed):
    ...
    :type X: array(Node)
    :param seed: seed
    :type seed: array(Node)
    :raises ValueError: "X and seed shapes do not match"
    """
...
def get_fm_derivative(Y):
    ...
    :param Y: input
    :type Y: array(Node)
    :return: forward mode derivative
    :rtype: array
    """
    node_to_fmd = np.vectorize(lambda x: x._fmd, otypes=[np.float64])
...
def derivative(f, X):
    ...
    :param f: function evaluated at X
    :type f: array(Node)
    :param X: variable to take the derivative w.r.t
    :type X: array(Node)
    :raises ValueError: f and X must both be 1-dimensional
    :return: derivative of f w.r.t X
    :rtype: array
    """
    try:
