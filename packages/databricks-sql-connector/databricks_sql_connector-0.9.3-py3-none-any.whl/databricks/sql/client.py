...
_ERROR_MSG_HEADER = "X-Thriftserver-Error-Message"
...
class Connection(object):
    ...
    def _make_request(self, method, request):
        ...
        def attempt_request(attempt):
            ...
            try:
                return method(request), None, None, None
            except Exception as error:
                retry_delay = extract_retry_delay(attempt)
                error_message = getattr(self._transport, "headers", {}).get(_ERROR_MSG_HEADER)
                return None, retry_delay, error, error_message
        ...
        max_elapsed = self._retry_stop_after_attempts_duration
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            with self._request_lock:
                response, retry_delay, error, error_message = attempt_request(attempt)
            elapsed = get_elapsed()

            # conditions: success, non-retry-able, no-attempts-left, no-time-left, delay+retry
            if response: # log nothing here, presume that main request logging covers
                return response

            log_base = "{}; Request: {} Error: {} Bounded-Retry-Delay: {} " \
                    "Attempt: {}/{} Elapsed-Seconds: {}/{}".format(
                    method.__name__, request, error_message, retry_delay, attempt, max_attempts,
                    elapsed, max_elapsed)

            if (retry_delay is None
                    or attempt >= max_attempts
                    or elapsed + retry_delay > self._retry_stop_after_attempts_duration):
                _logger.error("Error during " + log_base)
                raise OperationalError("Error during Thrift request" +
                    (": " + error_message) if error_message else "",
                    error)

            _logger.info("Delaying {} seconds before retry after ".format(retry_delay) + log_base)
