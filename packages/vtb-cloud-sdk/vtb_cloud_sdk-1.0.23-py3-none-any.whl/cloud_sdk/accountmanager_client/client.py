...
class Account(Resource):
    ...
    def make_replenishment(self, amount, reason):
        url = self._crud.url(self.id, "/replenishment")
        json = {"amount": format_money(amount), "reason": reason}
        res = self._crud.client.post(url, json=json)
        return Transfer(self, res["transfer"])
...
class OrganizationAccountCrud(CRUD):
    ...
    def create(self, name, parent, limit=0, folder_uid=None):
        json = {"name": name, "limit": limit, "parent_id": self.client.get_id(parent)}
        if folder_uid:
            json["folder_uid"] = folder_uid
