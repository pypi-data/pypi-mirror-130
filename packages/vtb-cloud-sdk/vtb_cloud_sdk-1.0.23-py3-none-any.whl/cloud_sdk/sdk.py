...
from cloud_sdk.checker import CheckerClient
...
_all_services = ("calculator", "authorizer", "keycloak", "state_service", "product_catalog", "portal")


class CloudSdk:
    def __init__(self, config):
        self.config = config
        logbook.debug("Created CloudSDK: {}", config.base_url)
    ...
    def get_env_config(cls, config=None) -> Dict[str, str]:
        prefixes = tuple(service.upper() + "_" for service in _all_services)
        ...
        for name, value in os.environ.items():
            for service_prefix, service_name in zip(prefixes, _all_services):
    ...
    def _get_checker_client_pool(self):
        config = self.config.checker
        url = config.url
        client_type = functools.partial(CheckerClient, url, "",
                                        ssl_verify=config.ssl_verify,
                                        timeout=config.timeout)
    ...
    @functools.cached_property
    def pool_checker(self):
        return self._get_checker_client_pool()
