...
from vtb_py_logging.request_id import get_context_request_id, REQUEST_ID_HEADER
...
class BaseApiClient:
    ...
    def inject_request_id(self, headers):
    ...
    def _pre_request(self, method, url, query_params, data, json, headers):
        ...
        self.inject_request_id(headers)
    ...
    def replace_mode(self, raw_response=_default_stub, timeout=_default_stub, json_response=_default_stub,
                     expect_status_code=_default_stub, mode: ApiClientMode = None,
                     auth_required=_default_stub, **kwargs):
        current = self._mode
        if mode:
            self._mode = mode
        else:
            self._mode = self._mode.replace(raw_response=raw_response, timeout=timeout, json_response=json_response,
                                            ...
                                            auth_required=auth_required, **kwargs)
        try:
            yield
        finally:
            self._mode = current
