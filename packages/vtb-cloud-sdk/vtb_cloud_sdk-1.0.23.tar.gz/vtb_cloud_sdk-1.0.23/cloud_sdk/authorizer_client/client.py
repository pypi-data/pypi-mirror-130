...
class Folder(Resource):
    ...
    def get_path(self, folder_uid):
        return self._crud.client.get(self._crud.url(folder_uid, "path"))["data"]["path"]
...
class Project(Resource):
    ...
    def get_path(self):
        return self._crud.client.get(self._crud.url(self.id, "path"))["data"]["path"]
...
class ResourceRulesService(Service):
    service_name = "api/v1/resource_rules"

    def test(self, http_url, http_method, parent=None):
        json = make_params(http_url=http_url, http_method=http_method, parent=parent)
        return self.client.post(self.url("test"), json=json)["data"]

    def info(self, http_method, http_url):
        return self.client.get(self.url("info"),
                               query_params={"http_url": http_url, "http_method": http_method})["data"]
...
class AuthorizerClient(BaseApiClient):
    ...
    def __init__(self, url, ssl_verify=True, raw_http_client=None, timeout=None):
        ...
        self.resource_rules = ResourceRulesService(self)

    def get_version(self):
        return self.get("api/v1/version")

    def get_path(self, folder_id):
        if folder_id.startswith("fold-"):
            return self.get(f"api/v1/folders/{folder_id}/path")["data"]["path"]
        if folder_id.startswith("proj-"):
            return self.get(f"api/v1/projects/{folder_id}/path")["data"]["path"]
