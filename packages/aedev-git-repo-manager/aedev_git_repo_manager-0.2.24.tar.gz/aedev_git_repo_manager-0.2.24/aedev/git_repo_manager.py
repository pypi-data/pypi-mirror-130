...
import ae.base                                                                  # type: ignore # for monkey patching
from ae.base import (
...
ACTION_SHORTCUTS = {
    ...
    'prepare': 'prepare_commit', 'push': 'push_project', 'release': 'release_project', 'renew': 'new_project',
    'request': 'request_merge', 'status': 'show_status'}
...
REGISTERED_TPL_PROJECTS: Dict[str, RegisteredTemplateProject] = {}  #: projects providing templates and outsourced files
...
def _act_callable(ini_pdv: PdvType, act_name: str) -> Optional[Callable]:
    return globals().get(act_name) or getattr(ini_pdv['remote_repo_api'], act_name, None)
...
def _chk_if(error_code: int, check_result: bool, error_message):
    """ quit this shell script if the `check_result` argument is False and the `force` app option is False. """
    if not check_result:
        if cae.get_option('force'):
            cae.po(f"    # forced to ignore error {error_code}: {error_message}")
        else:
...
def _check_children_not_exists(_parent_or_root_pdv: PdvType, *children_pdv: PdvType):
    for chi_pdv in children_pdv:
        _chk_if(12, pdv_str(chi_pdv, 'project_type') == NO_PRJ,
                f"***   project {pdv_str(chi_pdv, 'project_name')} does already exist")
...
def _cl(err_code: int, command_line: str, extra_args: Sequence = (), lines_output: Optional[List[str]] = None,
        exit_on_err: bool = True, exit_msg: str = ""):
...
def _get_domain(ini_pdv: PdvType) -> str:
    repo_domain = cae.get_option('domain') or pdv_str(ini_pdv, 'repo_domain') or REPO_CODE_DOMAIN
...
def _get_package(ini_pdv: PdvType, project_type: str = NO_PRJ) -> str:
    ...
    if not package_name:
        _exit_error(9, "****  missing project path (--path) or project/package name (--package) option")
...
def _get_path_package(ini_pdv: PdvType, project_type: str = NO_PRJ) -> Tuple[str, str, str]:
    ...
    ini_prj_type = pdv_str(ini_pdv, 'project_type')
    ...
    cae.dpo(f"    = initialized project path ({project_path}) and package name ({package_name}) from command line args")
...
def _git_checkout(pdv: PdvType, branch: str, from_branch: str = ""):
...
def _git_fetch(pdv: PdvType, *branches_and_tags: str, remote: str = 'origin') -> List[str]:
    args = []
    if _debug_or_verbose():
        args.append("-v")
    args.append(remote)
...
def _git_push(pdv: PdvType, *branches_and_tags: str):
...
def _git_renew_origin_remote(pdv: PdvType):
    with in_wd(pdv_str(pdv, 'project_path')):
        git_remotes: Dict[str, str] = pdv_val(pdv, 'git_remotes')
        repo_url = pdv_str(pdv, 'repo_url')
        if 'origin' not in git_remotes:
            _cl(32, "git remote", extra_args=("add", "origin", repo_url))
        elif git_remotes['origin'] != repo_url:
            _cl(32, "git remote", extra_args=("set-url", "origin", repo_url))
...
def _git_tag(pdv: PdvType, tag: str):
    with in_wd(pdv_str(pdv, 'project_path')):
        _cl(87, "git tag", extra_args=("--annotate", "--force", "--file", _check_commit_msg_file(pdv), tag))
    ...
    if os.path.isdir(os.path.join(project_path, GIT_FOLDER_NAME)):
        ...
        with in_wd(project_path):
            ...
            for remote_id in remote_ids:
                remote_url: List[str] = []
                _cl(22, "git remote", extra_args=("get-url", remote_id), lines_output=remote_url)
...
def _init_act_exec_args() -> Tuple[str, PdvType, tuple]:
    ...
    * run optional pre_action.
    ...
    cae.po(f"----- {act_name}: {pdv_str(ini_pdv, 'project_name')}{extra_msg}")
...
def _init_act_args_check(ini_pdv: PdvType, act_spec: Any, act_name: str, act_args: ActionArgs):
    ...
    cae.dpo(f"   -- check arguments of requested action {act_name} ({act_spec['docstring'].strip('. ')})")
    ...
    if project_type not in act_spec['project_types']:
        _exit_error(33, f"  **  action '{act_name}' only available for project types: {act_spec['project_types']}")
...
def _print_pdv(pdv: PdvType):
    ...
    if not cae.get_option('verbose'):
        ...
        if pdv['project_type'] in (PARENT_PRJ, ROOT_PRJ):
            pdv['children_presets'] = [f"{preset: <9} == {' '.join(pkg_name[nsp_len:] for pkg_name in dep_packages)}"
                                       for preset, dep_packages in pdv['children_presets'].items()]
            pdv['children_project_vars'] = " ".join(pdv['children_project_vars'])
...
def _renew_prj_dir(ini_pdv: PdvType):
    namespace_name = pdv_str(ini_pdv, 'namespace_name')
    ...
    if not main_file:
        ...
        if project_type == PACKAGE_PRJ:
            main_path = join(main_path, namespace_name, portion_name if is_portion else package_name)
            main_name = PY_INIT
        elif is_portion:
...
def _renew_project(ini_pdv: PdvType, project_type: str):
    namespace_name = _get_namespace(ini_pdv, project_type)
    ...
    if old_prj_type in ANY_PRJ_TYPE and old_prj_type != project_type and not cae.get_option('path'):
        _exit_error(7, f"  *** specify --path option to change the project type from {old_prj_type} to {project_type}")
    ...
    if _git_current_branch(new_pdv) == MAIN_BRANCH:
        renew_branch = norm_name(
            cae.get_option('branch') or f"{'new' if new_repo else 'renew'}_{project_type}_{package_name}")
        if renew_branch != MAIN_BRANCH:
            _git_checkout(new_pdv, renew_branch, MAIN_BRANCH)
    ...
    _git_add(new_pdv)
    _git_renew_origin_remote(new_pdv)
...
def _write_commit_message(pdv: PdvType):
    ...
    write_file(os.path.join(pdv_str(pdv, 'project_path'), COMMIT_MSG_FILE_NAME),
               f"V{{package_version}}: {sep}{sep}{os.linesep.join(_git_status(pdv))}")
...
class GithubCom(_RemoteHost):
    ...
    def new_fork(self, _ini_pdv: PdvType, fork_repo_remote_url: str):
...
class GitlabCom(_RemoteHost):
    ...
    def new_fork(self, ini_pdv: PdvType):
        """ create/renew fork of a remote repo into our user/group namespace. """
        if pdv_str(ini_pdv, 'upstream_url'):    # renew origin from upstream if already forked
            ...
            parent_path, project_path, project_name = _get_path_package(ini_pdv)
            ...
            domain = _get_domain(ini_pdv)
            with in_wd(parent_path):
                _cl(21, "git clone", extra_args=(f"{protocol}{domain}/{usr_name}/{project_name}.git", ))
            with in_wd(project_path):
                _cl(21, "git remote", extra_args=("add", "upstream", f"{protocol}{domain}/{group_name}/{project_name}"))
    ...
    @_action()
    def push_project(self, ini_pdv: PdvType, branch_name: str = '', release: bool = False):
        ...
        push_args = [branch_name or _get_branch(ini_pdv)]
        if release:
            tag = f"v{pdv_str(ini_pdv, 'package_version')}"
            _git_tag(ini_pdv, tag)
            push_args.append(tag)
        _git_push(ini_pdv, *push_args)

    @_action(PARENT_PRJ, ROOT_PRJ)
    def release_children(self, ini_pdv: PdvType, *children_pdv: PdvType):
        """ release specified children projects to remote/origin. """
        for chi_pdv in children_pdv:
            self.release_project(chi_pdv)
        cae.po(f"  === {len(children_pdv)} {_children_desc(ini_pdv)} released")

    @_action()
    def release_project(self, ini_pdv: PdvType):
        """ merge current/specified branch into MAIN_BRANCH to be tagged and pushed to remote as new release. """
        _chk_if(44, not bool(pdv_str(ini_pdv, 'upstream_url')),
                "***** fork cannot be released directly" + _hint(
                self.push_project, " to origin instead, then do a merge-request to upstream on the Gitlab server"))

        errors = _git_fetch(ini_pdv, MAIN_BRANCH)
        _chk_if(45, not bool(errors), f" **** git fetch errors: {PPF(errors)}")

        current_branch = pdv_str(ini_pdv, 'git_current_branch') or MAIN_BRANCH
        work_branch = cae.get_option('branch') or current_branch
        if work_branch != MAIN_BRANCH:
            _git_checkout(ini_pdv, MAIN_BRANCH)
            if not _git_merge(ini_pdv, work_branch):
                _cl(46, "git merge --abort")
                return

        self.push_project(ini_pdv, branch_name=MAIN_BRANCH, release=True)

        release_branch = f"release{pdv_str(ini_pdv, 'package_version')}"
        _git_checkout(ini_pdv, release_branch, from_branch=MAIN_BRANCH)
        _git_push(ini_pdv, release_branch)
        _git_checkout(ini_pdv, current_branch)

    @_action()
    def request_merge(self, pdv: PdvType):
        """ request the merge of a forked (origin) repository into the upstream repository.

        :param pdv:             vars of the forked/origin project repository.
        """
        upstream_url = pdv_str(pdv, 'upstream_url')
        _chk_if(54, bool(upstream_url),
                "***** merge requests can only be done from a fork origin repository" + _hint(
                self.release_project, " to origin instead or push and do a manual merge-request on the Gitlab server"))
        ...
        src_prj = self.project_from_name(55, " **** origin/fork repository {name} not found on remote server",
                                         f"{_get_group(pdv)}/{_get_package(pdv)}")
        tgt_prj = self.project_from_name(56, " **** target/upstream repository {name} not found on remote server",
                                         "/".join(upstream_url.split("/")[-2:]))
        branch = _get_branch(pdv)
        merge_req = tgt_prj.mergerequests.create({
            ...
            'title': read_file(os.path.join(pdv_str(pdv, 'project_path'), COMMIT_MSG_FILE_NAME)).split(os.linesep)[0],
        ...
        if _debug_or_verbose():
            cae.po(PPF(merge_req.diffs.list()))
    ...
    def show_repo(self, ini_pdv: PdvType):
        ...
        if isinstance(repo_api, Project):
            ...
            for attr in ('created_at', 'default_branch', 'id', 'name', 'namespace_id', 'visibility'):
                cae.po(f"    - {attr} = {getattr(repo_api, attr)}")
...
def bump_version(ini_pdv: PdvType):
    ...
    bump_file_version(pdv_str(ini_pdv, 'version_file'), version_part=inc_part)
...
# protect_types ordered with root first to not get the default arg_names for children actions
@_action(PARENT_PRJ, ROOT_PRJ, pre_action=_check_children_not_exists)
def clone_children(parent_or_root_pdv: PdvType, *children_pdv: PdvType):
    ...
    for chi_pdv in children_pdv:
        clone_project(parent_or_root_pdv, pdv_str(chi_pdv, 'package_name'))

    cae.po(f"  === {len(children_pdv)} projects cloned")


@_action(ROOT_PRJ, PARENT_PRJ, arg_names=(("--package", ), ("--path", ), ("package-name", ), ("portion-name", )),
         pre_action=_check_children_not_exists)
def clone_project(ini_pdv: PdvType, *package_or_portion_name: str):
    ...
    parent_path, _project_path, package_name = _get_path_package(ini_pdv)
    package_name = package_or_portion_name[0] if package_or_portion_name else package_name
    project_path = _git_clone(repo_root, package_name, cae.get_option('branch'), parent_path)
...
@_action(PARENT_PRJ, *ANY_PRJ_TYPE)
def new_app(ini_pdv: PdvType):
    """ create or complete/renew a gui app project. """
    _renew_project(ini_pdv, APP_PRJ)


@_action(PARENT_PRJ, ROOT_PRJ)
def new_children(ini_pdv: PdvType, *children_pdv: PdvType):
    """ initialize or renew parent folder children or namespace portions. """
    for chi_pdv in children_pdv:
        cae.po(f" ---  {pdv_str(chi_pdv, 'package_name') + '  ---': <18}{pdv_str(chi_pdv, 'project_desc')}")
        new_project(chi_pdv)
    cae.po(f"  === renewed {len(children_pdv)} {_children_desc(ini_pdv)}: {(_['package_name'] for _ in children_pdv)}")


@_action(PARENT_PRJ, *ANY_PRJ_TYPE)
def new_module(ini_pdv: PdvType):
    """ create or complete/renew module project. """
    _renew_project(ini_pdv, MODULE_PRJ)


@_action(PARENT_PRJ, *ANY_PRJ_TYPE)
def new_namespace_root(ini_pdv: PdvType):
    """ create or complete/renew namespace root package. """
    _renew_project(ini_pdv, ROOT_PRJ)


@_action(PARENT_PRJ, *ANY_PRJ_TYPE)
def new_package(ini_pdv: PdvType):
    """ create or complete/renew package project. """
    _renew_project(ini_pdv, PACKAGE_PRJ)


@_action()
def new_project(ini_pdv: PdvType):
    """ complete/renew an existing project. """
    _renew_project(ini_pdv, pdv_str(ini_pdv, 'project_type'))
...
def main():
    ...
    cae.add_option('namespaces', "supported namespaces to register (for templates)", ['ae', 'aedev'], short_opt='s')
...
if __name__ == '__main__':
    try:
        ...
        _exit_error(99, f"***** exception {main_ex} raised{debug_info}")
