...
from topoly.params import Closure, ReduceMethod, PlotFormat, TopolyException, OutputFormat, Colors, Translate
...
def translate_polvalues(data, invariant, translate, chiral, external_dictionary):
    translated = find_matching_structure(data, invariant, chiral=chiral,
                                         external_dictionary=external_dictionary)
...
class Invariant(Graph):
    ...
    def calculate(self, invariant, closure=Closure.TWO_POINTS, tries=200, direction=0, reduce_method=ReduceMethod.KMT,
                  ...
                  hide_trivial=True, hide_rare=True, max_cross=15, matrix=False, density=1, level=0,
                  cuda=True, matrix_plot=False, plot_ofile="KnotFingerPrintMap", plot_format=PlotFormat.PNG,
                  output_file='', matrix_cutoff=0.48, output_format=OutputFormat.Dictionary,
                  run_parallel=False, parallel_workers=None, palette=Colors.Knots, arrows=True, debug=False):
        ...
        if hide_trivial and matrix:
            ...
            print(hide_trivial, matrix)                                          
            print(result)                 
            result = remove_trivial_knots(result)
        if hide_rare and tries != 1:
            result = remove_rare_knots(result)
        if translate:
            result = translate_polvalues(result, invariant.name, translate=translate, 
                                         chiral=chiral, external_dictionary=external_dictionary)
