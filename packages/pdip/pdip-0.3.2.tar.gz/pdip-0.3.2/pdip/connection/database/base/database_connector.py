...
class DatabaseConnector(IScoped):
    ...
    @abstractmethod
    def get_table_data_with_paging_query(self, query, order_column, start, end):
        return f"WITH TEMP_INTEGRATION AS(SELECT ordered_query.*,ROW_NUMBER() OVER ( order by \"{order_column}\") row_number FROM ({query}) ordered_query) SELECT * FROM TEMP_INTEGRATION WHERE row_number >= {start} AND row_number < {end}"
