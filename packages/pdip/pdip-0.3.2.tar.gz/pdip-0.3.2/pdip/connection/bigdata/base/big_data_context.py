...
class BigDataContext(IScoped):
    @inject
    def __init__(self,
                 big_data_policy: BigDataPolicy,
                 retry_count=3):
        self.connector: BigDataConnector = big_data_policy.connector
    ...
    def get_table_data_with_paging(self, query, order_column, start, end):
        data_query = self.connector.get_table_data_with_paging_query(query=query, order_column=order_column, start=start,
                                                                     end=end)
        return self.fetch(data_query)
