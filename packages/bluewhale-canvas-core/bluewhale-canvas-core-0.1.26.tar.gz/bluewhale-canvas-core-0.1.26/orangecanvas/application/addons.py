    ...
    def _next(self):
        ...
        try:
            if command == Install \
                ...
                try:
                    install_name = pkg.installable.label
                except:
                    install_name = pkg.installable.name

                self.setStatusMessage(
                    __("help.install").format(install_name))
                if self.conda:
                    try:
                ...
                try:
                    upgrade_name = pkg.installable.label
                except:
                    upgrade_name = pkg.installable.name

                self.setStatusMessage(
                    __("help.upgrading").format(upgrade_name))
                if self.conda:
                    try:
                ...
                try:
                    uninstall_name = pkg[0].label
                except:
                    uninstall_name = pkg.local.project_name

                self.setStatusMessage(
                    __("help.uninstall").format(uninstall_name))
                if self.conda:
                    try:
