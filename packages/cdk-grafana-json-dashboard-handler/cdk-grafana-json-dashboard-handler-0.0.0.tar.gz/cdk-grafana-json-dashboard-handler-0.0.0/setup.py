import json
import setuptools

kwargs = json.loads(
    """
{
    "name": "cdk-grafana-json-dashboard-handler",
    "version": "0.0.0",
    "description": "cdk-grafana-json-dashboard-handler",
    "license": "Apache-2.0",
    "url": "https://github.com/gbvanrenswoude/cdk-grafana-json-dashboard-handler.git",
    "long_description_content_type": "text/markdown",
    "author": "gbvanrenswoude<gbvanrenswoude@gmail.com>",
    "bdist_wheel": {
        "universal": true
    },
    "project_urls": {
        "Source": "https://github.com/gbvanrenswoude/cdk-grafana-json-dashboard-handler.git"
    },
    "package_dir": {
        "": "src"
    },
    "packages": [
        "cdk-grafana-json-dashboard-handler",
        "cdk-grafana-json-dashboard-handler._jsii"
    ],
    "package_data": {
        "cdk-grafana-json-dashboard-handler._jsii": [
            "cdk-grafana-json-dashboard-handler@0.0.0.jsii.tgz"
        ],
        "cdk-grafana-json-dashboard-handler": [
            "py.typed"
        ]
    },
    "python_requires": ">=3.6",
    "install_requires": [
        "aws-cdk.aws-ec2>=1.134.0, <2.0.0",
        "aws-cdk.aws-iam>=1.134.0, <2.0.0",
        "aws-cdk.aws-kms>=1.134.0, <2.0.0",
        "aws-cdk.aws-lambda>=1.134.0, <2.0.0",
        "aws-cdk.aws-logs>=1.134.0, <2.0.0",
        "aws-cdk.aws-s3-deployment>=1.134.0, <2.0.0",
        "aws-cdk.aws-s3>=1.134.0, <2.0.0",
        "aws-cdk.aws-secretsmanager>=1.134.0, <2.0.0",
        "aws-cdk.aws-ssm>=1.134.0, <2.0.0",
        "aws-cdk.core>=1.134.0, <2.0.0",
        "constructs>=3.2.27, <4.0.0",
        "jsii>=1.46.0, <2.0.0",
        "publication>=0.0.3"
    ],
    "classifiers": [
        "Intended Audience :: Developers",
        "Operating System :: OS Independent",
        "Programming Language :: JavaScript",
        "Programming Language :: Python :: 3 :: Only",
        "Programming Language :: Python :: 3.6",
        "Programming Language :: Python :: 3.7",
        "Programming Language :: Python :: 3.8",
        "Programming Language :: Python :: 3.9",
        "Typing :: Typed",
        "Development Status :: 5 - Production/Stable",
        "License :: OSI Approved"
    ],
    "scripts": []
}
"""
)

with open("README.md", encoding="utf8") as fp:
    kwargs["long_description"] = fp.read()


setuptools.setup(**kwargs)
