...
class Converters:
    ...
    def create_repository_provider_response(repo: dict) -> RepositoryProviderResponse:
        ...
        return RepositoryProviderResponse(
            ...
            repositories=[Converters.create_account_repository_response(r) for r in repo["repositories"]],
            errors=[Error(e) for e in repo["errors"]],
        )
    ...
    def create_account_repository_response(repo: dict) -> AccountRepositoryResponse:
        ...
        """
        return AccountRepositoryResponse(
            repository_id=repo.get("id", None),
            name=repo.get("name", None),
            provider_name=repo.get("provider_name", None),
            allow_sharing=repo.get("allow_sharing", None),
            open_access=repo.get("open_access", None),
            account_id=repo.get("account_id", None),
            is_valid=repo.get("is_valid", None),
            repository_url=repo.get("repository_url", None),
            connection_details=repo.get("connection_details", None),
            errors=[Error(e) for e in repo["errors"]],
            repository_relationships=
            [Converters.create_repository_relationship_response(r) for r in
             repo.get("repository_relationships") or repo.get("repo_relationships") or []]
        )
