...
BOOST_AUTO_TEST_SUITE(ftdc_basic_suite)
    ...
    BOOST_AUTO_TEST_CASE(numpy_lists_lazy) {
        ...
        std::vector<Dataset::MetricsPtr> metricList = parser->getMetric(ftdc_metric_keys);
    ...
    BOOST_AUTO_TEST_CASE(numpy_matrix_lazy) {
        ...
        //
        size_t stride = 0;
        Dataset::MetricsPtr matrix = parser->getMetricMatrix(ftdc_metric_keys, &stride);
        Dataset::MetricsPtr ts = parser->getMetric("start");
    ...
    BOOST_AUTO_TEST_CASE(dump_json) {     //
        // Create parser
        auto parser = new FTDCParser();
        ...
        parser->dumpDocsAsJson(fileList[0], "output.json" );
