...
mock_json_port = arg('MOCK_JSON_PORT', 8080)
mock_xml_port = arg('MOCK_XML_PORT', 8090)

service_json_host = arg('SERVICE_JSON_HOST', 'http://localhost')
service_json_port = arg('SERVICE_JSON_PORT', 8080)

service_xml_host = arg('SERVICE_XML_HOST', 'http://localhost')
service_xml_port = arg('SERVICE_XML_PORT', 8090)
'''
...
def test_positive_json():
    ...
    with allure.step("Валидация ответа сервиса mock_json"):
        ...
        tmp_response_json.set_body_from_file(f'{config.base_path}templates/response_service_xml.xml')
        assert response.code == 200
