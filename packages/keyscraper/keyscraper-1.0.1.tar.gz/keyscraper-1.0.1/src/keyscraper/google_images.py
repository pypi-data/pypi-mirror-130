...
def main(page):
    ...
    droptions = DriverOptions(mode = DriverOptions.MODE_FIREFOX, path = "./../../../../geckodriver.exe", window = True)
    ...
    dscraper.scrape(start = 1, pages = 1, perPage = 20)
def download():
    global linkList
    length = len(linkList)
    for index, link in enumerate(linkList):
        sys.stdout.write("/r/tDownloading {index + 1} of {length}...")
        try:
