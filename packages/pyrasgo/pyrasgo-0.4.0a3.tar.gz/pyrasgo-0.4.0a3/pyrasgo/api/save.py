...
from pyrasgo.primitives import Feature, FeatureList, DataSource, Dataset
...
class Save():
    ...
    def dataset(self, dataset: Dataset,
        ...
        if dataset._api_dataset:
            raise APIError(f"This Dataset already exists in Rasgo. [Dataset(id={dataset._api_dataset.id}]")

        operation_set = dataset._api_operation_set if dataset._api_operation_set else self.create._operation_set(
            dataset._operations, dataset._dataset_dependencies)

        output_table_id = operation_set.operations[-1].dw_table_id

        dataset = self.create._dataset(name=name, description=description,
                                       status='published', dw_table_id=output_table_id, dw_operation_set_id=operation_set.id)

        return Dataset(api_dataset=dataset, api_operation_set=operation_set)
