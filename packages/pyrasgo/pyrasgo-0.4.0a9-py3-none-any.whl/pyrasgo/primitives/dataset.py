...
class Dataset(Connection):
    ...
    def __init__(self,
        ...
        if transforms:
            ...
            try:
                self._available_transforms = Get().transforms()
            except Exception as e:
                self._available_transforms = []
        ...
        for transform in self._available_transforms: 
            f = lambda name=transform.name, **kwargs: self.transform(name, **kwargs)
            f.__doc__ = json.dumps(transform, default=lambda o: o.__dict__, sort_keys=True, indent=4)
