...
class Publish():
    ...
    def table(self,
        ...
        if fqtn.count(".") != 2:
            raise ValueError(
                f"'{fqtn}' is not a valid fully qualified table name. FQTNs should follow the format DATABASE.SCHEMA.TABLE_NAME.  Please pass a valid FQTN and try again")

        table_database, table_schema, table_name = fqtn.split('.')

        try:
            src_table = self.data_warehouse.get_source_table(
                table_name=table_name, database=table_database, schema=table_schema, record_limit=10)
            if src_table.empty:
        ...
        self.data_warehouse.grant_table_access(
            table=table_name, role=self.data_warehouse.reader_role, database=table_database, schema=table_schema)
    ...
    def df(self,
        ...
        params:
            ...
            trigger_stats: Should Rasgo trigger a stats run after creation [NOT IMPLEMENTED]
        return:
            Rasgo Dataset
        """
        _org_defaults = self.api._get_default_namespace()
        ...
        return self.table(fqtn=fqtn, name=name, description=description)
