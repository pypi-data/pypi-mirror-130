...
class SingularDetector:
    ...
    def __init__(self):
        ...
        with open(self.dir_path + "resource/exceptions.txt") as f:
            tmp = [x.strip() for x in f.readlines()]
            self.exceptions_word = tmp
    ...
    def singular_checker(self, word, word_pos=None):
        ...
        """
        # some singular words doesn't track with rules so should add them to exceptions list.
        if word in self.exceptions_word:
            return True
