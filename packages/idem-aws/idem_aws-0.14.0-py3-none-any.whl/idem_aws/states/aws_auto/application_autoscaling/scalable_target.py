...
hub.exec.boto3.client.application_autoscaling.deregister_scalable_target
hub.exec.boto3.client.application_autoscaling.describe_scalable_targets
hub.exec.boto3.client.application_autoscaling.register_scalable_target
"""
...
async def present(hub, ctx, name: Text, service_namespace: Text, resource_id: Text, scalable_dimension: Text, min_capacity: int = None, max_capacity: int = None, role_arn: Text = None, suspended_state: Dict = None)  -> Dict[str, Any]:
    r'''
    **Autogenerated function**
    ...
    Args:
        ...
        resource_id(Text): The identifier of the resource that is associated with the scalable target. This string consists
        ...
        scalable_dimension(Text): The scalable dimension associated with the scalable target. This string consists of the service
            ...
            count of an ECS service.    ec2:spot-fleet-request:TargetCapacity - The target capacity of a
            Spot Fleet request.    elasticmapreduce:instancegroup:InstanceCount - The instance count of an
            EMR Instance Group.    appstream:fleet:DesiredCapacity - The desired capacity of an AppStream
            2.0 fleet.    dynamodb:table:ReadCapacityUnits - The provisioned read capacity for a DynamoDB
            table.    dynamodb:table:WriteCapacityUnits - The provisioned write capacity for a DynamoDB
            table.    dynamodb:index:ReadCapacityUnits - The provisioned read capacity for a DynamoDB global
            secondary index.    dynamodb:index:WriteCapacityUnits - The provisioned write capacity for a
            DynamoDB global secondary index.    rds:cluster:ReadReplicaCount - The count of Aurora Replicas
            in an Aurora DB cluster. Available for Aurora MySQL-compatible edition and Aurora PostgreSQL-
        ...
        suspended_state(Dict, optional): An embedded object that contains attributes and attribute values that are used to suspend and
    ...
    Examples:
        ...
        .. code-block:: sls
            ...
            resource_is_present:
              aws_auto.application_autoscaling.scalable_target.present:
                - name: value
    ...
    ret = await hub.exec.boto3.client.application_autoscaling.scalable_target.id(
        ctx,
        jmes_path=name
    )
    if ret["status"]:
    ...
    before = await hub.exec.boto3.client.application_autoscaling.describe_scalable_targets(name)

    if before:
        result["comment"] = f"'{name}' already exists"
    else:
        try:
            ret = await hub.exec.boto3.client.application_autoscaling.register_scalable_target(
                ctx,
                
                
                **{"ServiceNamespace": service_namespace, "ResourceId": resource_id, "ScalableDimension": scalable_dimension, "MinCapacity": min_capacity, "MaxCapacity": max_capacity, "RoleARN": role_arn, "SuspendedState": suspended_state}
            )
    ...
    after = await hub.exec.boto3.client.application_autoscaling.describe_scalable_targets(name)
    result["changes"] = differ.deep_diff(before, after)
    return result

async def absent(hub, ctx, name: Text, service_namespace: Text, resource_id: Text, scalable_dimension: Text)  -> Dict[str, Any]:
    r'''
    **Autogenerated function**
    ...
    Examples:
        ...
        .. code-block:: sls
            ...
            resource_is_absent:
              aws_auto.application_autoscaling.scalable_target.absent:
                - name: value
                - service_namespace: value
                - resource_id: value
                - scalable_dimension: value
    ...
    if not before:
        ...
        try:
            ret = await hub.exec.boto3.client.application_autoscaling.deregister_scalable_target(
                ctx,
                
                
                **{"ServiceNamespace": service_namespace, "ResourceId": resource_id, "ScalableDimension": scalable_dimension}
            )
            result["result"] = ret["status"]
            if not result["result"]:
                result["comment"] = ret["comment"]
