...
class ExtinctionToAttenuatedBackscatter:
    ...
    def simulate_attenuated_backscatter(self, ds: xr.Dataset):
        ...
        for i, t in enumerate(time):
            ...
            attenuated_backscatter.append(np.asarray([total_backscatter[j] * (transmission[j]**1) for j, z in enumerate(altitude)]))
        # convert list to np array
        attenuated_backscatter = np.asarray(attenuated_backscatter)

        print(type(attenuated_backscatter))
        # add attenuated_backscatter as new dataarray
        ds["attenuated_backscatter_0"] = xr.DataArray(
            data=attenuated_backscatter * 1e-6, #conversion from m-1.sr-1 to µm-1.sr-1
            dims=["time", "altitude"],
