...
def check_condition(condition, defined_variables):
    ...
    # Special handling if condition contains "path"
    if "path" in condition:

        # First of all get the data which will be parsed for Json Path
        attribute_value = _get_variable_real_value(defined_variables, name)

        # Get the value from json
        path = condition['path']
        jsonpath_expr = parse(path)
        value = [match.value for match in jsonpath_expr.find(attribute_value)]

        # Workaround - sometime the data is a list, if it is returned as list, pick its first value
        list_check = isinstance(value, list)
        if list_check:
            if len(value) > 0:
                value = value[0]
            else:
...
def _get_variable_real_value(defined_variables, name):
