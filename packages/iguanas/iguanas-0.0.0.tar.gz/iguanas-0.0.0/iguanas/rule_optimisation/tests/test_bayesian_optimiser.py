...
def test_fit(_create_data, _instantiate, _expected_results, _expected_X_rules_mean):
    ...
    with pytest.warns(UserWarning,
        ...
        np.testing.assert_array_almost_equal(
            X_rules.mean().values, exp_X_rules.values)
...
def test_transform(_instantiate):
    ...
    exp_rule_descriptions = pd.DataFrame(
        data=np.array([
            ['Rule', 1.0, 1.0, 0.3333333333333333, 1.0, "(X['A'] > 1)", 1]]
        ),
        columns=['Rule', 'Precision', 'Recall', 'PercDataFlagged', 'Metric', 'Logic', 'nConditions'])
    exp_rule_descriptions.set_index('Rule', inplace=True)
    exp_rule_descriptions = exp_rule_descriptions.astype({
        'Precision': float, 'Recall': float, 'PercDataFlagged': float,
        'Metric': float, 'nConditions': int}
    )
    ro.rule_strings = {
        'Rule': "(X['A'] > 1)",
    }
    ...
    np.testing.assert_array_equal(
        ro.rule_descriptions, exp_rule_descriptions.values)
...
def test_optimise_rule_thresholds(_expected_results, _create_data,
                                  _create_inputs, _instantiate):
