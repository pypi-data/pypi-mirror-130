...
def test_return_rules_with_zero_var_features(_instantiate, _create_inputs):
    ...
    with pytest.warns(UserWarning, match="Rules `zero_var` have all zero variance features based on the dataset `X` - unable to optimise these rules"):
        rule_names_zero_var_features = ro._return_rules_with_zero_var_features(
            lambda_kwargs=lambda_kwargs, all_space_funcs=all_space_funcs,
...
def test_return_orig_rule_if_better_perf(_instantiate):
    ...
    orig_rule_descriptions = pd.DataFrame([
        ['Rule1', 0.5],
        ['Rule2', 0.5],
        ['Rule3', 0.5],
    ], columns=['Rule', 'Metric']
    )
    orig_rule_descriptions.set_index('Rule', inplace=True)
    opt_rule_descriptions = pd.DataFrame([
        ['Rule1', 0.3],
        ['Rule2', 0.5],
        ['Rule3', 0.7],
    ], columns=['Rule', 'Metric']
    )
    opt_rule_descriptions.set_index('Rule', inplace=True)
    ...
    expected_opt_rule_descriptions = pd.DataFrame([
        ['Rule1', 0.5],
        ['Rule2', 0.5],
        ['Rule3', 0.7],
    ], columns=['Rule', 'Metric']
    )
    expected_opt_rule_descriptions.set_index('Rule', inplace=True)
    ...
    opt_rule_strings, opt_rule_performances, opt_rule_descriptions, opt_X_rules = ro._return_orig_rule_if_better_perf(
        orig_rule_performances=orig_rule_performances,
        opt_rule_performances=opt_rule_performances,
        orig_rule_strings=orig_rule_strings,
        opt_rule_strings=opt_rule_strings,
        orig_rule_descriptions=orig_rule_descriptions,
        opt_rule_descriptions=opt_rule_descriptions,
    ...
    assert all(opt_rule_descriptions == expected_opt_rule_descriptions)
