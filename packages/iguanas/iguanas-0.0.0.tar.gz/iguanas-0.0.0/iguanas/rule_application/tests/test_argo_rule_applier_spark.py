...
def create_data():
    ...
    X = X.astype(float)
    weights = y.transform(lambda x: 1000 if x == 1 else 1)
    X, y, weights = ks.from_pandas(
        X), ks.from_pandas(y), ks.from_pandas(weights)
    return [X, y, weights]
...
def create_X_rules(create_data):
    ...
    X_rules = ks.from_pandas(X_rules)
    return X_rules
...
def return_exp_results():
    ...
    X_rules_sums = ks.Series({'Rule2': 503,
                              'Rule3': 584,
                              'Rule1': 71,
                              'Rule4': 424,
                              'Rule5': 529,
...
def test_transform(create_data, ara_instantiated,
                   return_exp_results):
    X, y, _ = create_data
    ...
    assert all(X_rules.sum().sort_index().to_numpy() ==
               exp_X_rules_sums.sort_index().to_numpy())


def test_transform_weighted(create_data, ara_instantiated,
                            return_exp_results):
...
def test_transform_unlabelled(create_data, ara_instantiated, return_exp_results):
    X, _, _ = create_data
