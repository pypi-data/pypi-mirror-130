...
class RuleGeneratorDT(Rules, _BaseGenerator):
    """
    ...
    verbose : int, optional 
        ...
        extraction of the rules from the trees; >1 : also shows the 
        progress of the training of the individual trees in the ensemble 
        model. Defaults to 0.
    ...
    def __init__(self, metric: Callable,
        ...
        self.tree_ensemble.verbose = verbose
        Rules.__init__(self, rule_strings={}, metric=self.metric)

    def __repr__(self):
        if self.rule_strings:
            return f'RuleGeneratorDT object with {len(self.rule_strings)} rules generated'
        else:
            return f'RuleGeneratorDT(metric={self.metric}, n_total_conditions={self.tree_ensemble.max_depth}, tree_ensemble={self.tree_ensemble}, precision_threshold={self.precision_threshold}, num_cores={self.num_cores}, target_feat_corr_types={self.target_feat_corr_types})'
    ...
    def _extract_rules_from_ensemble(self, X: PandasDataFrameType, y: PandasSeriesType,
        ...
        X_rules = self.transform(X=X, y=y, sample_weight=sample_weight)
        return X_rules
