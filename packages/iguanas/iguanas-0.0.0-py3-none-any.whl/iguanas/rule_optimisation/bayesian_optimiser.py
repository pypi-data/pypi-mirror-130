    ...
    **args : tuple , optional
    ...
    def __repr__(self):
        if self.rule_strings == {}:
            return f'RuleOptimiser object with {len(self.orig_rule_lambdas)} rules to optimise'
        else:
            return f'RuleOptimiser object with {len(self.rule_strings)} rules optimised'
    ...
    def fit(self, X: PandasDataFrameType, y=None, sample_weight=None) -> PandasDataFrameType:
        ...
        self.orig_rule_performances = optimisable_rules.rule_descriptions['Metric'].to_dict(
        )
        if self.verbose > 0:
        ...
        self.opt_rule_performances = opt_rules.rule_descriptions['Metric'].to_dict(
        )
        self.rule_strings, self.opt_rule_performances, self.rule_descriptions, X_rules = self._return_orig_rule_if_better_perf(
