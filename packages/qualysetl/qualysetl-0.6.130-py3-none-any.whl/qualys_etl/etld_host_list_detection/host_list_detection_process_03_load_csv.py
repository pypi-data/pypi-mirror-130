...
def write_one_host_with_one_detection_record(csv_headers, writer):
    ...
    for hdr in csv_headers:
        if hdr in host_list_detection_item.keys():
            csv_row[hdr] = prepare_csv_cell(host_list_detection_item[hdr], hdr,
                                            etld_lib_config.host_list_detection_present_csv_cell_as_json)
            csv_row[hdr], truncated_field_list = etld_lib_functions.truncate_csv_cell(
                max_length=etld_lib_config.host_list_detection_csv_truncate_cell_limit,
                csv_cell=host_list_detection_item[hdr],
...
def host_list_detection_to_csv():
    ...
    counter_obj = \
        etld_lib_functions.DisplayCounterToLog(display_counter_at=10000,
                                               ...
                                               display_counter_log_message="count host list detection records written to csv")
