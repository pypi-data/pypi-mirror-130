...
class PInt(TypeMeta):
	def __init__(self, sort=0, text="", default=0, editable=True, visible=True, func=None, tip="", group=None, min=None, max=None, step=1):
		...
		:param tip: 分组名名称，属性面板将按此名称进行分组。注意：仅支持第一级结点分组。
		:param min: 最小值
		:param max: 最大值
		:param step: 步进量
		"""
...
class PCustom(TypeMeta):
	def __init__(self, sort=0, text="", default=None, editable=True, visible=True, func=None, tip="", group=None, editAttribute="", **kwargs):
		# type: (int, str, Any, bool, bool, Callable[[float], Dict[str, Any]], str, str, str, Any) -> None
		"""
		自定义类型
		:param sort: 排列顺序
		:param text: 标题文本
