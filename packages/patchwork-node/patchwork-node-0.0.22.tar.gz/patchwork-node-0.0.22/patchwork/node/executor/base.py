...
class BaseExecutor(Module):
    ...
    async def _handle_task_result(self, task: Task, receiver: AsyncSubscriber, processing_fut: asyncio.Future):
        ...
        if processing_fut.cancelled():
            self.logger.info(f"CANCELLED: Task '{task.task_type} (uuid={task.uuid})' execution cancelled internally")
            return
        elif exc is not None:
            self.logger.warning(f"ERROR: Task '{task.task_type} (uuid={task.uuid})' execution failed with exception {exc.__class__.__name__}({exc})")
            ...
            self.logger.info(f"DONE: Task '{task.task_type} (uuid={task.uuid})' processing completed")
