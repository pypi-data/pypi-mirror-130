...
class Config(object):
	...
	def init(self):
		...
		if not os.path.isfile(path):
			path = os.path.realpath(os.path.join(my_dir, "config.json"))
			print "use default config %s" % path
	...
	def setup_mouse_key(self):
		...
		def on_mouse_key(key, mouse_key, key_event):
			...
			if key_event.event_type == keyboard.KEY_DOWN:
				log("mouse press", mouse_key)
				mouse.press(mouse_key)
			if key_event.event_type == keyboard.KEY_UP:
				log("mouse release", mouse_key)
				mouse.release(mouse_key)
...
def init():
	...
	parser.add_argument("config", type=str, default="", help="config file path", nargs='?')
