...
relic_data, set_data = build_json_files()
    ...
    for relic in relics:
        for drop in relic_data[relic]['Intact']['drops']:
            if get_set_price(drop) >= min_price:
                plat_list.append([drop, get_price(drop)])
            else:
                ducat_list.append([drop, get_ducats(drop)])
...
def simulate_relic(relics, offcycle_relics, refinement, offcycle_refinement, style, amount, drop_priority=None):
    reward_list = []
    reward_screen = []
    offcycle_drops = []
