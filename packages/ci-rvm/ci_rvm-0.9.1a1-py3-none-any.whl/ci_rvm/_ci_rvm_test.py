...
try:
    from .ci_rvm import find_CI_bound, CounterFun
    from .ci_rvm_mpinv import find_profile_CI_bound as find_profile_CI_bound_mpinv
except ImportError:
    try:
        from ci_rvm.ci_rvm import find_CI_bound, CounterFun
...
def find_CI(tester, index, direction, method="RVM", f_track=False):
    ...
    try:
        if method == "RVM":
            res = find_CI_bound(tester.MLE, f, g, h, index, direction, 
                                         nmax=200, apprxtol=0.5,
