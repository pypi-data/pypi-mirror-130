...
import csv
from .Citybrain_Spider import gl
...
def login(username, password):
    ...
    resp = requests.post("%s%s" % (gl.global_dict['CURRENT_SSO_DOMAIN'], "/token"), json=data)
    data = resp.json()
...
def osslist(token):
    ...
    url = "%s%s" % (gl.global_dict['CURRENT_DOMAIN'], "/api/cloudP/oss/list")
    print("url: %s" % url)
...
def bucket(id, path, token):
    ...
    url = "%s%s" % (gl.global_dict['CURRENT_DOMAIN'], "/api/dataNode/oss/bucket")
    print("url: %s" % url)
...
def library(instance_id, token):
    ...
    url = "%s%s%s%s" % (gl.global_dict['CURRENT_DOMAIN'], "/api/cloudP/library/list?instance_id=", instance_id, "&page=1&page_size=100")
    print("url: %s" % url)
    resp = requests.get(url, headers=headers)
    data = resp.json()
    return data['data']
...
def check_table_name(library_id, table_name, token):
    ...
    url = "%s%s%s%s%s" % (
        gl.global_dict['CURRENT_DOMAIN'], "/api/dataNode/check/tableName?library_id=", library_id, "&t_name=", table_name)
    print("url: %s" % url)
    resp = requests.get(url, headers=headers)
    data = resp.json()
    return data['data']
...
def build(build_param):
    ...
    url = "%s%s" % (gl.global_dict['CURRENT_DOMAIN'], "/api/dataNode/build")
    print("url: %s" % url)
    resp = requests.post(url, json=data, headers=headers)
    data = resp.json()
    return data['data']
...
def resource(product, token):
    ...
    url = "%s%s%s%s" % (gl.global_dict['CURRENT_DOMAIN'], "/api/dataNode/apply/resource/list?produce_type=", product,
...
def main():
    gl.init()
    try:
        opts, args = getopt.getopt(sys.argv[1:], "f:", ["env="])  # 短选项模式
    except:
    ...
    for opt, arg in opts:
        ...
        if opt in ['-f']:
            ...
            if arg == 'prod':
                gl.global_dict['CURRENT_SSO_DOMAIN'] = gl.global_dict['PRODUCTION_SSO_DOMAIN']
                print('CURRENT_SSO_DOMAIN: %s' % gl.global_dict['CURRENT_SSO_DOMAIN'])
                gl.global_dict['CURRENT_DOMAIN'] = gl.global_dict['PRODUCTION_DOMAIN']
                print('CURRENT_DOMAIN: %s' % gl.global_dict['CURRENT_DOMAIN'])
    crawl(path)
    while True:
        menu()
