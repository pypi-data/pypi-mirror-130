...
def detail():
    ...
    dbName = request.args.to_dict().get("dbName")
    login_url = request.args.to_dict().get("login_url")
    if not dbName:
        dbName = types
    if not login_url:
    ...
    code = int(res.get(ip))
    if int(res.get(ip)) == 0:
        res.set(dbName, Cookies().taobao_cookies(user.get(types), login_url))
        print(res.get(dbName))
        return {
            "code": code,
            "data": res.get(dbName).decode('utf-8'),
        }
    else:
        return {
            "code": code,
