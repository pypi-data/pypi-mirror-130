...
def makeSongResponse(song,lyrics:bool=False):
    return {
    'id': song['id'],
    'songName':song['song'],
    ...
    'lyrics': _asyncFetch.lyrics(song['id']) if song['has_lyrics'] == "true" and lyrics else None,
...
def makeAlbumSearchResponse(data):
    return [{
            'id': i['id'],
            'albumName': i['title'],
            'music': i['music'],
...
def makePlaylistResponse(data,lyrics:bool=False):
    return {
        'id': data['listid'],
