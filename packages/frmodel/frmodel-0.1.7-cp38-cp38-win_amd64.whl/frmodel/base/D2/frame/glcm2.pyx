...
cdef class CyGLCM:
    cdef public np.int64_t radius, diameter, D2, step_size, bins, invalid_value
    ...
    def __init__(self, np.ndarray[double, ndim=3] ar,
        ...
        self.features = np.zeros([<np.int64_t> ar.shape[0] - (step_size + radius) * 2,
                                  <np.int64_t> ar.shape[1] - (step_size + radius) * 2,
                                  ar.shape[2], 5],
                                 dtype=np.double)
    ...
    def create_glcm(self):
        ...
        cdef np.ndarray[np.int64_t, ndim=3] ar_bin = self._binarize(ar)

        # This is the number of channels of the array
        # E.g. if RGB, then 3.
        cdef np.int64_t chs = <np.int64_t>ar_bin.shape[2]
    ...
    def _populate_glcm(self,
                       np.ndarray[np.int64_t, ndim=4] windows_i,
                       np.ndarray[np.int64_t, ndim=4] windows_j,
        ...
        :return:
        """
        cdef np.int64_t wrs = <np.int64_t>windows_i.shape[0]
        cdef np.int64_t wcs = <np.int64_t>windows_i.shape[1]
        cdef np.int64_t wr = 0;
        cdef np.int64_t wc = 0;
    ...
    cdef _populate_glcm_single(self,
                              np.ndarray[np.int64_t, ndim=2] window_i,
                              np.ndarray[np.int64_t, ndim=2] window_j,
    ...
    @cython.boundscheck(True)
    def _paired_windows(self, np.ndarray[np.int64_t, ndim=2] ar):
