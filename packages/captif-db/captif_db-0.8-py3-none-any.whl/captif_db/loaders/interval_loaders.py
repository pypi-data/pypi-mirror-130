...
class TextureProfilerLoader(StationIntervalLoader):
    ...
    def load_raw(self):
        results = []
        for pp in self.path.glob("*.dat"):
            reading = captif_slp.Reading.from_file(
                pp,
            ...
            rr = reading.mpd(include_meta=True)

            del rr["distance_m"]
            rr["mpd"] = rr.pop("mean")
            rr["station_no"] = rr.pop("file_number")
            rr["trace"] = reading.trace[["distance_mm", "relative_height_mm"]].to_dict(
                "records"
            )
