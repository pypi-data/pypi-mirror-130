...
class TestAiDriver():
    def __init__(self, driver, api_key, server_url=None, test_case_name=None, debug=False, use_classifier_during_creation=False):
        self.debug = debug
        self.driver = driver
    ...
    def find_element(self, by='id', value=None, element_name=None):
        ...
        try:
            ...
            if driver_element and self.debug == True:
        ...
        if self.test_case_creation_mode and find_by_elem==True:
            ...
            if element_box:
                ...
                element = testai_elem(real_elem.parent, real_elem, element_box, self.driver,
                                      self.multiplier)
                return element, self.last_test_case_screenshot_uuid
            else:
                print('Waiting for bounding box of element {} to be drawn in the UI {}'.format(element_name, self.url + '/test_case/label/' + self.test_case_uuid))
                while True:
                    element_box = self._test_case_get_box(element_name)
