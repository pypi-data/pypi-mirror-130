...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=NziQT(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not NziQO
   return result
  except NziQo:
   props=self.props
   ...
   if parts[0]in candidates and NziQR(parts)>1:
