...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or SftzM
 @Sftzb
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return Sftzx(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @SftzX
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if SftzO(origins,SftzV)else origins.get("Items")
  origins_to_create=Sftzc([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if SftzO(aliases,SftzV)else aliases.get("Items")
  aliases_to_create=Sftzc(aliases)
  for distr in distrs:
   origins=Sftzc([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=Sftzc(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",Sftzg)
   config["Enabled"]=SftzU(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if SftzO(custom_errors,Sftzv)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=SftzU(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":Sftzg}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=SftzU(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=SftzM):
    parent=config if parent is SftzM else parent
    items=parent.get(key)or[]
    if SftzO(items,Sftzv)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":SftzR(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if SftzO(cache_behavs,Sftzv)else cache_behavs)
