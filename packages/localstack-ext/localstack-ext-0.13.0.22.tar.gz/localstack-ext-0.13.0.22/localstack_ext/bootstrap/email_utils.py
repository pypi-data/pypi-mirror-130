...
SENT_EMAILS=[]
EMAIL_BLACKLIST=afCzg()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",afCzk(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=afCzj,from_name=afCzj,smtp_host=afCzj,smtp_user=afCzj,smtp_pass=afCzj,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if afCzE(recipients,afCzX)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=afCzj):
 ...
 result["From"]=formataddr((afCzk(Header(from_name,"utf-8")),from_email))
 return result
