...
def install_package(packages:Union[List[XclEv],XclEv],api_name:XclEv):
 if not(is_debian()and in_docker()):
  ...
  return
 if XclEC(packages,XclEv):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=XclEW)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=XclEW)
def check_or_install(command:XclEv,packages:Union[List[XclEv],XclEv],api:XclEv):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:XclEv,target_file:XclEv,timeout:int=XclEp):
 def do_download(url,print_error=XclEW):
  try:
