...
class PodInfo:
 def __init__(self,name=JnTfC,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(JnTfb):
 BACKEND="_none_"
 def __init__(self,pod_name=JnTfC,config=JnTfC):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise JnTfA("Not implemented")
 def delete(self,remote:JnTfd)->JnTfd:
  raise JnTfA("Not implemented")
 def push(self,comment:JnTfE=JnTfC,three_way:JnTfd=JnTfr)->PodInfo:
  raise JnTfA("Not implemented")
 def push_overwrite(self,version:JnTfi,comment:JnTfE=JnTfC):
  pass
 def pull(self,inject_version_state:JnTfd=JnTfr,reset_state_before:JnTfd=JnTfr,fetch_all:JnTfd=JnTfC):
  raise JnTfA("Not implemented")
 def commit(self,message:JnTfE=JnTfC):
  raise JnTfA("Not implemented")
 def inject(self,version:JnTfi,reset_state:JnTfd):
  raise JnTfA("Not implemented")
 def list_versions(self)->List[JnTfE]:
  raise JnTfA("Not implemented")
 def version_info(self,version:JnTfi):
  raise JnTfA("Not implemented")
 def version_metamodel(self,version:JnTfi):
  raise JnTfA("Not implemented")
 def set_version(self,version:JnTfi,inject_version_state:JnTfd,reset_state:JnTfd,commit_before:JnTfd):
  raise JnTfA("Not implemented")
 def list_version_commits(self,version:JnTfi)->List[JnTfE]:
  raise JnTfA("Not implemented")
 def get_commit_diff(self,version:JnTfi,commit:JnTfi)->JnTfE:
  raise JnTfA("Not implemented")
 def register_remote(self,pod_name:JnTfE,ci_pod:JnTfd)->JnTfd:
  raise JnTfA("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->JnTfd:
  raise JnTfA("Not implemented")
 def list_pods(self,fetch_remote:JnTfd)->Set[JnTfE]:
  raise JnTfA("Not implemented")
 ...
 def get(cls,pod_name,pre_config=JnTfC):
  ...
  raise JnTfA('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=JnTfr
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=JnTfP)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=JnTfP
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise JnTfA("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @JnTfV
 def get_state_zip_from_instance(get_content=JnTfr):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise JnTfA("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:JnTfE=JnTfC):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @JnTfV
 def parse_pod_name_from_qualifying_name(qualifying_name:JnTfE)->JnTfE:
 ...
 @JnTfV
 def _get_max_version_for_pod_from_platform(pod_name:JnTfE,auth_headers):
  ...
  remote_max_ver=JnTfi(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with JnTfj(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with JnTfj(meta_archive,"rb")as meta_archive_content:
   ...
   with JnTfj(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=JnTfC)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:JnTfE):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:JnTfd)->JnTfd:
  ...
  if remote:
   pass
  return JnTfr
 def _push_to_remote(self,url:JnTfE):
 ...
 def push_overwrite(self,version:JnTfi,comment:JnTfE=JnTfC):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return JnTfP
 def pull(self,inject_version_state:JnTfd=JnTfr,reset_state_before:JnTfd=JnTfr,fetch_all:JnTfd=JnTfP):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=JnTfr):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(JnTfm(lambda ver:JnTfE(ver),JnTfD(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:JnTfi,reset_state:JnTfd):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=JnTfP,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def get_commit_diff(self,version:JnTfi,commit:JnTfi)->JnTfE:
  ...
  return commit_diff
 def register_remote(self,pod_name:JnTfE,ci_pod:JnTfd=JnTfr)->JnTfd:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:JnTfd)->Set[JnTfE]:
  ...
  return result
 @JnTfK
 def upload_content(cls,presigned_url:JnTfE,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise JnTfA("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @JnTfV
 def create_platform_url(request:JnTfE)->JnTfE:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:JnTfE=JnTfC,three_way:JnTfd=JnTfr)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=JnTfP)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(JnTfR(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=JnTfR(zip_data_content)
  return result
 def pull(self,inject_version_state:JnTfd=JnTfr,reset_state_before:JnTfd=JnTfr,fetch_all:JnTfd=JnTfC):
  ...
  rm_rf(zip_path)
 @JnTfV
 def presigned_url(pod_name:JnTfE,mode:JnTfE)->JnTfE:
  ...
  if response.status_code>=400:
   raise JnTfA("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise JnTfA("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:JnTfE=JnTfC,three_way:JnTfd=JnTfr):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=JnTfP)
 ...
 def pull(self,inject_version_state:JnTfd=JnTfr,reset_state_before:JnTfd=JnTfr,fetch_all:JnTfd=JnTfC):
  ...
  if remote_ref not in remote_refs:
   raise JnTfA('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=JnTfk(self,"clone_dir",JnTfC)
...
class PodConfigManagerMeta(JnTfY):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=JnTfC
   for manager in cls.CHAIN:
    try:
     tmp=JnTfk(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif JnTfW(tmp,JnTfM)and JnTfW(result,JnTfM):
       result.extend(tmp)
    except JnTfA:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not JnTfC:
    return result
   raise JnTfA('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(JnTfb,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @JnTfK
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise JnTfA('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(JnTfb):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=JnTfP).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=JnTfF([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=JnTfr):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except JnTfA:
   if safe:
    return{}
   raise JnTfA('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(JnTfb):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise JnTfA("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise JnTfA("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:JnTfE,remote:JnTfd,pre_config:Dict[JnTfE,JnTfE])->JnTfd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],**kwargs)->JnTfd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",JnTfr))
 return result
def rename_pod(current_pod_name:JnTfE,new_pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:JnTfd,pre_config:Dict[JnTfE,JnTfE],**kwargs)->List[JnTfE]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],message:JnTfE=JnTfC,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:JnTfE,version:JnTfi,reset_state:JnTfd,pre_config:Dict[JnTfE,JnTfE],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],**kwargs)->List[JnTfE]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:JnTfi,pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:JnTfi,pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:JnTfi,inject_version_state:JnTfd,reset_state:JnTfd,commit_before:JnTfd,pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE],**kwargs)->JnTfd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:JnTfi,pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE])->List[JnTfE]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:JnTfi,commit:JnTfi,pod_name:JnTfE,pre_config:Dict[JnTfE,JnTfE])->JnTfE:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:JnTfi,pod_name:JnTfE,comment:JnTfE,pre_config:Dict[JnTfE,JnTfE]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=JnTfC,squash_commits=JnTfr,comment=JnTfC,three_way=JnTfr,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=JnTfr,reset_state_before=JnTfr,**kwargs):
 pre_config=kwargs.get("pre_config",JnTfC)
 if not pod_name:
  raise JnTfA("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=JnTfr,exclude_from_reset:List[JnTfE]=JnTfC):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise JnTfA("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->JnTfE:
 try:
...
def get_persisted_resource_names(data_dir)->List[JnTfE]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and JnTfR(os.listdir(entry.path))>0:
    names.append(entry.name)
