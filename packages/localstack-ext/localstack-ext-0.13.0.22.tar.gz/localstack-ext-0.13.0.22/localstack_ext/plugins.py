...
def api_key_configured()->qYjHv:
 return qYjHg if os.environ.get("LOCALSTACK_API_KEY")else qYjHR
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if qYjHI([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
