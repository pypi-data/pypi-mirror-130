...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or DUfry
 @DUfrq
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return DUfrK(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @DUfrh
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if DUfrY(origins,DUfri)else origins.get("Items")
  origins_to_create=DUfrj([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if DUfrY(aliases,DUfri)else aliases.get("Items")
  aliases_to_create=DUfrj(aliases)
  for distr in distrs:
   origins=DUfrj([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=DUfrj(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",DUfrG)
   config["Enabled"]=DUfrk(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if DUfrY(custom_errors,DUfrl)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=DUfrk(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":DUfrG}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=DUfrk(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=DUfry):
    parent=config if parent is DUfry else parent
    items=parent.get(key)or[]
    if DUfrY(items,DUfrl)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":DUfru(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if DUfrY(cache_behavs,DUfrl)else cache_behavs)
