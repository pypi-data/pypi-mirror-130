...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[jvscO])[0]
 def get_physical_resource_id(self,attribute=jvscO,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if jvscn(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return jvscS(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @jvscp
 def add_defaults(resource,stack_name:jvscI):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return jvscS(SESReceiptRule,self).get_cfn_attribute(attribute)
 @jvsca
