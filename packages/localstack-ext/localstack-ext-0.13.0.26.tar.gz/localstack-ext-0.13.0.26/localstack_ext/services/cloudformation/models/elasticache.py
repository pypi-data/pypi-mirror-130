...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=qhAYV(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not qhAYu
   return result
  except qhAYR:
   props=self.props
   ...
   if parts[0]in candidates and qhAYp(parts)>1:
