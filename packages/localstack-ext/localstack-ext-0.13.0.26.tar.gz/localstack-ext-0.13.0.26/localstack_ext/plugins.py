...
def api_key_configured()->LarBQ:
 return LarBP if os.environ.get("LOCALSTACK_API_KEY")else LarBK
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if LarBY([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
