...
def install_package(packages:Union[List[XFkjA],XFkjA],api_name:XFkjA):
 if not(is_debian()and in_docker()):
  ...
  return
 if XFkjc(packages,XFkjA):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=XFkjE)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=XFkjE)
def check_or_install(command:XFkjA,packages:Union[List[XFkjA],XFkjA],api:XFkjA):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:XFkjA,target_file:XFkjA,timeout:int=XFkjq):
 def do_download(url,print_error=XFkjE):
  try:
