...
class DecryptionHandler:
 decryption_key:ObhBK
 def __init__(self,decryption_key:ObhBK):
  self.decryption_key=decryption_key
 def decrypt(self,content)->ObhBK:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=ObhBT):
  if path and not ObhBM(path,ObhBC):
   path=ObhBC(ObhBd(path,"_path",[]))
  if not path:
   return ObhBT
...
def patch_traceback_lines():
 if ObhBd(traceback.FrameSummary,"_ls_patch_applied",ObhBT):
  return
 @ObhBk
 def line(self):
  try:
 ...
 ObhBj(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=ObhBX
def patch_inspect_findsource():
 if ObhBd(inspect,"_ls_patch_applied",ObhBT):
  return
 ...
 inspect._ls_patch_applied=ObhBX
