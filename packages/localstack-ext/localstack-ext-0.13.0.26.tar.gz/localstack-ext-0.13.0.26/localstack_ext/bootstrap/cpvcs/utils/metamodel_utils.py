...
CAPTURE_METAMODEL_SEPARATELY=pMbBW
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=pMbBc(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=pMbBc(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->pMbBL:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[pMbBL,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,pMbBr)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(pMbBr,state_files_to)
...
class CommitMetamodelUtils:
 @pMbBm
 def create_metadata_archive(cls,version:Version,delete_reference:bool=pMbBW,overwrite:bool=pMbBW,metamodels_file:pMbBL=pMbBr):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @pMbBm
 def create_delta_metamodel_file(cls,version:pMbBi,revision:pMbBi,store_to_zip=pMbBW)->Tuple[Dict,Dict]:
 ...
 @pMbBm
 def reconstruct_metamodel(cls,version:pMbBi,revision:pMbBi)->Dict:
  result={}
  for rev in pMbBX(1,revision+1):
  ...
  return result
 @pMbBm
 def get_version_metamodel(cls,version:pMbBi,revision:pMbBi=pMbBr)->Dict:
 ...
 @pMbBm
 def get_commit_diff(cls,version_no:pMbBi,commit_no:pMbBi)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->pMbBo:
 if pMbBQ(clazz,pMbBY):
  return{}
 constructor=pMbBT(clazz,"__init__",pMbBr)
...
def _create_metamodel_helper(obj,width=25,visited:set=pMbBr):
 if obj is pMbBr:
  return obj
