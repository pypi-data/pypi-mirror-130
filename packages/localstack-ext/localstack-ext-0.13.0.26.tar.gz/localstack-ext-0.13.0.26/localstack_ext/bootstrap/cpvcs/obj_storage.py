...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=idnzN
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:idnzS)->idnzS:
  pass
 @idnzw
 def get(cls,requested_file_locator:idnzS):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=idnzN
 @idnzw
 def get(cls,state_file_locator:idnzS,requested_storage:idnzS,serializers:Dict[idnzS,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[idnzS,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:idnzS)->idnzS:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:idnzS)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:idnzS)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:idnzS)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:idnzS)->idnzS:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[idnzS]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:idnzS,new_key:idnzS,referenced_by_version:idnzS=idnzN):
  pass
 @abc.abstractmethod
 def version_exists(self,key:idnzS)->idnzC:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:idnzS,key:idnzS):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:idnzS,new_key:idnzS,base_path:idnzS)->idnzC:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[idnzS]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[idnzS,idnzS],remote_location:idnzS=idnzN)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[idnzS]:
  ...
  for cpvcs_obj in args:
   if idnzA(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif idnzA(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[idnzS,idnzS],remote_location:idnzS=idnzN)->List[Union[Revision,Version]]:
