...
class PodInfo:
 def __init__(self,name=BUOJG,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(BUOJN):
 BACKEND="_none_"
 def __init__(self,pod_name=BUOJG,config=BUOJG):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise BUOJw("Not implemented")
 def delete(self,remote:BUOJd)->BUOJd:
  raise BUOJw("Not implemented")
 def push(self,comment:BUOJz=BUOJG,three_way:BUOJd=BUOJu)->PodInfo:
  raise BUOJw("Not implemented")
 def push_overwrite(self,version:BUOJf,comment:BUOJz=BUOJG):
  pass
 def pull(self,inject_version_state:BUOJd=BUOJu,reset_state_before:BUOJd=BUOJu,fetch_all:BUOJd=BUOJG):
  raise BUOJw("Not implemented")
 def commit(self,message:BUOJz=BUOJG):
  raise BUOJw("Not implemented")
 def inject(self,version:BUOJf,reset_state:BUOJd):
  raise BUOJw("Not implemented")
 def list_versions(self)->List[BUOJz]:
  raise BUOJw("Not implemented")
 def version_info(self,version:BUOJf):
  raise BUOJw("Not implemented")
 def version_metamodel(self,version:BUOJf):
  raise BUOJw("Not implemented")
 def set_version(self,version:BUOJf,inject_version_state:BUOJd,reset_state:BUOJd,commit_before:BUOJd):
  raise BUOJw("Not implemented")
 def list_version_commits(self,version:BUOJf)->List[BUOJz]:
  raise BUOJw("Not implemented")
 def get_commit_diff(self,version:BUOJf,commit:BUOJf)->Dict:
  raise BUOJw("Not implemented")
 def register_remote(self,pod_name:BUOJz,ci_pod:BUOJd)->BUOJd:
  raise BUOJw("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->BUOJd:
  raise BUOJw("Not implemented")
 def list_pods(self,fetch_remote:BUOJd)->Set[BUOJz]:
  raise BUOJw("Not implemented")
 ...
 def get(cls,pod_name,pre_config=BUOJG):
  ...
  raise BUOJw('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=BUOJu
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=BUOJD)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=BUOJD
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise BUOJw("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @BUOJb
 def get_state_zip_from_instance(get_content=BUOJu):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise BUOJw("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:BUOJz=BUOJG):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @BUOJb
 def parse_pod_name_from_qualifying_name(qualifying_name:BUOJz)->BUOJz:
 ...
 @BUOJb
 def _get_max_version_for_pod_from_platform(pod_name:BUOJz,auth_headers):
  ...
  remote_max_ver=BUOJf(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with BUOJA(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with BUOJA(meta_archive,"rb")as meta_archive_content:
   ...
   with BUOJA(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=BUOJG)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:BUOJz):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:BUOJd)->BUOJd:
  ...
  if remote:
   pass
  return BUOJu
 def _push_to_remote(self,url:BUOJz):
 ...
 def push_overwrite(self,version:BUOJf,comment:BUOJz=BUOJG):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return BUOJD
 def pull(self,inject_version_state:BUOJd=BUOJu,reset_state_before:BUOJd=BUOJu,fetch_all:BUOJd=BUOJD):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=BUOJu):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(BUOJI(lambda ver:BUOJz(ver),BUOJP(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:BUOJf,reset_state:BUOJd):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=BUOJD,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_info(self,version:BUOJf):
  ...
  return version_info
 def version_metamodel(self,version:BUOJf)->BUOJz:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
  if version==-1:
   version=cpvcs_api.get_max_version_no()
 ...
 def get_commit_diff(self,version:BUOJf,commit:BUOJf)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:BUOJz,ci_pod:BUOJd=BUOJu)->BUOJd:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:BUOJd)->Set[BUOJz]:
  ...
  return result
 @BUOJY
 def upload_content(cls,presigned_url:BUOJz,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise BUOJw("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @BUOJb
 def create_platform_url(request:BUOJz)->BUOJz:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:BUOJz=BUOJG,three_way:BUOJd=BUOJu)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=BUOJD)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(BUOJW(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=BUOJW(zip_data_content)
  return result
 def pull(self,inject_version_state:BUOJd=BUOJu,reset_state_before:BUOJd=BUOJu,fetch_all:BUOJd=BUOJG):
  ...
  rm_rf(zip_path)
 @BUOJb
 def presigned_url(pod_name:BUOJz,mode:BUOJz)->BUOJz:
  ...
  if response.status_code>=400:
   raise BUOJw("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise BUOJw("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:BUOJz=BUOJG,three_way:BUOJd=BUOJu):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=BUOJD)
 ...
 def pull(self,inject_version_state:BUOJd=BUOJu,reset_state_before:BUOJd=BUOJu,fetch_all:BUOJd=BUOJG):
  ...
  if remote_ref not in remote_refs:
   raise BUOJw('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=BUOJt(self,"clone_dir",BUOJG)
...
class PodConfigManagerMeta(BUOJn):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=BUOJG
   for manager in cls.CHAIN:
    try:
     tmp=BUOJt(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif BUOJE(tmp,BUOJH)and BUOJE(result,BUOJH):
       result.extend(tmp)
    except BUOJw:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not BUOJG:
    return result
   raise BUOJw('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(BUOJN,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @BUOJY
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise BUOJw('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(BUOJN):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=BUOJD).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=BUOJc([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=BUOJu):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except BUOJw:
   if safe:
    return{}
   raise BUOJw('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(BUOJN):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise BUOJw("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise BUOJw("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:BUOJz,remote:BUOJd,pre_config:Dict[BUOJz,BUOJz])->BUOJd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],**kwargs)->BUOJd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",BUOJu))
 return result
def rename_pod(current_pod_name:BUOJz,new_pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:BUOJd,pre_config:Dict[BUOJz,BUOJz],**kwargs)->List[BUOJz]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],message:BUOJz=BUOJG,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:BUOJz,version:BUOJf,reset_state:BUOJd,pre_config:Dict[BUOJz,BUOJz],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],**kwargs)->List[BUOJz]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:BUOJf,pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:BUOJf,pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:BUOJf,inject_version_state:BUOJd,reset_state:BUOJd,commit_before:BUOJd,pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz],**kwargs)->BUOJd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:BUOJf,pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz])->List[BUOJz]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:BUOJf,commit:BUOJf,pod_name:BUOJz,pre_config:Dict[BUOJz,BUOJz])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:BUOJf,pod_name:BUOJz,comment:BUOJz,pre_config:Dict[BUOJz,BUOJz]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=BUOJG,squash_commits=BUOJu,comment=BUOJG,three_way=BUOJu,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=BUOJu,reset_state_before=BUOJu,**kwargs):
 pre_config=kwargs.get("pre_config",BUOJG)
 if not pod_name:
  raise BUOJw("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=BUOJu,exclude_from_reset:List[BUOJz]=BUOJG):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise BUOJw("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->BUOJz:
 try:
...
def get_persisted_resource_names(data_dir)->List[BUOJz]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and BUOJW(os.listdir(entry.path))>0:
    names.append(entry.name)
