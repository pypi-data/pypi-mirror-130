...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qwUIO()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qwUIG(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qwUIH,from_name=qwUIH,smtp_host=qwUIH,smtp_user=qwUIH,smtp_pass=qwUIH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qwUIJ(recipients,qwUIS)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qwUIH):
 ...
 result["From"]=formataddr((qwUIG(Header(from_name,"utf-8")),from_email))
 return result
