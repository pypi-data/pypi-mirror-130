...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return oRMxb if e else oRMxA
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->oRMxH:
...
@click.option("--revert",is_flag=oRMxb,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:oRMxF):
...
def _cpvcs_initialized(pod_name:oRMxE)->oRMxF:
...
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_init(name:oRMxE):
...
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=oRMxb,default=oRMxA)
def cmd_cpvcs_delete(name:oRMxE,remote:oRMxF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_register_remote(name:oRMxE):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=oRMxb)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=oRMxb)
def cmd_cpvcs_rename(name:oRMxE,new_name:oRMxE):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_commit(message:oRMxE,name:oRMxE):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=oRMxb,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=oRMxb,default=oRMxA,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_push(squash:oRMxF,message:oRMxE,name:oRMxE,three_way:oRMxF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
@click.option("-v","--version",type=oRMxD)
@click.option("-m","--message",required=oRMxA)
def cmd_cpvcs_push_overwrite(version:oRMxD,message:oRMxE,name:oRMxE):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=oRMxD,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=oRMxb,default=oRMxA,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_inject(version:oRMxD,reset:oRMxF,name:oRMxE):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=oRMxb,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=oRMxb,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:oRMxE,inject:oRMxF,reset:oRMxF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=oRMxb,default=oRMxA)
def cmd_cpvcs_list_pods(remote:oRMxF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_versions(name:oRMxE):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=oRMxb,type=oRMxD)
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_version_info(version:oRMxD,name:oRMxE):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=oRMxD,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_version_metamodel(version:oRMxD,name:oRMxE):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=oRMxb,type=oRMxD,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=oRMxb,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=oRMxb,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=oRMxA,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_set_version(version:oRMxD,inject:oRMxF,reset:oRMxF,commit_before:oRMxF,name:oRMxE):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_commits(version:oRMxD,name:oRMxE):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=oRMxb)
@click.option("--commit","-c",required=oRMxb)
@click.option("-n","--name",help="Name of the cloud pod",required=oRMxb)
def cmd_cpvcs_commit_diff(version:oRMxD,commit:oRMxD,name:oRMxE):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(oRMxH)
  status.stop()
...
def print_table(columns:Dict[oRMxE,oRMxE],rows:List[Dict[oRMxE,Any]],formatters:Dict[oRMxE,Callable[[Any],oRMxE]]=oRMxH):
 from rich.table import Table
 if formatters is oRMxH:
  formatters=oRMxC()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  oRMxX(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->oRMxH:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->oRMxH:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->oRMxH:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->oRMxH:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
