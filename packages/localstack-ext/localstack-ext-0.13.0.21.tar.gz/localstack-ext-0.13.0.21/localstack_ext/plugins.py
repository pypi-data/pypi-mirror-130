...
def api_key_configured()->DYTxR:
 return DYTxH if os.environ.get("LOCALSTACK_API_KEY")else DYTxe
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if DYTxt([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
