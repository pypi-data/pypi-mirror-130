...
def random_hash()->xlWCn:
 return xlWCq(random.getrandbits(160))
def compute_file_hash(file_path:xlWCn,accum=xlWCJ)->Optional[xlWCn]:
 try:
  with xlWCH(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:xlWCp,state_archive:xlWCn)->xlWCn:
 ...
 m.update(xlWCn(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->xlWCn:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=xlWCT(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with xlWCH(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except xlWCM as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if xlWCR(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(xlWCn(cpvcs_node.revision_number).encode("utf-8"))
 elif xlWCR(cpvcs_node,Version):
  m.update(xlWCn(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
