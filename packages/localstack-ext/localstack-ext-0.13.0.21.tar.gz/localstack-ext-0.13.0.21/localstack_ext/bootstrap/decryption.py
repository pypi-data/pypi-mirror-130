...
class DecryptionHandler:
 decryption_key:hJpUi
 def __init__(self,decryption_key:hJpUi):
  self.decryption_key=decryption_key
 def decrypt(self,content)->hJpUi:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=hJpUE):
  if path and not hJpUb(path,hJpUQ):
   path=hJpUQ(hJpUD(path,"_path",[]))
  if not path:
   return hJpUE
...
def patch_traceback_lines():
 if hJpUD(traceback.FrameSummary,"_ls_patch_applied",hJpUE):
  return
 @hJpUA
 def line(self):
  try:
 ...
 hJpUe(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=hJpUl
def patch_inspect_findsource():
 if hJpUD(inspect,"_ls_patch_applied",hJpUE):
  return
 ...
 inspect._ls_patch_applied=hJpUl
