...
SENT_EMAILS=[]
EMAIL_BLACKLIST=GLhbN()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",GLhbA(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=GLhbn,from_name=GLhbn,smtp_host=GLhbn,smtp_user=GLhbn,smtp_pass=GLhbn,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if GLhbF(recipients,GLhbP)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=GLhbn):
 ...
 result["From"]=formataddr((GLhbA(Header(from_name,"utf-8")),from_email))
 return result
