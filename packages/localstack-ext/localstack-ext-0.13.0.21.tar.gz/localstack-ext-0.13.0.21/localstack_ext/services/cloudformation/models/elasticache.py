...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=hWvSC(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not hWvSG
   return result
  except hWvSy:
   props=self.props
   ...
   if parts[0]in candidates and hWvSj(parts)>1:
