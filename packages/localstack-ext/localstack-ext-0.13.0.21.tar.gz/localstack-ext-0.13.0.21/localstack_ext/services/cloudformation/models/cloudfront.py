...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or spVzB
 @spVzx
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return spVzW(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @spVzU
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if spVzy(origins,spVzb)else origins.get("Items")
  origins_to_create=spVzj([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if spVzy(aliases,spVzb)else aliases.get("Items")
  aliases_to_create=spVzj(aliases)
  for distr in distrs:
   origins=spVzj([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=spVzj(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",spVzY)
   config["Enabled"]=spVzr(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if spVzy(custom_errors,spVzd)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=spVzr(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":spVzY}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=spVzr(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=spVzB):
    parent=config if parent is spVzB else parent
    items=parent.get(key)or[]
    if spVzy(items,spVzd)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":spVzQ(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if spVzy(cache_behavs,spVzd)else cache_behavs)
