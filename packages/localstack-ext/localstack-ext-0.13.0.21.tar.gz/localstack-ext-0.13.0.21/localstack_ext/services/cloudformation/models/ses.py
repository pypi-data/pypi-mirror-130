...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[tYzea])[0]
 def get_physical_resource_id(self,attribute=tYzea,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if tYzeK(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return tYzeJ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @tYzeH
 def add_defaults(resource,stack_name:tYzeu):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return tYzeJ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @tYzeN
