...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return xTYvs if e else xTYvU
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->xTYvk:
...
@click.option("--revert",is_flag=xTYvs,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:xTYvW):
...
def _cpvcs_initialized(pod_name:xTYvH)->xTYvW:
...
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_init(name:xTYvH):
...
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=xTYvs,default=xTYvU)
def cmd_cpvcs_delete(name:xTYvH,remote:xTYvW):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_register_remote(name:xTYvH):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=xTYvs)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=xTYvs)
def cmd_cpvcs_rename(name:xTYvH,new_name:xTYvH):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_commit(message:xTYvH,name:xTYvH):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=xTYvs,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=xTYvs,default=xTYvU,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_push(squash:xTYvW,message:xTYvH,name:xTYvH,three_way:xTYvW):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
@click.option("-v","--version",type=xTYvh)
@click.option("-m","--message",required=xTYvU)
def cmd_cpvcs_push_overwrite(version:xTYvh,message:xTYvH,name:xTYvH):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=xTYvh,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=xTYvs,default=xTYvU,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_inject(version:xTYvh,reset:xTYvW,name:xTYvH):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=xTYvs,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=xTYvs,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:xTYvH,inject:xTYvW,reset:xTYvW):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=xTYvs,default=xTYvU)
def cmd_cpvcs_list_pods(remote:xTYvW):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_versions(name:xTYvH):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=xTYvs,type=xTYvh)
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_version_info(version:xTYvh,name:xTYvH):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=xTYvh,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_version_metamodel(version:xTYvh,name:xTYvH):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=xTYvs,type=xTYvh,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=xTYvs,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=xTYvs,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=xTYvU,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_set_version(version:xTYvh,inject:xTYvW,reset:xTYvW,commit_before:xTYvW,name:xTYvH):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_commits(version:xTYvh,name:xTYvH):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=xTYvs)
@click.option("--commit","-c",required=xTYvs)
@click.option("-n","--name",help="Name of the cloud pod",required=xTYvs)
def cmd_cpvcs_commit_diff(version:xTYvh,commit:xTYvh,name:xTYvH):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(xTYvk)
  status.stop()
...
def print_table(columns:Dict[xTYvH,xTYvH],rows:List[Dict[xTYvH,Any]],formatters:Dict[xTYvH,Callable[[Any],xTYvH]]=xTYvk):
 from rich.table import Table
 if formatters is xTYvk:
  formatters=xTYvA()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  xTYvl(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->xTYvk:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->xTYvk:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->xTYvk:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->xTYvk:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
