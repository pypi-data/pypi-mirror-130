...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return VvQMW if e else VvQMn
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->VvQMd:
...
@click.option("--revert",is_flag=VvQMW,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:VvQMB):
...
def _cpvcs_initialized(pod_name:VvQMS)->VvQMB:
...
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_init(name:VvQMS):
...
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=VvQMW,default=VvQMn)
def cmd_cpvcs_delete(name:VvQMS,remote:VvQMB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_register_remote(name:VvQMS):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=VvQMW)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=VvQMW)
def cmd_cpvcs_rename(name:VvQMS,new_name:VvQMS):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_commit(message:VvQMS,name:VvQMS):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=VvQMW,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=VvQMW,default=VvQMn,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_push(squash:VvQMB,message:VvQMS,name:VvQMS,three_way:VvQMB):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
@click.option("-v","--version",type=VvQMH)
@click.option("-m","--message",required=VvQMn)
def cmd_cpvcs_push_overwrite(version:VvQMH,message:VvQMS,name:VvQMS):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=VvQMH,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=VvQMW,default=VvQMn,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_inject(version:VvQMH,reset:VvQMB,name:VvQMS):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=VvQMW,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=VvQMW,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:VvQMS,inject:VvQMB,reset:VvQMB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=VvQMW,default=VvQMn)
def cmd_cpvcs_list_pods(remote:VvQMB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_versions(name:VvQMS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=VvQMW,type=VvQMH)
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_version_info(version:VvQMH,name:VvQMS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=VvQMH,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_version_metamodel(version:VvQMH,name:VvQMS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=VvQMW,type=VvQMH,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=VvQMW,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=VvQMW,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=VvQMn,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_set_version(version:VvQMH,inject:VvQMB,reset:VvQMB,commit_before:VvQMB,name:VvQMS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_commits(version:VvQMH,name:VvQMS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=VvQMW)
@click.option("--commit","-c",required=VvQMW)
@click.option("-n","--name",help="Name of the cloud pod",required=VvQMW)
def cmd_cpvcs_commit_diff(version:VvQMH,commit:VvQMH,name:VvQMS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(VvQMd)
  status.stop()
...
def print_table(columns:Dict[VvQMS,VvQMS],rows:List[Dict[VvQMS,Any]],formatters:Dict[VvQMS,Callable[[Any],VvQMS]]=VvQMd):
 from rich.table import Table
 if formatters is VvQMd:
  formatters=VvQMa()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  VvQMr(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->VvQMd:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->VvQMd:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->VvQMd:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->VvQMd:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
