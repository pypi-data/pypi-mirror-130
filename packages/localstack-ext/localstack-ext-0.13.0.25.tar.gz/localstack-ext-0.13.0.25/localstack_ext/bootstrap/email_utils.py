...
SENT_EMAILS=[]
EMAIL_BLACKLIST=BbgFa()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",BbgFf(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=BbgFs,from_name=BbgFs,smtp_host=BbgFs,smtp_user=BbgFs,smtp_pass=BbgFs,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if BbgFA(recipients,BbgFi)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=BbgFs):
 ...
 result["From"]=formataddr((BbgFf(Header(from_name,"utf-8")),from_email))
 return result
