...
class DecryptionHandler:
 decryption_key:FHYen
 def __init__(self,decryption_key:FHYen):
  self.decryption_key=decryption_key
 def decrypt(self,content)->FHYen:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=FHYeO):
  if path and not FHYeX(path,FHYev):
   path=FHYev(FHYeu(path,"_path",[]))
  if not path:
   return FHYeO
...
def patch_traceback_lines():
 if FHYeu(traceback.FrameSummary,"_ls_patch_applied",FHYeO):
  return
 @FHYeJ
 def line(self):
  try:
 ...
 FHYef(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=FHYex
def patch_inspect_findsource():
 if FHYeu(inspect,"_ls_patch_applied",FHYeO):
  return
 ...
 inspect._ls_patch_applied=FHYex
