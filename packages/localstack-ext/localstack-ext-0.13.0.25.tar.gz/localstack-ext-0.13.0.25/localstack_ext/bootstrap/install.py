...
def install_package(packages:Union[List[VNFzs],VNFzs],api_name:VNFzs):
 if not(is_debian()and in_docker()):
  ...
  return
 if VNFzO(packages,VNFzs):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=VNFza)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=VNFza)
def check_or_install(command:VNFzs,packages:Union[List[VNFzs],VNFzs],api:VNFzs):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:VNFzs,target_file:VNFzs,timeout:int=VNFzL):
 def do_download(url,print_error=VNFza):
  try:
