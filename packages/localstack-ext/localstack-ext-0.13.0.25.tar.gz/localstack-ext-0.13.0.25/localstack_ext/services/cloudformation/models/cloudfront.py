...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or oKInb
 @oKInP
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return oKInv(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @oKInh
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if oKInL(origins,oKInH)else origins.get("Items")
  origins_to_create=oKInN([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if oKInL(aliases,oKInH)else aliases.get("Items")
  aliases_to_create=oKInN(aliases)
  for distr in distrs:
   origins=oKInN([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=oKInN(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",oKInz)
   config["Enabled"]=oKInC(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if oKInL(custom_errors,oKInR)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=oKInC(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":oKInz}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=oKInC(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=oKInb):
    parent=config if parent is oKInb else parent
    items=parent.get(key)or[]
    if oKInL(items,oKInR)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":oKIna(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if oKInL(cache_behavs,oKInR)else cache_behavs)
