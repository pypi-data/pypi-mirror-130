...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=QdSIT(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not QdSIB
   return result
  except QdSIR:
   props=self.props
   ...
   if parts[0]in candidates and QdSIi(parts)>1:
