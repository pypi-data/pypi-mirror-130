...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[deCsF])[0]
 def get_physical_resource_id(self,attribute=deCsF,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if deCsO(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return deCsW(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @deCsi
 def add_defaults(resource,stack_name:deCsT):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return deCsW(SESReceiptRule,self).get_cfn_attribute(attribute)
 @deCsS
