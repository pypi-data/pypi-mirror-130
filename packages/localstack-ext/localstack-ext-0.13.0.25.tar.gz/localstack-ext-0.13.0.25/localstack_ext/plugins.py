...
def api_key_configured()->eHcro:
 return eHcrK if os.environ.get("LOCALSTACK_API_KEY")else eHcrV
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if eHcrt([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
