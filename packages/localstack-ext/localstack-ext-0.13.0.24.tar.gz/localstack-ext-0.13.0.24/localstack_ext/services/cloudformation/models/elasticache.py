...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=SbUkY(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not SbUkn
   return result
  except SbUkm:
   props=self.props
   ...
   if parts[0]in candidates and SbUkl(parts)>1:
