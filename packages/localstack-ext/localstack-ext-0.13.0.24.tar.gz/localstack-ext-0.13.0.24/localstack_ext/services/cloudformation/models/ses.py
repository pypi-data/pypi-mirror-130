...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[TSfAW])[0]
 def get_physical_resource_id(self,attribute=TSfAW,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if TSfAK(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return TSfAc(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @TSfAm
 def add_defaults(resource,stack_name:TSfAp):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return TSfAc(SESReceiptRule,self).get_cfn_attribute(attribute)
 @TSfAq
