...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or YPxjJ
 @YPxjr
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return YPxjN(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @YPxjs
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if YPxjv(origins,YPxjH)else origins.get("Items")
  origins_to_create=YPxjX([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if YPxjv(aliases,YPxjH)else aliases.get("Items")
  aliases_to_create=YPxjX(aliases)
  for distr in distrs:
   origins=YPxjX([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=YPxjX(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",YPxjd)
   config["Enabled"]=YPxjK(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if YPxjv(custom_errors,YPxja)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=YPxjK(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":YPxjd}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=YPxjK(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=YPxjJ):
    parent=config if parent is YPxjJ else parent
    items=parent.get(key)or[]
    if YPxjv(items,YPxja)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":YPxjA(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if YPxjv(cache_behavs,YPxja)else cache_behavs)
