...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return Xitco if e else Xitcq
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->XitcB:
...
@click.option("--revert",is_flag=Xitco,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:Xitcr):
...
def _cpvcs_initialized(pod_name:Xitcy)->Xitcr:
...
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_init(name:Xitcy):
...
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=Xitco,default=Xitcq)
def cmd_cpvcs_delete(name:Xitcy,remote:Xitcr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_register_remote(name:Xitcy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=Xitco)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=Xitco)
def cmd_cpvcs_rename(name:Xitcy,new_name:Xitcy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_commit(message:Xitcy,name:Xitcy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=Xitco,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=Xitco,default=Xitcq,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_push(squash:Xitcr,message:Xitcy,name:Xitcy,three_way:Xitcr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
@click.option("-v","--version",type=Xitcf)
@click.option("-m","--message",required=Xitcq)
def cmd_cpvcs_push_overwrite(version:Xitcf,message:Xitcy,name:Xitcy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=Xitcf,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=Xitco,default=Xitcq,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_inject(version:Xitcf,reset:Xitcr,name:Xitcy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=Xitco,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=Xitco,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:Xitcy,inject:Xitcr,reset:Xitcr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=Xitco,default=Xitcq)
def cmd_cpvcs_list_pods(remote:Xitcr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_versions(name:Xitcy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=Xitco,type=Xitcf)
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_version_info(version:Xitcf,name:Xitcy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=Xitcf,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_version_metamodel(version:Xitcf,name:Xitcy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=Xitco,type=Xitcf,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=Xitco,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=Xitco,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=Xitcq,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_set_version(version:Xitcf,inject:Xitcr,reset:Xitcr,commit_before:Xitcr,name:Xitcy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_commits(version:Xitcf,name:Xitcy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=Xitco)
@click.option("--commit","-c",required=Xitco)
@click.option("-n","--name",help="Name of the cloud pod",required=Xitco)
def cmd_cpvcs_commit_diff(version:Xitcf,commit:Xitcf,name:Xitcy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(XitcB)
  status.stop()
...
def print_table(columns:Dict[Xitcy,Xitcy],rows:List[Dict[Xitcy,Any]],formatters:Dict[Xitcy,Callable[[Any],Xitcy]]=XitcB):
 from rich.table import Table
 if formatters is XitcB:
  formatters=Xitcp()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  Xitck(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->XitcB:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->XitcB:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->XitcB:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->XitcB:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
