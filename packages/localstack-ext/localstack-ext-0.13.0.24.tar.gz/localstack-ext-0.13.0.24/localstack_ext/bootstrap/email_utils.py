...
SENT_EMAILS=[]
EMAIL_BLACKLIST=bRzdx()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",bRzde(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=bRzdD,from_name=bRzdD,smtp_host=bRzdD,smtp_user=bRzdD,smtp_pass=bRzdD,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if bRzds(recipients,bRzdK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=bRzdD):
 ...
 result["From"]=formataddr((bRzde(Header(from_name,"utf-8")),from_email))
 return result
