...
def install_package(packages:Union[List[VeQRx],VeQRx],api_name:VeQRx):
 if not(is_debian()and in_docker()):
  ...
  return
 if VeQRW(packages,VeQRx):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=VeQRn)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=VeQRn)
def check_or_install(command:VeQRx,packages:Union[List[VeQRx],VeQRx],api:VeQRx):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:VeQRx,target_file:VeQRx,timeout:int=VeQRy):
 def do_download(url,print_error=VeQRn):
  try:
