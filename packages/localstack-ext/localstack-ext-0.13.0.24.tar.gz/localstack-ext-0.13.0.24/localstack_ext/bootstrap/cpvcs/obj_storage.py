...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=CbrPW
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:CbrPs)->CbrPs:
  pass
 @CbrPE
 def get(cls,requested_file_locator:CbrPs):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=CbrPW
 @CbrPE
 def get(cls,state_file_locator:CbrPs,requested_storage:CbrPs,serializers:Dict[CbrPs,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[CbrPs,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:CbrPs)->CbrPs:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:CbrPs)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:CbrPs)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:CbrPs)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:CbrPs)->CbrPs:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[CbrPs]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:CbrPs,new_key:CbrPs,referenced_by_version:CbrPs=CbrPW):
  pass
 @abc.abstractmethod
 def version_exists(self,key:CbrPs)->CbrPl:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:CbrPs,key:CbrPs):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:CbrPs,new_key:CbrPs,base_path:CbrPs)->CbrPl:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[CbrPs]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[CbrPs,CbrPs],remote_location:CbrPs=CbrPW)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[CbrPs]:
  ...
  for cpvcs_obj in args:
   if CbrPm(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif CbrPm(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[CbrPs,CbrPs],remote_location:CbrPs=CbrPW)->List[Union[Revision,Version]]:
