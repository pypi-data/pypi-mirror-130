...
class DecryptionHandler:
 decryption_key:MwPhR
 def __init__(self,decryption_key:MwPhR):
  self.decryption_key=decryption_key
 def decrypt(self,content)->MwPhR:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=MwPhE):
  if path and not MwPhL(path,MwPhA):
   path=MwPhA(MwPhF(path,"_path",[]))
  if not path:
   return MwPhE
...
def patch_traceback_lines():
 if MwPhF(traceback.FrameSummary,"_ls_patch_applied",MwPhE):
  return
 @MwPhQ
 def line(self):
  try:
 ...
 MwPhl(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=MwPhD
def patch_inspect_findsource():
 if MwPhF(inspect,"_ls_patch_applied",MwPhE):
  return
 ...
 inspect._ls_patch_applied=MwPhD
