...
class PodInfo:
 def __init__(self,name=tdPIe,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(tdPIm):
 BACKEND="_none_"
 def __init__(self,pod_name=tdPIe,config=tdPIe):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise tdPIf("Not implemented")
 def delete(self,remote:tdPIx)->tdPIx:
  raise tdPIf("Not implemented")
 def push(self,comment:tdPIW=tdPIe,three_way:tdPIx=tdPIs)->PodInfo:
  raise tdPIf("Not implemented")
 def push_overwrite(self,version:tdPIX,comment:tdPIW=tdPIe):
  pass
 def pull(self,inject_version_state:tdPIx=tdPIs,reset_state_before:tdPIx=tdPIs,fetch_all:tdPIx=tdPIe):
  raise tdPIf("Not implemented")
 def commit(self,message:tdPIW=tdPIe):
  raise tdPIf("Not implemented")
 def inject(self,version:tdPIX,reset_state:tdPIx):
  raise tdPIf("Not implemented")
 def list_versions(self)->List[tdPIW]:
  raise tdPIf("Not implemented")
 def version_info(self,version:tdPIX):
  raise tdPIf("Not implemented")
 def version_metamodel(self,version:tdPIX):
  raise tdPIf("Not implemented")
 def set_version(self,version:tdPIX,inject_version_state:tdPIx,reset_state:tdPIx,commit_before:tdPIx):
  raise tdPIf("Not implemented")
 def list_version_commits(self,version:tdPIX)->List[tdPIW]:
  raise tdPIf("Not implemented")
 def get_commit_diff(self,version:tdPIX,commit:tdPIX)->tdPIW:
  raise tdPIf("Not implemented")
 def register_remote(self,pod_name:tdPIW,ci_pod:tdPIx)->tdPIx:
  raise tdPIf("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->tdPIx:
  raise tdPIf("Not implemented")
 def list_pods(self,fetch_remote:tdPIx)->Set[tdPIW]:
  raise tdPIf("Not implemented")
 ...
 def get(cls,pod_name,pre_config=tdPIe):
  ...
  raise tdPIf('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=tdPIs
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=tdPIY)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=tdPIY
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise tdPIf("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @tdPIH
 def get_state_zip_from_instance(get_content=tdPIs):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise tdPIf("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:tdPIW=tdPIe):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @tdPIH
 def parse_pod_name_from_qualifying_name(qualifying_name:tdPIW)->tdPIW:
 ...
 @tdPIH
 def _get_max_version_for_pod_from_platform(pod_name:tdPIW,auth_headers):
  ...
  remote_max_ver=tdPIX(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with tdPIl(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with tdPIl(meta_archive,"rb")as meta_archive_content:
   ...
   with tdPIl(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=tdPIe)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:tdPIW):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:tdPIx)->tdPIx:
  ...
  if remote:
   pass
  return tdPIs
 def _push_to_remote(self,url:tdPIW):
 ...
 def push_overwrite(self,version:tdPIX,comment:tdPIW=tdPIe):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return tdPIY
 def pull(self,inject_version_state:tdPIx=tdPIs,reset_state_before:tdPIx=tdPIs,fetch_all:tdPIx=tdPIY):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=tdPIs):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(tdPIg(lambda ver:tdPIW(ver),tdPIS(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:tdPIX,reset_state:tdPIx):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=tdPIY,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def get_commit_diff(self,version:tdPIX,commit:tdPIX)->tdPIW:
  ...
  return commit_diff
 def register_remote(self,pod_name:tdPIW,ci_pod:tdPIx=tdPIs)->tdPIx:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:tdPIx)->Set[tdPIW]:
  ...
  return result
 @tdPIk
 def upload_content(cls,presigned_url:tdPIW,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise tdPIf("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @tdPIH
 def create_platform_url(request:tdPIW)->tdPIW:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:tdPIW=tdPIe,three_way:tdPIx=tdPIs)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=tdPIY)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(tdPIi(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=tdPIi(zip_data_content)
  return result
 def pull(self,inject_version_state:tdPIx=tdPIs,reset_state_before:tdPIx=tdPIs,fetch_all:tdPIx=tdPIe):
  ...
  rm_rf(zip_path)
 @tdPIH
 def presigned_url(pod_name:tdPIW,mode:tdPIW)->tdPIW:
  ...
  if response.status_code>=400:
   raise tdPIf("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise tdPIf("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:tdPIW=tdPIe,three_way:tdPIx=tdPIs):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=tdPIY)
 ...
 def pull(self,inject_version_state:tdPIx=tdPIs,reset_state_before:tdPIx=tdPIs,fetch_all:tdPIx=tdPIe):
  ...
  if remote_ref not in remote_refs:
   raise tdPIf('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=tdPIo(self,"clone_dir",tdPIe)
...
class PodConfigManagerMeta(tdPIv):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=tdPIe
   for manager in cls.CHAIN:
    try:
     tmp=tdPIo(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif tdPIR(tmp,tdPIn)and tdPIR(result,tdPIn):
       result.extend(tmp)
    except tdPIf:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not tdPIe:
    return result
   raise tdPIf('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(tdPIm,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @tdPIk
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise tdPIf('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(tdPIm):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=tdPIY).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=tdPIc([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=tdPIs):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except tdPIf:
   if safe:
    return{}
   raise tdPIf('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(tdPIm):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise tdPIf("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise tdPIf("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:tdPIW,remote:tdPIx,pre_config:Dict[tdPIW,tdPIW])->tdPIx:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],**kwargs)->tdPIx:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",tdPIs))
 return result
def rename_pod(current_pod_name:tdPIW,new_pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:tdPIx,pre_config:Dict[tdPIW,tdPIW],**kwargs)->List[tdPIW]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],message:tdPIW=tdPIe,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:tdPIW,version:tdPIX,reset_state:tdPIx,pre_config:Dict[tdPIW,tdPIW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],**kwargs)->List[tdPIW]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:tdPIX,pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:tdPIX,pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:tdPIX,inject_version_state:tdPIx,reset_state:tdPIx,commit_before:tdPIx,pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW],**kwargs)->tdPIx:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:tdPIX,pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW])->List[tdPIW]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:tdPIX,commit:tdPIX,pod_name:tdPIW,pre_config:Dict[tdPIW,tdPIW])->tdPIW:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:tdPIX,pod_name:tdPIW,comment:tdPIW,pre_config:Dict[tdPIW,tdPIW]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=tdPIe,squash_commits=tdPIs,comment=tdPIe,three_way=tdPIs,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=tdPIs,reset_state_before=tdPIs,**kwargs):
 pre_config=kwargs.get("pre_config",tdPIe)
 if not pod_name:
  raise tdPIf("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=tdPIs,exclude_from_reset:List[tdPIW]=tdPIe):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise tdPIf("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->tdPIW:
 try:
...
def get_persisted_resource_names(data_dir)->List[tdPIW]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and tdPIi(os.listdir(entry.path))>0:
    names.append(entry.name)
