...
class KeyActivationError(LmWHM):
 pass
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def is_enterprise():
 return read_api_key(raise_if_missing=LmWHi)=="enterprise"
def read_api_key(raise_if_missing=LmWHO):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise LmWHM("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:LmWHQ):
 return '"%s..."(%s)'%(api_key[:3],LmWHs(api_key))
def fetch_key()->LmWHj:
 ...
 if api_key=="test":
  ...
  key_base64=load_cached_key(api_key=api_key,check_timeout=LmWHi)
  decoded_key=base64.b64decode(key_base64)
  return decoded_key
 data={"api_key":api_key,"version":__version__}
 try:
  logging.getLogger("py.warnings").setLevel(logging.ERROR)
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=LmWHi)
  ...
  if log_license_issues():
   api_key=LmWHQ(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 timestamp=LmWHQ(LmWHB(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in LmWHT(LmWHs(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 cache=get_key_cache()
 cache.update({"timestamp":LmWHB(timestamp),"key_hash":md5(api_key),"key":key_b64})
 cache.save()
def load_cached_key(api_key:LmWHQ,check_timeout:bool=LmWHO)->LmWHQ:
 cache=get_key_cache()
 ...
 timestamp=LmWHQ(cache["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:LmWHj):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise LmWHJ("Decryption resulted in invalid python file!")
 except LmWHM:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise LmWHM("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=LmWHi)
def is_logged_in():
 return LmWHO if get_auth_cache().get("token")else LmWHi
def get_auth_headers()->Dict:
 ...
 api_key=read_api_key(raise_if_missing=LmWHi)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise LmWHM("Please log in first")
