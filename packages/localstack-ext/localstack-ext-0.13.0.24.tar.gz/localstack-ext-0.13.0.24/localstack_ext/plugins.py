...
def api_key_configured()->Ktsgy:
 return KtsgC if os.environ.get("LOCALSTACK_API_KEY")else KtsgJ
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if Ktsgm([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
