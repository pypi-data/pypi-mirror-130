...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return rhtwT if e else rhtwi
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->rhtwM:
...
@click.option("--revert",is_flag=rhtwT,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:rhtwa):
...
def _cpvcs_initialized(pod_name:rhtwy)->rhtwa:
...
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_init(name:rhtwy):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=rhtwT,default=rhtwi)
def cmd_cpvcs_delete(name:rhtwy,remote:rhtwa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_register_remote(name:rhtwy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=rhtwT)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=rhtwT)
def cmd_cpvcs_rename(name:rhtwy,new_name:rhtwy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_commit(message:rhtwy,name:rhtwy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=rhtwT,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=rhtwT,default=rhtwi,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_push(squash:rhtwa,message:rhtwy,name:rhtwy,three_way:rhtwa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
@click.option("-v","--version",type=rhtwC)
@click.option("-m","--message",required=rhtwi)
def cmd_cpvcs_push_overwrite(version:rhtwC,message:rhtwy,name:rhtwy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=rhtwC,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=rhtwT,default=rhtwi,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_inject(version:rhtwC,reset:rhtwa,name:rhtwy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=rhtwT,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=rhtwT,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:rhtwy,inject:rhtwa,reset:rhtwa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=rhtwT,default=rhtwi)
def cmd_cpvcs_list_pods(remote:rhtwa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_versions(name:rhtwy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=rhtwT,type=rhtwC)
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_version_info(version:rhtwC,name:rhtwy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=rhtwC,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_version_metamodel(version:rhtwC,name:rhtwy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=rhtwT,type=rhtwC,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=rhtwT,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=rhtwT,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=rhtwi,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_set_version(version:rhtwC,inject:rhtwa,reset:rhtwa,commit_before:rhtwa,name:rhtwy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_commits(version:rhtwC,name:rhtwy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=rhtwT)
@click.option("--commit","-c",required=rhtwT)
@click.option("-n","--name",help="Name of the cloud pod",required=rhtwT)
def cmd_cpvcs_commit_diff(version:rhtwC,commit:rhtwC,name:rhtwy):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(rhtwM)
  status.stop()
...
def print_table(columns:Dict[rhtwy,rhtwy],rows:List[Dict[rhtwy,Any]],formatters:Dict[rhtwy,Callable[[Any],rhtwy]]=rhtwM):
 from rich.table import Table
 if formatters is rhtwM:
  formatters=rhtwL()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  rhtwW(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->rhtwM:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->rhtwM:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->rhtwM:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->rhtwM:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
