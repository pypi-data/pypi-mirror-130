...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=SiHyE,state_files=SiHyr(),version_number=0)
 ...
 with SiHyp(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with SiHyp(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with SiHyp(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:SiHyW,meta_archives:Dict[SiHyW,SiHyW],state_archives:Dict[SiHyW,SiHyW],remote_info:Dict[SiHyW,SiHyW],pod_name:SiHyW):
...
def merge_from_remote(version_space_archive:SiHyW,meta_archives:Dict[SiHyW,SiHyW],state_archives:Dict[SiHyW,SiHyW]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->SiHyb:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:SiHyW):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:SiHyW)->SiHyb:
...
def list_locally_available_pods(show_remote_or_local:SiHyb=SiHyT)->Set[SiHyW]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:SiHyW,file_name:SiHyW,service:SiHyW,region:SiHyW)->SiHyW:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->SiHyW:
 ...
 return key
def _get_state_file_path(key:SiHyW)->SiHyW:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=SiHyr(SiHyQ(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:SiHyW)->Optional[SiHyW]:
...
def get_version_info(version_no:SiHyV)->Dict[SiHyW,SiHyW]:
...
def read_file_from_archive(archive_path:SiHyW,file_name:SiHyW):
 try:
...
def commit(message:SiHyW=SiHyE)->Revision:
 ...
 curr_expansion_point_parent_state_files=SiHyr()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=SiHyE
...
def zip_directories(zip_dest:SiHyW,directories:List[SiHyW])->SiHyW:
 ...
 return zip_dest
def create_version_space_archive()->SiHyW:
...
def get_max_version_no()->SiHyV:
 with SiHyp(config_context.get_max_ver_path())as fp:
  return SiHyV(os.path.basename(fp.readline()))
...
def push_overwrite(version:SiHyV,comment:SiHyW)->SiHyb:
 ...
 _create_metadata_archive(version_node=version_node,overwrite=SiHyT,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return SiHyT
def push(comment:SiHyW=SiHyE,three_way:SiHyb=SiHyj)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=SiHyr(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 return new_version
def get_version_meta_archive(version_no:SiHyV)->SiHyW:
...
def get_version_state_archive(version_no:SiHyV)->SiHyW:
...
def get_version_metamodel(version_no:SiHyV)->SiHyW:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:SiHyb=SiHyj,overwrite:SiHyb=SiHyj,metamodels_file:SiHyW=SiHyE):
 ...
 with SiHyp(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with SiHyp(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:SiHyV,state_file_refs:Set[StateFileRef],delete_files=SiHyj,archive=SiHyT):
...
def set_active_version(version_no:SiHyV,commit_before=SiHyj)->SiHyb:
 ...
 return SiHyj
def _set_active_version(key:SiHyW):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=SiHyr(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:SiHyV)->Version:
 versions=load_version_references()
 version_ref=SiHyJ((version[1]for version in versions if version[0]==version_no),SiHyE)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[SiHyV,SiHyW]]:
 result={}
 with SiHyp(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with SiHyp(symlink,"r")as sp:
    result[SiHyV(os.path.basename(symlink))]=sp.readline()
 return SiHyC(result.items(),key=lambda x:x[0],reverse=SiHyT)
def list_versions()->List[SiHyW]:
 ...
 return result
def list_version_commits(version_no:SiHyV)->List[SiHyW]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:SiHyV,commit_no:SiHyV)->SiHyW:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->SiHyW:
 with SiHyp(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->SiHyW:
 with SiHyp(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->SiHyW:
 with SiHyp(config_context.get_known_ver_path(),"a")as fp:
...
def _is_special_case(obj)->SiHyb:
 return SiHyv(obj,(Queue,FakeBucket))
...
def _merge_two_state_dir_func(**kwargs):
 ...
 if os.path.isfile(dst_state_file_path):
  ...
  mkdir(dst_state_dir)
  persist_object(src_state,dst_state_file_path)
def _merge_expansion_point_with_max(three_way=SiHyj):
 ...
 common_ancestor_state_archive=SiHyE
 tmp_common_ancestor_state_dir=SiHyE
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=SiHyj)
...
def _infer_backend_init(clazz:Type,sf:StateFileRef)->SiHyY:
 if SiHyv(clazz,SiHym):
  return{}
 constructor=SiHyO(clazz,"__init__",SiHyE)
 ...
 if "region" in sig_args.args:
  ...
  backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->SiHyW:
 try:
  ...
  save_file(dest,"{}")
  return key
def _do_create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->SiHyW:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[SiHyW,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,SiHyE)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(SiHyE,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=SiHyz(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=SiHyz(backend1)
