...
class DecryptionHandler:
 decryption_key:Nvdfz
 def __init__(self,decryption_key:Nvdfz):
  self.decryption_key=decryption_key
 def decrypt(self,content)->Nvdfz:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=Nvdfu):
  if path and not Nvdfc(path,NvdfI):
   path=NvdfI(Nvdfp(path,"_path",[]))
  if not path:
   return Nvdfu
...
def patch_traceback_lines():
 if Nvdfp(traceback.FrameSummary,"_ls_patch_applied",Nvdfu):
  return
 @NvdfC
 def line(self):
  try:
 ...
 NvdfR(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=NvdfJ
def patch_inspect_findsource():
 if Nvdfp(inspect,"_ls_patch_applied",Nvdfu):
  return
 ...
 inspect._ls_patch_applied=NvdfJ
