...
SENT_EMAILS=[]
EMAIL_BLACKLIST=zsKfl()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",zsKfM(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=zsKfV,from_name=zsKfV,smtp_host=zsKfV,smtp_user=zsKfV,smtp_pass=zsKfV,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if zsKfI(recipients,zsKfb)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=zsKfV):
 ...
 result["From"]=formataddr((zsKfM(Header(from_name,"utf-8")),from_email))
 return result
