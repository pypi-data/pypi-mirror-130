...
def install_package(packages:Union[List[rtpkI],rtpkI],api_name:rtpkI):
 if not(is_debian()and in_docker()):
  ...
  return
 if rtpkT(packages,rtpkI):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=rtpkL)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=rtpkL)
def check_or_install(command:rtpkI,packages:Union[List[rtpkI],rtpkI],api:rtpkI):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:rtpkI,target_file:rtpkI,timeout:int=rtpki):
 def do_download(url,print_error=rtpkL):
  try:
