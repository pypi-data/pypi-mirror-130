...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or OwjTR
 @OwjTi
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return OwjTJ(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @OwjTG
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if OwjTy(origins,OwjTW)else origins.get("Items")
  origins_to_create=OwjTF([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if OwjTy(aliases,OwjTW)else aliases.get("Items")
  aliases_to_create=OwjTF(aliases)
  for distr in distrs:
   origins=OwjTF([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=OwjTF(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",OwjTQ)
   config["Enabled"]=OwjTf(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if OwjTy(custom_errors,OwjTz)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=OwjTf(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":OwjTQ}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=OwjTf(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=OwjTR):
    parent=config if parent is OwjTR else parent
    items=parent.get(key)or[]
    if OwjTy(items,OwjTz)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":OwjTB(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if OwjTy(cache_behavs,OwjTz)else cache_behavs)
