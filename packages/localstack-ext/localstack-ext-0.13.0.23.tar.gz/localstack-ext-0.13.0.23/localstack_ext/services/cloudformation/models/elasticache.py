...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=ipTwK(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not ipTwX
   return result
  except ipTwP:
   props=self.props
   ...
   if parts[0]in candidates and ipTwn(parts)>1:
