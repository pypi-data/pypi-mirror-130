...
def api_key_configured()->lGoWI:
 return lGoWn if os.environ.get("LOCALSTACK_API_KEY")else lGoWV
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if lGoWk([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
