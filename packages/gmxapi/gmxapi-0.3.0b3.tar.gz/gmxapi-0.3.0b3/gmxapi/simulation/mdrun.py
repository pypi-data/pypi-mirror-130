        ...
        with scoped_communicator(None) as context_comm:
            ...
            if context_comm_size > ensemble_width:
                ...
                if context_rank == 0:
                    ...
                    # The root rank gets stuck at the `allgather(instance)` before
                    # getting to the above send.
...
class SubscriptionPublishingRunner(object):
    ...
    def __call__(self):
        ...
        # output, and how the user interface should represent the different possible
        # behaviors.
        ...
        for name, output_file in (('trajectory', trajectory), ('checkpoint', checkpoint)):
            try:
                ...
                logger.error(f'{name} is {output_file}')
                missing_outputs.append(f'<{name}>')
        ...
        publisher.trajectory = str(trajectory)
        publisher.checkpoint = str(checkpoint)
...
class StandardDirector(gmxapi.abc.OperationDirector):
    ...
    def resource_factory(self,
        ...
        raise gmxapi.exceptions.ValueError('No dispatching from {} context to {}'.format(
            source,
            target))
