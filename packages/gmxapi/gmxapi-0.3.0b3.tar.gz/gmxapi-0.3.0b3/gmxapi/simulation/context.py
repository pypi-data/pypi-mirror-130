    ...
    class Builder(object):
        ...
        def build(self, dag):
            ...
            def launch(rank=None):
                ...
                if isinstance(self.runtime_params, dict):
                    width = 1
                    runtime_params = dict()
                    for param, value in self.runtime_params.items():
                        if isinstance(value, list):
                            if len(value) > rank:
                                raise DataShapeError(f'{value} too wide for ensemble.')
                            if len(value) > 1:
                                if width == 1:
                                    width = len(value)
                                elif width != len(value):
                                    raise DataShapeError(
                                        f'Could not match shape of {value} to ensemble '
                                        f'width {width}')
                            runtime_params[param] = value[rank]
                        runtime_params[param] = value
                    runtime_params = self.runtime_params
                elif isinstance(self.runtime_params, list):
                    runtime_params = self.runtime_params[rank]
                else:
                    raise ApiError(f'Could not get params from {self.runtime_params}')
