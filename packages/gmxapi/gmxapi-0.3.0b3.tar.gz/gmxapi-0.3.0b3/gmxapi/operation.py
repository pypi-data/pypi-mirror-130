...
class OutputData(object):
    ...
    def __init__(self, name: str, description: ResultDescription):
        ...
        self._callbacks = list(list() for _ in range(self._description.width))
        self._reset_count = 0
        OutputData._instance_count += 1
        self._instance = OutputData._instance_count * description.width * comm_size

    def tag(self, *, ensemble_member, id):
        return self._instance + (ensemble_member * comm_size) + id
    ...
    def set(self, value, member: int):
        ...
        try:
            for callback in self._callbacks[member]:
                callback(self)
        except Exception as e:
            logger.exception(
                f'Exception during callbacks for {self}[{self.name}][{member}].',
                exc_info=e
            )
            raise e

    def add_done_callback(self, func: typing.Callable[['OutputData'], None], member: int):
        """Register a callback for when `set` is called.

        Note that this signature is not yet normative. We need to reconcile our notions of
        Future slicing and adopt a more conventional interface where `add_done_callback()`
        takes only a single (callback) argument.
        """
        if func in self._callbacks[member]:
            raise UsageError('Function {func} already registered in {self} callbacks.')
        self._callbacks[member].append(func)
        if self._done[member]:
            try:
                func(self)
            except Exception as e:
                logger.exception(
    ...
    def remove_done_callback(self, func, member: int):
        self._callbacks[member].remove(func)
    ...
    def __init__(self, source_collection: DataSourceCollection, sink_terminal: SinkTerminal):
        ...
        for name in sink_terminal.inputs:
            ...
            if name not in source_collection:
                ...
                # TODO: Dispatch first on sink type, then source type.
...
class ResourceManager(SourceResource[_OutputDataProxyType, _PublishingDataProxyType]):
    ...
    def __publishing_context(self, ensemble_member=0) -> _PublishingDataProxyType:
        ...
        #         logger.debug(
        #             'Callback should now have been called to send results for member '
        #             f'{ensemble_member} for outputs '
        #             ', '.join(name for name in resource))
        #     else:
        #         assert rank_number < comm_size
    ...
    def ensemble_width(self):
        # TODO: Remove __ensemble_width cache after testing.
        if self.__ensemble_width is None:
            self.__ensemble_width = self._input_edge.sink_terminal.ensemble_width
        if self.__ensemble_width != self._input_edge.sink_terminal.ensemble_width:
            warnings.warn(
                'ResourceManager ensemble width has changed '
                f'from {self.__ensemble_width} to {self._input_edge.sink_terminal.ensemble_width}')
            self.__ensemble_width = self._input_edge.sink_terminal.ensemble_width
    ...
    def set_result(self, name, value, member: int):
        ...
        # Note that callbacks are registered with and triggered through the OutputData
        # objects in self._data.
                ...
                for ensemble_member in range(self.ensemble_width):
                    ...
                    with self.local_input(ensemble_member) as input:
                        ...
                        with publishing_resources(ensemble_member=ensemble_member) as output:
                            ...
                            tags = {name: dict() for name in output}
                            # output is a PublishingDataProxy
                            for name in self._data:
                                manager = self._data[name]
                                assert isinstance(manager, OutputData)
                                tags[name] = manager.tag(ensemble_member=ensemble_member,
                                                         id=self.__reset_counter)
                            try:
                                # TODO: The resources should be built with
                                #  consideration for what the operation actually needs
                                #  (see builder proposal above).
                                # TODO: Are we using "communicator"?
                            ...
                            #         logger.debug(f'Getting {key} for {self}[{i}] from broadcast.')
                            #         result = comm.bcast(None, root=0)
                            #         setattr(output, key, result)
                            # End publishing context.
                # if comm_size > 1 and rank_number == 0:
                #     for i in range(self.ensemble_width):
                #         # output is obtained through Publisher, so is equivalent to
                #         # `resource_manager._data.get(key_name).data(member=client_id)`
                #         for key in self._data.keys():
                #             logger.debug(f'Sending {key} for {self}[{i}].')
                #             comm.bcast(self._data.get(key).data(member=i), root=0)
    ...
    def publishing_resources(self):
        ...
        """
        resource_factory_wrapper = self.__publishing_resources.pop()
        ...
        if comm_size > 1 and not self._allow_duplicate:
            ...
            for name, output in self._data.items():
                ...
                for member in range(output.width):
                    cb = get_callback(
                        operation=self.operation_id,
                        output_name=name,
                        ensemble_member=member,
                        instance=self.__reset_counter,
                        owner=owning_rank
                    )
                    output.add_done_callback(
                        func=cb,
                        member=member)
        return resource_factory_wrapper
...
def get_callback(*,
    ...
    """Generate the OutputData callback function.

    Arguments:
        operation: Identifier for the resource manager
        output_name: Named output port
        ensemble_member: element of the output_name data
        instance: Additional unique identifier for this callback instance
        owner: The MPI rank responsible for obtaining the result by executing a task.

    We assume this factory is only called when MPI is active on more than one rank.
    """
    assert comm.Get_size() > 1

    # TODO: Remove after testing.
    instances = comm.allgather(instance)
    assert all(instance == i for i in instances)

    subscribers = [id for id in range(comm_size) if id != owner]
    # The callback will be called at the end of the OutputData.set method.

    def subscription_provider_cb(obj: OutputData):
        tag = obj.tag(ensemble_member=ensemble_member, id=instance)
        result = obj.data(member=ensemble_member)
        logger.debug(f'Sending "{obj.name}" for tag {tag}')
        for subscriber in subscribers:
            comm.send(result, dest=subscriber, tag=tag)

    def subscriber_callback(obj: OutputData):
        # The callback will be called at the end of the OutputData.set method.
        tag = obj.tag(ensemble_member=ensemble_member, id=instance)
        logger.debug(f'Callback for subscriber to "{obj.name}" tag {tag} received '
                     f'{operation}: {output_name}[{ensemble_member}] '
                     f'= {obj.data(member=ensemble_member)} on rank {rank_number}')

    if rank_number == owner:
        callback = subscription_provider_cb
    else:
...
class PyFunctionRunnerResources(collections.UserDict):
    ...
    """
    def __init__(self, communicator=None, tags=(), **kwargs):
        super().__init__(**kwargs)
        self.communicator = communicator
        # Dictionary with the tag from OutputData for each named output.
        self.tags = dict(tags)
...
class SubscriberRunner(PyFunctionRunner):
    """Alternative PyFunctionRunner for ranks where function should not be executed.

    *MUST* collaborate with the publishing resources provided to ranks where execution
    _is_ performed in order to receive copies of the actual output. Output is
    re-published in the local ResourceManager by a proxy function, according to
    *output_description*.
    """
    @classmethod
    def make_runner(
        ...
        def _function(resources: PyFunctionRunnerResources):
            names = ', '.join(resources.keys())
            logger.debug(f'SubscriberRunner received kwargs {names}')
            output = resources.output()
            for name in output_description.keys():
                tag = resources.tags[name]
                logger.debug(f'Waiting for "{name}" tag {tag}')
                result = comm.recv(source=owner, tag=tag)
                logger.debug(f'Received tag {tag}.')
                setattr(output, name, result)

        return cls(function=_function, output_description=output_description)
...
def wrapped_function_runner(
    ...
    assert callable(function)
    signature = inspect.signature(function)
    logger.debug(f'Creating runner for {signature}.')
    ...
    if rank_number != owning_rank and not allow_duplicate:
        ...
        runner = SubscriberRunner.make_runner(
...
class OperationDirector(object):
    ...
    def __init__(self,
                 *args,
                 ...
                 label=None,
                 **kwargs):
        self.operation_details = operation_details  # Can we inspect for requirements here?
    ...
    def __call__(self) -> AbstractOperation:
        ...
        # In the case of a PyFunctionRunner, we have chosen the subclass at the
        # time the dynamically defined OperationDetails subclass is defined in
        # function_wrapper, instead of customizing the runner here. However,
        # this may be an appropriate place to move the "tags" derivation, which we
        # use in the runner (through the resources built by the resource_factory) and in
        # the call-backs (created while calling ResourceManager.publishing_resources),
        # which currently have to figure out the tags on their own. Maybe we can get
        # the callback factory from the operation_details? At the least, we need to
        # direct some logic, so we'll set a *allow_duplicate* property on the
        # ResourceManager itself, for now, using a property on the runner director.
    ...
    def decorator(function) -> typing.Callable:
        ...
        class OperationDetails(OperationDetailsBase):
            ...
            def resource_director(cls, *, input=None,
                ...
                """
                resources = PyFunctionRunnerResources(communicator=communicator,
                                                      tags=tags)
...
def while_loop(*, operation, condition, max_iteration=10):
    ...
    def run_loop(output: OutputCollectionDescription):
        ...
        # FIXME! This doesn't work for Futures.
        # It is essential that we be able at least to apply `logical_not(
        # subgraph.member)` as a condition, but we currently rely on the special
        # behavior of SubgraphBuilder objects (those returned by the `subgraph`
        # command) to provide lambda functions from attribute access to the named
        # variables (yet another reason to convert subgraphs to classes where we can
        # let variables be data descriptors and use them as class methods here).
        while (condition(obj)):
            ...
            if iteration > max_iteration:
                logger.warning(
                    f'while_loop condition still False after {iteration} iterations. '
                    'Check logic or increase `max_iterations`?'
                )
                break
        for name in outputs:
...
def logical_not(value):
    ...
    returns a wrapped function that produces the logical opposite of the callable.
    """
    if callable(value):
        def negate(arg):
            result = value(arg)
