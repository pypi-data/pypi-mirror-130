...
class KafkaSimpleConsumer(TriggererEventSource):
    def __init__(self,
                 topic: KafkaTopic,
                 group_id: str,
    ...
    def start_generating(self) -> None:
        self.consuming = True
        threading.Thread(target=self.__repetitively_generate)

    def __repetitively_generate(self):
...
class KafkaConsumer(TriggerableEventBatchSource):
    ...
    def get_event_batch(self) -> List[Event]:
        records: Dict = \
            self.inner_consumer.poll(max_records=self.max_records) \
                if self.max_records is not None \
                else self.inner_consumer.poll(60000)
