...
def run_master(bin_path, config_path, log_path, sock_file):
    ...
    path = os.path.join(bin_path, sys.platform, arch, 'jennifer_agent')
    log_stream = open(log_path, 'w+')  # log_path == '/tmp/jennifer-python-agent.log'
    process = subprocess.Popen(
        [
            path,  # '/mnt/d/workshop2/python-agent/jennifer/bin/linux/amd64/jennifer_agent'
            config_path,  # '/mnt/d/pycharm/work/myapp/jennifer.ini'
...
def run(args):
    ...
    try:
        ...
        # print('jennifer_file_path:', jennifer_file_path) # /mnt/d/workshop2/python-agent/jennifer/__init__.py
    # except ModuleNotFoundError as e:
    # raise e
    except ModuleNotFoundError:
        jennifer_file_path = pathlib.Path(__file__).parent.absolute()

    root_dir = os.path.dirname(jennifer_file_path)
    bootstrap = os.path.join(root_dir, 'bootstrap')
    # print('jennifer_boot_path', bootstrap) # /mnt/d/workshop2/python-agent/jennifer/bootstrap
