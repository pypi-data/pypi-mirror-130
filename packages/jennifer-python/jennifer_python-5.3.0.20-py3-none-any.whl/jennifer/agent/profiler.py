...
# from jennifer.protocol.types.profile import  \
...
class TransactionProfiler(ProfileData):
    ...
    def db_open(self, host, port, db):
        msg = sql.Message(0, 'mysql:host={0};port={1};db={2}'.format(host, port, db), sql.Message.TYPE_OPEN)
    ...
    def db_execute(self, host='', port=3306, query='', params=[], style='format'):
