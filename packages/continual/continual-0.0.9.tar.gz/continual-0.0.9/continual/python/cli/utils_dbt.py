...
def generate_yaml(continual_configs, name, table_name):
    ...
    excluded_columns = continual_configs.get("continual.exclude_columns", None)
...
def process_node(
    ...
    name = continual_config.get("continual.name", node.get("name"))
    ...
    if continual_enabled:
        ...
        try:
            ...
            stub_path = None
            stub_text = None
            if continual_config.get("continual.type").lower() == "model":
                create_sources = continual_config.get("continual.create_sources")
                ...
                if continual_config.get("continual.create_stub"):
            ...
            if continual_config.get("continual.create_exposures") == True:
                entity = continual_config.get("continual.entity", name)
                table_list = entity_dict.get(entity, [])
                table_list.append(node.get("name"))
                entity_dict[entity] = table_list
                if continual_config.get("continual.type").lower() == "model":
                    entity_list = [entity]
                    for col in continual_config.get("continual.columns", []):
    ...
    if nodes:
        ...
        if model_nodes:
            ...
            continual_nodes = {
                ...
                if v.get("config").get("meta", {}).get("continual.type", None)
                is not None
