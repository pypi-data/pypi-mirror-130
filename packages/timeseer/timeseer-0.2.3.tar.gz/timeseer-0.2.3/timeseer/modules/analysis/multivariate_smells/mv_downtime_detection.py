...
<p>This check tries to automatically detect periods of downtime within a series set
...
def _run_downtime_detection(
    ...
    concatenated_df = (
        pd.concat(
            [
                ...
                for series in analysis_input.inputs
            ],
            axis=1,
            sort=False,
        )
    ...
    all_anomalies = [
        ...
        for i, analysis in enumerate(analysis_input.inputs)
    ]
...
def run(
    ...
    event_frames = _run_downtime_detection(analysis_input)
