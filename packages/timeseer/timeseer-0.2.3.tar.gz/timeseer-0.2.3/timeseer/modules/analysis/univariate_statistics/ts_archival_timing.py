...
def run(
    ...
    check_mean = timeseer.Statistic(
        META["statistics"][0]["name"], "timedelta", float(archival_mean)
    )
    check_min = timeseer.Statistic(
        META["statistics"][1]["name"], "timedelta", float(archival_min)
    )
    check_max = timeseer.Statistic(
        META["statistics"][2]["name"], "timedelta", float(archival_max)
    )
    check_median = timeseer.Statistic(
        META["statistics"][3]["name"], "timedelta", float(archival_median)
    )
    return timeseer.AnalysisResult(
        statistics=[check_min, check_mean, check_max, check_median]
    )
