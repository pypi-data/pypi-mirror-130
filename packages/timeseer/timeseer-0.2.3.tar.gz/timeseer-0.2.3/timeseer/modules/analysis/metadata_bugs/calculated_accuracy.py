...
def _calculate_range(df: pd.DataFrame):
...
def _calculated_accuracy(df: pd.DataFrame):
    calculated_range = _calculate_range(df)
    if calculated_range is None:
        return None
...
def run(
    ...
    if _calculate_range(analysis_input.data) is None:
        return timeseer.AnalysisResult()
    if _calculate_range(analysis_input.data) == 0:
        return timeseer.AnalysisResult()

    calculated_accuracy = _calculated_accuracy(analysis_input.data)
    if math.isnan(calculated_accuracy) or calculated_accuracy is None:
        return timeseer.AnalysisResult()
