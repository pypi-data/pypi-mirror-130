...
The [scripts/deploy-azure.yml](scripts/deploy-azure.yml) playbook is able to create a VM where a Timeseer wheel can be deployed.
...
(venv-ansible) $ pip install 'ansible[azure]'
(venv-ansible) $ pip install -r https://raw.githubusercontent.com/ansible-collections/azure/dev/requirements-azure.txt
...
```bash
(venv-ansible) $ ansible-playbook scripts/deploy-azure.yml --extra-vars "deploy_name=001"
```

To run this command on mac, an extra environment variable need to be added

```bash
(venv-ansible) $ OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES ansible-playbook scripts/deploy-azure.yml --extra-vars "deploy_name=001"
```
...
For example:
```bash
(venv-ansible) $ ansible-playbook scripts/deploy-azure.yml --extra-vars "deploy_name=001 cpu_cores=8"
```
...
```bash
(venv-ansible) $ ansible-playbook scripts/remove-azure.yml --extra-vars "deploy_name=001"
```
