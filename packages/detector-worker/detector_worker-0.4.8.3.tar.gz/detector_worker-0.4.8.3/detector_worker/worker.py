...
class BlobReader:
    ...
    def __init__(self, return_only_source):
    ...
    def read_next_batch(self, blob_name, offset, segment_count):
        segment_id = -1
        next_batch = []
        blobs_iterator = self.read_blob_by_segments(blob_name)
        for segment in blobs_iterator:
            segment_id += 1
            # Skip all segments until required position
            if segment_id < offset:
                continue
            next_batch.append(segment)
            if len(next_batch) == segment_count:
                break
        return next_batch
...
class MetadataRepository:
    ...
    def record_failure(self, request_id, e: Exception):
        trace = traceback.format_exc()[:MAX_PROP_SIZE]
        error = str(e)[:MAX_PROP_SIZE]
        entity = {"PartitionKey": self.worker_name, "RowKey": request_id, "FailureMessage": error, "StackTrace": trace}
        self.table_client.insert_entity(self.failure_table, entity)
...
class Worker:
    def __init__(self, worker_name: str, 
        ...
        self.reader = BlobReader(return_only_source)
    ...
    def process_message(self, request_id: str, func):    
        ...
        try:
            ...
            self.metadata_repository.record_failure(request_id, e)
    ...
    def _process_blob(self, request_id: str, metadata: TaskMetadata, func):
        try:
            if metadata.next_batch:
                result, failures, next_batch = self._process_next_batch(metadata, func)
            else:
                result, failures, next_batch = self._process_first_batch(metadata, func)
        except Exception as e:
        ...
        if next_batch:
            self._move_task_to_next_stage(metadata, result, next_batch, request_id)
        else:
    ...
    def _move_task_to_next_stage(self, metadata: TaskMetadata, result: list, next_batch: list, request_id: str):
        metadata.offset += len(result)
        metadata.batch_count += 1
        metadata.next_batch = next_batch
    ...
    def _finalize_task(self, metadata: TaskMetadata, result: list):
        ...
        if metadata.offset == 0:
            ...
            for batch in range(metadata.batch_count):
                blob_content = self.reader.read_output_blob("%s/%s_%d" % (self.worker_name, blob_name, batch))
    ...
    def _process_first_batch(self, metadata: TaskMetadata, func):
        ...
        for segment in segments_iterator:
            next_batch.append(segment)
            if len(next_batch) == self.segment_limit:
                break

        return result, failures, next_batch

    def _process_next_batch(self, metadata: TaskMetadata, func):
        # Start reading new batch asynchrounously
        batch_read_task = self.pool.apply_async(self.reader.read_next_batch, args=(
            metadata.blob_name, 
            metadata.offset + len(metadata.next_chunk), 
            self.segment_limit 
        )) 

        # Process current batch
        result, failures = func(metadata.next_chunk, metadata.params)
        ...
        next_batch = batch_read_task.get()
        return result, failures, next_batch
