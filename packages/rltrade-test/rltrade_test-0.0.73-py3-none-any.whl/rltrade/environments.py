...
class SmartDayTradeEnv2(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(self.stock_dim + 3 + len(self.tech_indicator_list), self.stock_dim),
        )
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.state[-3,:] = np.array(contracts).ravel()
            self.state[-2,:] += return_values.ravel()
            self.state[-1,:] += (return_values.ravel() - total_transaction_cost_per_stock.ravel())
