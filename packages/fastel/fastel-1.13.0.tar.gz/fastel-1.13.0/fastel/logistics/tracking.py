...
class TrackingClient:
    ...
    def verify_signature(self, time: int, signature: str, key: str) -> None:
        bytes_time = str(time).encode()
        sign = hmac.new(key=key, msg=bytes_time, digestmod=hashlib.sha256).hexdigest()  # type: ignore
        if sign != signature:
            raise ValueError("invalid signature")

    def verify(self, time: int, signature: str, keys: Set[str]) -> bool:
        for key in keys:
            try:
                self.verify_signature(time, signature, key)
                return True
            except ValueError:
                continue
        raise ValueError("invalid signature")
