...
common_errors = {} #map [input bi-gram word in error to the corrected bi-gram]
# e.g. இருக்க பிடிக்கவே,இருக்கப் பிடிக்கவே,வினையெச்சம்
CommonErrors = collections.namedtuple('CommonErrors',('error_bigram','corrected_bigram','reason'))
def get_common_errors():
    common_errors_file = os.path.join(get_data_dir('koppu'),'Error Annotated Corpus.csv')
    if len(common_errors) != 0: return common_errors
    with codecs.open(common_errors_file,'r','utf-8') as fp:
        _fp = csv.reader(fp)
        for row in _fp:
            err_bigram = re.split(SPC,row[0].strip())
            corr_bigram = re.split(SPC,row[1].strip())
            reason = row[2].strip()
            common_errors[row[0].strip()] = CommonErrors(err_bigram,corr_bigram,reason)
    if DEBUG: print("Loaded common errors records ",len(common_errors))
...
def gpathil11(mword, opt=True, mode='exe'):
    ...
    for i in range(len(mword)):
        ...
        #0 - check common mispellings
        if i < MAX_LEN:
            bigram = mword[i]+' '+mword[i+1]
            if bigram in common_errors:
                alternate : CommonErrors = common_errors[bigram]
                parinthu = addparinthu(parinthu, i, alternate.corrected_bigram[0])
                parinthu = addparinthu(parinthu, i+1, alternate.corrected_bigram[1])
                i=i+2
                continue
