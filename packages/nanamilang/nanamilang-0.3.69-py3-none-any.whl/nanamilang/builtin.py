...
class BuiltinFunctions:
    ...
    @meta({'spec': [[Spec.ArityVariants, [2]],
                    [Spec.ArgumentsTypeChainVariants, [[datatypes.Function, datatypes.Set],
                                                       [datatypes.Keyword, datatypes.Set],
                                                       [datatypes.Function, datatypes.Vector],
                                                       [datatypes.Keyword, datatypes.Vector],
                                                       [datatypes.Function, datatypes.HashMap],
                                                       [datatypes.Keyword, datatypes.hashmap]]]],
           'name': 'map',
           'type': 'function',
        ...
        if isinstance(function, datatypes.Keyword):
            ...
            function = datatypes.Function(
                {'function_name': randstr(),
                 'function_reference':
                     lambda _args_: BuiltinFunctions.get_func(_args_ + [keyword])})

        return datatypes.Vector(tuple(map(lambda e: function.reference()([e]), collection.elements())))
    ...
    def filter_func(args: List[datatypes.Base]) -> datatypes.Vector:
        ...
        f: datatypes.Function or datatypes.Keyword
        c: datatypes.Set or datatypes.Vector or datatypes.HashMap

        f, c = args

        if isinstance(f, datatypes.Keyword):
            k = f
            f = datatypes.Function(
                {'function_name': randstr(),
                 'function_reference':
                     lambda _a_: BuiltinFunctions.get_func(_a_ + [k])})

        return datatypes.Vector(tuple(filter(lambda e: f.reference()([e]).reference() is True, c.elements())))
