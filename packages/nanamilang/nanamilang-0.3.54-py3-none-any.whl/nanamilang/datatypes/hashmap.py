...
class HashMap(Base):
    ...
    def get(self, key: Base) -> Base:
        ...
        return shortcuts.get(shortcuts.get(tuple(filter(lambda d: d[0].hashed() == key.hashed(),
                                                        self.reference())), 0, ()), 1, Nil('nil'))
    ...
    def __init__(self, reference: tuple) -> None:
        ...
        self.init_assert_only_base(shortcuts.depart(reference))

        self._hashed = reduce(
            lambda e, n: e + n,
            map(lambda e: e.hashed(), shortcuts.depart(reference))
        )
