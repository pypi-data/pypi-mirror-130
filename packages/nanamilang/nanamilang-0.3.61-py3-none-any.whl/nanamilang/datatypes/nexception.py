...
class NException(Base):
    ...
    def format(self, showtraceback=False) -> str:
        ...
        _message = self._python_reference.get(Keyword('message')).format()

        traceback_str = '\n' + ''.join(traceback.format_tb(
            self._exception_instance.__traceback__)
        ) + '\n' if showtraceback else ''
        ...
        return f'Python 3 Exception has been occurred\n' \
               ...
               f'{traceback_str}' \
               f'Python 3 Exception class name: <{_name}>\n' \
               f'And the message passed to the <{_name}> was: {_message}.'
