...
def enpart(coll: tuple) -> tuple:
    """Turn collection into partitioned"""

    return tuple(coll[i:i + 2] for i in range(0, len(coll), 2))


def depart(coll: tuple) -> tuple:
    """Return partitioned into collection"""

    return functools.reduce(lambda e, n: e + n, coll or ((None,),))


def find(coll, default_value, element=None, filter_function=None):
    """Safely find value by element (maybe with filter-function)"""

    if not filter_function:
        if not element:
            return None or default_value
        # Pylint, babeee, pleeeeease...
        filter_function = lambda e: e == element

    return get(tuple(filter(filter_function, coll)), 0, default_value)
