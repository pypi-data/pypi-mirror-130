...
import zipline
from foreverbull_core.models.finance import Asset, Order, Portfolio
...
class Broker(threading.Thread):
    def __init__(self, engine, feed, configuration=None):
        self.engine = engine
        self.feed = feed
        ...
        self.router.add_route(self.engine._get_all_assets, "get_all_assets")
        ...
        self.router.add_route(self._get_portfolio, "get_portfolio")
    ...
    def _can_trade(self, asset: Asset) -> bool:
        try:
            equity = self.engine.trading_algorithm.symbol(asset.symbol)
    ...
    def _order(self, order: Order) -> dict:
        ...
        try:
            equity = self.engine.trading_algorithm.symbol(order.asset.symbol)
        except zipline.errors.SymbolNotFound as e:
            raise BrokerError(repr(e))
        order.id = self.engine.trading_algorithm.order(
            asset=equity, amount=order.amount, limit_price=order.limit_price, stop_price=order.stop_price
        )
        event = self.engine.trading_algorithm.get_order(order.id)
        order.update(event)
    ...
    def _get_open_orders(self) -> dict:
        response = {"orders": []}
        for _, open_orders in self.engine.trading_algorithm.get_open_orders().items():
            for order in open_orders:
                response["orders"].append(Order.create(order))
        return response

    def _cancel_order(self, order: Order) -> dict:
        self.engine.trading_algorithm.cancel_order(order.id)
        event = self.engine.trading_algorithm.get_order(order.id)
        if event is None:
            raise BrokerError(f"order {order.id} not found")
        order.update(event)
    ...
    def _get_portfolio(self) -> Portfolio:
        return Portfolio.from_backtest(self.engine.trading_algorithm.portfolio)
