...
def rand_mask(num_filters, num_channels, weights_per_kernel = 4):
    """
    args:
        num_filters:
        num_channels:
    ...
    mask = np.zeros((num_filters, num_channels, 3, 3))
    for f in range(num_filters):
        for c in range(num_channels):
            ...
            for k in ks[:weights_per_kernel-1]: 
                i, j = choices[k]
                mask[f, c, i, j] = 9./weights_per_kernel
...
class IrregConv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, dilation=1, mask_fn = None, weights_per_kernel = 4):
        super(IrregConv, self).__init__(in_channels, out_channels, kernel_size)
        self.mask = mask_fn(out_channels, in_channels, weights_per_kernel = weights_per_kernel)
