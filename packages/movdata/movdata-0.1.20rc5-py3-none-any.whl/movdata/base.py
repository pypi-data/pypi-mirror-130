...
class BaseMixin:
    ...
    def unpack_columns(cls, dataframe: typing.Union[gpd.GeoDataFrame, pd.DataFrame],
        ...
        if isinstance(keyname, str):
            dataframe[keyname] = dataframe[column].map(functools.partial(BaseMixin.get_value, keyname=keyname))
        elif isinstance(keyname, list):
            for key in keyname:
                dataframe[key] = dataframe[column].map(functools.partial(BaseMixin.get_value, keyname=key))
