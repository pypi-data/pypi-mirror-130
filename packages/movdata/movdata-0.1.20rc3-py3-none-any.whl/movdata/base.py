...
class BaseMixin:
    ...
    @classmethod
    def unpack_columns(cls, dataframe: typing.Union[gpd.GeoDataFrame, pd.DataFrame],
                       column: str,
        ...
        if isinstance(keyname, str):
            dataframe[keyname] = dataframe[column].map(functools.partial(lambda item: item[keyname], keyname=keyname))
        elif isinstance(keyname, list):
            for key in keyname:
                dataframe[key] = dataframe[column].map(functools.partial(lambda item: item[keyname], keyname=key))
...
class Relocations:
    ...
    def unpack_additional(self, keyname: typing.Union[str, typing.List[str]]):
        """function to extract values from metadata (additional) column and create independent column"""
        BaseMixin.unpack_columns(dataframe=self._gdf, column='additional', keyname=keyname)
