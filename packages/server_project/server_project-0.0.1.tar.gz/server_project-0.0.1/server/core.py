...
from decos import DecoratorLogger, logger, login_required
from data.variables import MAX_CONNECTIONS, ENCODING, MAX_PACKAGE_LENGTH, DESTINATION, ACTION, \
    SENDER, MESSAGE, MESSAGE_TEXT, RESPONSE_200, RESPONSE_400, ERROR, ACCOUNT_NAME, \
    EXIT, GET_CONTACTS, USER, RESPONSE_202, LIST_INFO, ADD_CONTACT, REMOVE_CONTACT, USERS_REQUEST, PUBLIC_KEY_REQUEST, \
    RESPONSE_511, DATA, RESPONSE, PUBLIC_KEY, RESPONSE_205, TIME, PRESENCE
...
class MessageProcessor(threading.Thread):
    ...
    def send_message(self, sock, message):
        '''
            ...
            '''
        print(f'[INFO] Sending Data {message} to Client!')
        js_message = json.dumps(message)
        encoded_message = js_message.encode(ENCODING)
        sock.send(encoded_message)
        print('[INFO] Data sent succesfully to address!')
    ...
    def process_client_message(self, message, client):
        ...
        print('SERVER: ', message, message.get(ACTION) == MESSAGE, DESTINATION in message, TIME in message, SENDER in message, MESSAGE_TEXT in message, self.names.get(message.get(SENDER)) == client)
    ...
    def service_update_lists(self):
        ...
        for client in self.names:
            try:
                self.send_message(self.names[client], RESPONSE_205)
            except OSError:
