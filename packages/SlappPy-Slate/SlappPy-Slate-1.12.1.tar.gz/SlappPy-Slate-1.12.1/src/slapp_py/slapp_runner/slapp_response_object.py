import logging
from typing import Dict, Union, List, Tuple, Optional, Iterable, SupportsInt
...
class SlappResponseObject:
    ...
    def get_placements_by_place(self, p: Player, place: Union[int, SupportsInt] = 1) -> List[Tuple[SimpleSource, Bracket]]:
        """
            ...
            for bracket in brackets:  # Note: We need the bracket name so it's easier to do as a rolled-foreach loop
                if place in bracket.placements.players_by_placement:
                    ...
                    if p.guid.__str__() in first_place_ids:
                        result.append((self.sources[source_id], bracket))
        return result
