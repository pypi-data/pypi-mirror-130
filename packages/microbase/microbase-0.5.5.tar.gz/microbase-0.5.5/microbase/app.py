...
from microbase.middlewares import MiddlewareType, group_middleware_decorators
...
class Application(object):
    ...
    def __init__(self):
        ...
        self._middlewares: List[Tuple[MiddlewareType, Union[List[str], str], Callable]] = []
        self._init_logging()
    ...
    def add_error_handler(self, exc: Union[Type[BaseException], List[Type[BaseException]]], status_code: int = 500,
        ...
        :param message:
        :param callback:
        :param status_code_field: Название поля в exception'е, откуда получать status code ответа
        :return:
        """
        handler = ErrorHandler(exc, status_code, message, callback, status_code_field)
    ...
    def add_middleware(self, middleware_type: MiddlewareType, middleware: Callable,
        ...
        self._middlewares.append((middleware_type, group, middleware))
    ...
    def _apply_routes(self, sapp: Sanic):
        decorators = group_middleware_decorators(self._middlewares)

        bp = Blueprint(name=self.config.APP_PREFIX, url_prefix=self.config.APP_PREFIX)

        for r in self._routes:
            handler = r.handler
            if r.group in decorators:
                handler = decorators[r.group](handler)

            bp.add_route(handler, r.uri, methods=r.methods, strict_slashes=r.strict_slashes, name=r.name)
    ...
    def _apply_middlewares(self, sapp: Sanic):
        for m_type, group, middleware in self._middlewares:
            if group is None:
                sapp.register_middleware(middleware, m_type.value)
    ...
    def run_script(self, script_func):
        async def _run_handlers(handlers: List, loop_: asyncio.AbstractEventLoop):
            for handler_ in handlers:
                await handler_(self, loop_)

        async def _run_script_function(fn):
        ...
        if not callable(script_func):
            raise ApplicationError(f'Argument `script_func` must be callable')

        before_start = [hook for ht, hook in self._hooks if ht == HookNames.before_server_start]
        before_start += [hook for ht, hook in self._hooks if ht == HookNames.after_server_start]
        after_stop = [hook for ht, hook in self._hooks if ht == HookNames.before_server_stop]
        after_stop += [hook for ht, hook in self._hooks if ht == HookNames.after_server_stop]
        ...
        loop.run_until_complete(_run_handlers(before_start, loop))
        try:
            loop.run_until_complete(_run_script_function(script_func))
        finally:
            loop.run_until_complete(_run_handlers(after_stop, loop))
