...
def custom_opener(follow=None, policy=None, force_min=None, force_max=None):
    ...
    # regarding the handler_order thing:
    # starts with the smallest #
    #  100  ProxyHandler
    #  500  defaults
    # 1000  HTTPErrorProcessor
...
class BrowserlyHeaderHandler(BaseHandler):
    ...
    def http_request(self, req):
        req.add_unredirected_header('Accept', 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8')
...
def FindBestHandler(BaseHandler):
    """ try to figure out the best handler via grading function """

    def __init__(self, handlers, grader, try_n=3):
        self.handlers = handlers
        self.grader = grader
        self.try_n = try_n

    def http_request(self, req):
        if not hasattr(req, handers_pending):
            # first time, choose random handlers
            req.handlers_pending = random.sample(self.handlers, self.try_n)

        # pick next one
        req.handlers_current = req.handlers_pending.pop()

        # apply selected handler
        req = req.handlers_current(req)

        return req

    def http_response(self, req, resp):
        # FIXME data?
        grade = self.grader(req, resp, data)

        if not hasattr(req, handlers_grades):
            req.handlers_grades = []
            req.handlers_responses = []

        req.handlers_grades[req.handlers_current] = grade
        req.handlers_responses[req.handlers_current] = data

        if len(req.handlers_pending) > 0:
            # haven't tried em all yet
            new = Request(req.get_full_url(), headers=req.headers) #FIXME

        else:
            # we finished 'em all!
            pass

        # FIXME issue w/ data_response(). shitty wrapper...
