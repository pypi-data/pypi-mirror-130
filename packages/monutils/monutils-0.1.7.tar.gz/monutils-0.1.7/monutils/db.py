...
def uri_connect(host: str = 'localhost',
                port: int = 27017,
                replicaset: str = None,
                user: str = None,
                password: str = None,
                ...
                mode: Mode = Mode.AUTO,
                timeout: int = MAX_SEV_SEL_DELAY) -> MongoClient:
    ...
    if cert_key_file:
        if replicaset:
            client = MongoClient(uri, replicaset=replicaset, tls=True, tlsCertificateKeyFile=cert_key_file,
                                 tlsCAFile=ca_file, serverSelectionTimeoutMS=timeout)
        else:
            client = MongoClient(uri, tls=True, tlsCertificateKeyFile=cert_key_file,
                                 tlsCAFile=ca_file, serverSelectionTimeoutMS=timeout)
    elif replicaset:
        client = MongoClient(uri, replicaset=replicaset, serverSelectionTimeoutMS=timeout)
    else:
...
def auth_connect(host: str = 'localhost',
                 port: int = 27017,
                 replicaset: str = None,
                 user: str = None,
                 password: str = None,
    ...
    if replicaset:
        return MongoClient(host, port, replicaset=replicaset, username=user, password=password,
                           authMechanism=mode.value, serverSelectionTimeoutMS=timeout)
...
def connect(host: str = 'localhost',
    ...
    if not user or mode == Mode.NONE:
        return uri_connect(host, port, replicaset, mode=Mode.NONE, timeout=timeout)
    if mode == Mode.AUTO:
    ...
    if mode in [Mode.SCRAM_SHA_256, Mode.SCRAM_SHA_1]:
        return auth_connect(host, port, replicaset, user, password, mode, timeout)
    return uri_connect(host, port, replicaset, user, password, cert_key_file, ca_file, session_token, mode, timeout)


def connect_database(host: str = 'localhost',
                     port: int = 27017,
                     replicaset: str = None,
                     database: str = None,
                     user: str = None,
                     ...
                     session_token: str = None,
                     mode: Mode = Mode.AUTO,
                     timeout: int = MAX_SEV_SEL_DELAY) -> Database:
    """ Connect with the database.
    :param host: The database connection host.
    :param port: The database connection port.
    :param replicaset: The replicaset. If None, then, replicaset is not used.
...
def detect_connection(host: str = 'localhost',
                      port: int = 27017,
                      replicaset: str = None,
                      user: str = None,
                      passw: str = None,
