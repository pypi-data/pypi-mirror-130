...
def create_prospect_report(data, quarter_name, output_path):
    ...
    pd.DataFrame().to_excel(writer, index=False, sheet_name="3_filtered_funds")
    workbook = writer.book
    target_list_sheet = writer.sheets["1_target_list"]
    full_details_sheet = writer.sheets["2_full_details"]
    filtered_funds_sheet = writer.sheets["3_filtered_funds"]

    full_details_sheet.write("N1", 1)
    full_details_sheet.write("O1", "=SUBTOTAL(109, N1)")
    for i in range(len(full_details_df)):
        n_cell = "N" + str(i+2)
        o_cell = "O" + str(i+2)
        full_details_sheet.write(n_cell, 1)
        full_details_sheet.write(o_cell, "=SUBTOTAL(109, {cell})".format(cell=n_cell))
    
    filtered_funds_sheet.write("A1", "=UNIQUE(FILTER('2_full_details'!A:A, '2_full_details'!O:O>0))")
    ...
    full_details_sheet.set_column("N:N", None, None, {'hidden': True})
    full_details_sheet.set_column("O:O", None, None, {'hidden': True})
    filtered_funds_sheet.set_column(0, 0, 60)
